...
(the topology rule: each unique-seam bridge is a singleton, while seam-free engines may scale and bridges never execute engine lanes): **three bridges + an
...
execution work to the engineering-manager — is **dead** (the topology rule: each unique-seam bridge is a singleton, while seam-free engines may scale and bridges never execute engine lanes): a planned child becomes
...
## Relationship to companion pipeline skill suite and pipeline-crew-mcp

- **pipeline-crew consumes the companion pipeline skill suite, never the reverse.** The
...
companion pipeline skill suite carries a **control-plane (§CP)** boundary: PRs that touch the agent control
plane bank for a human merge behind a hard GitHub gate (the control-plane rule: a non-author must approve the current head before the pipeline enqueues the change).
...
cartographer (the topology rule: each unique-seam bridge is a singleton, while seam-free engines may scale and bridges never execute engine lanes, the on-demand cartographer rule). To bring up **one**
...
- [`EXPLANATION.md`](EXPLANATION.md) — the *why* behind the four defs: the roster law, the §CP
  hard gate, verify-don't-relay, and single-owner human comms — pointing out to the roster, approval, and gate invariants.
...
- the update rule: omit a plugin version so commit-addressed installations can receive updates — why neither
