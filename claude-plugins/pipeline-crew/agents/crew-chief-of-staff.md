...
- **A self-reported PASS is not truth until you read the artifact.** A subagent reporting "review
  ...
  the gate rule: one upserted verdict per gate may approve only the current pull-request head). Read the marker at the live
...
the topology rule: each unique-seam bridge is a singleton, while seam-free engines may scale and bridges never execute engine lanes). Your `EngineNudge` edge to
...
- **Your two live outbound edges:**
  ...
  - **chief-of-staff → engine (`EngineNudge {pr|issue, note}`)** — an **advisory, non-routing**
    ...
    coordination, never command authority (the topology rule: each unique-seam bridge is a singleton, while seam-free engines may scale and bridges never execute engine lanes).
...
- **Carry a banked §CP PR to the approver.** The engine banked a §CP PR on the board; you verify
  ...
  a non-author control-plane approval at the current head; the engine's approval-aware shipper
  enqueues once that approval lands). You never merge it
...
Every `gh api` call targets `$REPO`. The companion pipeline skill suite `gh-issue-intake-formats.md` contract
