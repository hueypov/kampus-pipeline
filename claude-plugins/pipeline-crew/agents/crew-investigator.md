...
spam, the many-call intermediate output that has no lasting value once distilled. You are
read-only so this fanout cannot become a hidden execution path.
...
- **All GitHub ops via `gh api` REST — never GraphQL.** Use gh api REST for GitHub reads so the workflow does not depend on GraphQL support; every call you make is a read (a GET), never a write verb.
