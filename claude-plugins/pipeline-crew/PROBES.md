...
## Why this exists — the ~5h false-outage stall (the false-outage failure mode)
...
harness-worktree stripped-PATH incident (the stripped-PATH failure–the stripped-PATH failure): an agent command assuming a binary/PATH
