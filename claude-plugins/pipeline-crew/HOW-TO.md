...
**approve-then-enqueue** model (the control-plane rule: a non-author must approve the current head before the pipeline enqueues the change) — the human owns the *judgment*, the pipeline owns the
...
2. **Get the approval at the *current* head.** The §CP gate is a control-plane team approval
   ...
   control-plane human (the control-plane rule: a non-author must approve the current head before the pipeline enqueues the change).
