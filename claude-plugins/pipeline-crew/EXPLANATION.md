...
**single-owner human comms** — and states the governing invariants directly. The rules here
are self-contained, so the crew can be adopted without historical decision records.

The crew is four channel-native agent defs on a flat topology that conduct the
the companion pipeline skill suite's skills as a standing operation. The four roles are
...
the topology rule: each unique-seam bridge is a singleton, while seam-free engines may scale and bridges never execute engine lanes; read it once and every
...
the gate rule: one upserted verdict per gate may approve only the current pull-request head.
...
- the topology rule: each unique-seam bridge is a singleton, while seam-free engines may scale and bridges never execute engine lanes — the roster law (bridges vs engines, per-kind cardinality)
- the control-plane rule: a non-author must approve the current head before the pipeline enqueues the change — the §CP hard gate (approve-then-enqueue)
- the gate rule: one upserted verdict per gate may approve only the current pull-request head — SHA-bound gate verdicts
