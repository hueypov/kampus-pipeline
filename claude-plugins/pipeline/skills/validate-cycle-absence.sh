...
for entry in "${CYCLE_SKILLS[@]}"; do
	...
	if ! grep -qiE "$noop_re" "$md"; then
		fail "$skill: no absent⇒no-op branch found (expected /$noop_re/i) — an absent cycle doc must no-op gracefully (the repository-resolution rule that uses an explicit override or the current checkout, never a hardcoded repository)"
	else
		ok "$skill has an absent⇒no-op branch"
	fi
done

# No cycle-aware skill may hardcode the adopting repository's flag policy unconditionally. The flag/dark-ship
...
if [ "$errors" -gt 0 ]; then
	echo "validate-cycle-absence: FAILED — $errors error(s); the graceful-absence guarantee (the repository-resolution rule that uses an explicit override or the current checkout, never a hardcoded repository / 0083) is broken"
	exit 1
fi
