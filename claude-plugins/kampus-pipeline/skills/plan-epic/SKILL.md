...
children from, and every child traces back to one. See the applicable safety invariant for why.
...
The Use gh api REST for GitHub operations so the pipeline does not require GraphQL support. Every read and write goes through `gh api`. Native sub-issues have a REST
...
- **`## Dependencies` grammar** (format 1) — the topology you pin at the bottom of
  ...
  **Topology only** — no retry budgets, no concurrency caps, no code flags; those are
  ...
  job — the applicable safety invariant; the topology is the
  only dependency artifact the adopting repository keeps.)
...
# Acquire the two-layer lock. `epic-lock acquire` runs the WHOLE the claim-ownership rule that gives work to the earliest authorized session-stamped claim protocol over $CLAUDE_CODE_SESSION_ID
...
# silently-swallowed DELETE LEAKS the lock and wedges the epic, the exact the planning lock that gives one active planner ownership and releases on completion or expiry catastrophe).
$LOCK release <EPIC>
```
...
- **Testing strategy** — which behaviors/modules get tested and why, and at which tier (cite
  the applicable safety invariant's T0–T3
  taxonomy); choose unit tests for pure decisions and control flow behind substituted seams, and
  integration tests only for behavior that depends on real deployed infrastructure. Test behavior,
  not implementation; follow relevant prior art in the repo. This is what sets each child's `**TDD:**`
...
- **Vocabulary impact** — the required point-of-coining glossary catch (see [§Vocabulary
  ...
  **or** a plainly-stated "none" when the plan introduces no new term. This section is never
  ...
  coining-time authoring hook only, off the fail-closed `review-code` gate (the vocabulary rule that captures newly coined or redefined terms at their source and through a backstop sweep prong (c)).
...
the set is a human roadmap act, the applicable safety invariant §3) and assigns a child only to the epic's existing
...
re-derive it here.** The *why* is the rule that repository-specific release-cycle behavior no-ops when the repository has no cycle document
...
concurrency caps, or code flags (those are the out-of-repo orchestrator's, per the applicable safety invariant).
...
the SHA-bound verdict contract, the verdict rule that upserts one verdict per gate and accepts it only when bound to the PR current head SHA). Don't claim a "lock"; claim "no silent lost-update of
