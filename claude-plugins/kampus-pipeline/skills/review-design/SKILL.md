...
This gate is the review surface the applicable safety invariant's Consequences named ("checks every UI PR against these four
...
look like. You only know what the applicable safety invariant forbids and what the preview *actually renders*. Judge the
second against the first, from the outside.

## The judged source is the LOCAL captured bytes; the upload is evidence-only (the rule that requires independent visual review for UI-affecting changes)
...
### The six hard-FAIL prohibitions (enumerated from the applicable safety invariant §Prohibitions)
...
1. **Deterministic diff (the objective signal, never the verdict).** For each changed *blessed*
   ...
   never read as deviation). The diff is a **signal**, not a verdict (the seam's own contract, the applicable safety invariant) — a large `magnitude` does **not** by itself FAIL anything.
...
3. **Non-trivial deviation → ESCALATE to your multimodal judgment.** A non-trivial `magnitude`
   ...
   - **Explained / justified → PASS (the intentional-redesign branch, story 8).** The PR
     **intentionally and legitimately** changes this surface — the linked issue / PR body says it
     ...
     founder keeps the golden current with an explicit **re-bless** (`golden-bless`, story 9, the applicable safety invariant)
...
namespace, then a SHA-staleness refusal (the verdict rule that upserts one verdict per gate and accepts it only when bound to the PR current head SHA). Your verdict's first line is **always**
...
The Use gh api REST for GitHub operations so the pipeline does not require GraphQL support.
...
**Target repo resolution** ([`../gh-issue-intake-formats.md`](../gh-issue-intake-formats.md), the repository-resolution rule that uses an explicit override or the current checkout, never a hardcoded repository §1); in the adopting repository this defaults to `<owner>/<repository>` with no config.
...
The design contract you verify against is **the applicable safety invariant + the design-system-manifest**, not an issue's
...
The pipeline already produces a **per-PR preview deploy** (the applicable safety invariant):
...
render-exception still apply to it). The pointer is the committed source of truth (the applicable safety invariant); its
...
it is embedded in the verdict as evidence only (the rule that requires independent visual review for UI-affecting changes). If a capture's `hostedUrl` is `null` (an
...
confidently call below 4.5:1 from the capture — is downgraded to an advisory note, not a FAIL** (the rule that requires independent visual review for UI-affecting changes). When in doubt, advisory.
...
only add a FAIL, it removes none of the seven checks and promotes no taste note to blocking (the rule that requires independent visual review for UI-affecting changes
...
Rendered PR #<PR> over the preview deploy and judged the changed UI surfaces against the visual-quality rule that prohibits the six objective UI failures
...
Judged the changed UI surfaces against the visual-quality rule that prohibits the six objective UI failures four-pillars law (local captured bytes; hosted
...
Do **not** post a native `REQUEST_CHANGES` review — `review-design` is comment-only (the verdict rule that upserts one verdict per gate and accepts it only when bound to the PR current head SHA rule
...
verdict_readback_guard "$CID" review-design "$HEAD_SHA" \
  || { echo "read-back failed — re-post the real verdict and re-assert; if it still can't land clean, surface a posting failure (the PR is genuinely ungated) — never swallow it (fail-closed, the applicable safety invariant §ZS)"; }
```
...
decides whether your marker binds `ship-it` or merely advises is the control-plane rule that requires human approval for changes to automation, CI, or merge safeguards
...
its own authority (`ship-it` does that) nor strays into another's namespace. You realize the rule that requires independent visual review for UI-affecting changes, the review
surface the applicable safety invariant
