...
You are the **gather-and-present** layer of the founder-facing ship digest (epic <related work item>). The
...
issues <related work item>/<related work item>/<related work item>). That core is **deliberately IO-free**: it consumes a pre-gathered
...
built, adds a `.github/workflows/` cron, which is control-plane and human-merged per the control-plane rule that requires human approval for changes to automation, CI, or merge safeguards, not
part of this skill).

**You gather and present; you never flip a flag.** The live/dark axis is *read* from Flagship, never
written — the human remains the sole releaser (the rule that repository-specific release-cycle behavior no-ops when the repository has no cycle document; the applicable safety invariant Consequences). This skill reads
authoritative state to *report* it; it does not automate release.

## All `gh` via REST — never GraphQL

Every issue/PR/label read goes through `gh api` REST. The the adopting organization org runs a legacy
...
([`../gh-issue-intake-formats.md`](../gh-issue-intake-formats.md), the repository-resolution rule that uses an explicit override or the current checkout, never a hardcoded repository):
...
`@kampus/pipeline-cli` CLI via `pnpm dlx` (the shared-CLI rule that provides reusable ledger mechanics outside a repository-local package; epic <related work item>). Build it once:

```bash
if [ -f packages/pipeline-cli/src/bin.ts ]; then
  DIGEST="node packages/pipeline-cli/src/bin.ts ship-digest"   # the adopting repository-local: the in-repo consolidated bin
else
  # foreign install: the PUBLISHED consolidated CLI; the pin is the single source-of-truth version
  # shared with the other skills' published-fallback (epic <related work item>) — bump in lockstep on release.
...
```bash
# per merged PR: title, its `area:*` label (join-free product/infra signal, <related work item>), and the
...
- **`area:*` label** → the entry's **`area`** (`product` from `area:product`, `infra` from
  ...
  in [`../gh-issue-intake-formats.md`](../gh-issue-intake-formats.md); <related work item>). Absent is well-formed
...
This is the axis that answers **"what is actually LIVE to users,"** not just what merged. Per the applicable safety invariant
the state is sourced from **authoritative Cloudflare Flagship values** (the repository-defined feature-flag substrate rule substrate),
...
out-of-band release flip (the rule that repository-specific release-cycle behavior no-ops when the repository has no cycle document), which leaves no in-repo trace.
...
**no-match percentage split**, never a `defaultVariation` flip (<related work item>), so only the effective
serving is truthful — a split-released flag's `defaultVariation` stays `off` forever:

```bash
# authoritative Flagship state — every flag × env with its enabled/default value (the applicable safety invariant).
...
- **Flag-gated feature** (the merged work rides a Flagship flag — the authorship-loop /v1-membrane
  class, <related work item>): look up that flag's **effective serving** (`SERVES`) for the prod env in the
  `flag list` output.
  - **`on@100% (split)` or `on (default)`** → `live` (a split-released flag is live even though its
    `defaultVariation` is off — never read the default as the release state, <related work item>).
  ...
  - **`off (default)` (no split serving, not yet released)** → `dark` (merged behind a default-off
    flag; live only once a human releases it — the rule that repository-specific release-cycle behavior no-ops when the repository has no cycle document).
...
- **Non-flag-gated work** (internal / refactor / infra / docs — the rule that repository-specific release-cycle behavior no-ops when no cycle document exists exemptions, no flag to
...
- **No resolvable flag/release state** — a feature that *should* be flag-gated but you cannot map to
  a flag, or the read is inconclusive → **`unknown`**. Never silently treat it as `live` (the applicable safety invariant;
...
trust boundary (`packages/pipeline-cli/src/tools/ship-digest/command.ts`). Validate untrusted JSON
there, then pass only typed data downstream. Each entry:
