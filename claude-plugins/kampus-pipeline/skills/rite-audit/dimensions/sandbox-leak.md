...
(<related work item>): a çaylak's write lands `sandboxed_at`-stamped while `the adopting repository-authorship-loop` is on, and
...
- **`/search`** — `features/search/Search.ts` filters via `postVisibleWhere` (the shared sandbox
  arm, the applicable safety invariant), so a sandboxed item must not appear in results.
...
functional-rite dimension (<related work item>) **promotes** its çaylak (its T4 vouch tandem flips the author to
...
for the <related work item> verdict report. A single surface where the sandbox content reaches anon or the second
