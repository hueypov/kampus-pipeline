...
- A çaylak's new sözlük definition lands **sandboxed** when the flag is on
  ...
  <related work item>). Sandboxed content earns the author **global karma** only through the divan
...
- **assert / record** — PASS iff `/divan` resolves for the test-mod and the candidate appears in
  the roster. `/divan` 404 ⇒ **BLOCKED** (the flag-force seam <related work item> is broken — the rite cannot be
...
screenshots as evidence at each transition, and hand the bundle to the harness for the <related work item>
verdict report.
