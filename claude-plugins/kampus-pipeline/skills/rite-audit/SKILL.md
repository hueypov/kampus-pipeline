...
name: rite-audit
description: >-
  Drive the Playwright MCP against a flag-on audit stage to walk the v1 earned-authorship rite (çaylak→yazar) as an agentic explorer and emit raw pass/fail findings per dimension. Trigger on "run the rite audit", "audit the authorship rite", "rite-audit", "walk the çaylak→yazar rite", "/rite-audit". This is the audit harness's explorer: it consumes the ephemeral stage lifecycle (@kampus/audit-stage, <related work item>) — its base URL + minted test-mod — provisioned by epic <related work item>, runs each registered dimension over that one stage, and produces the raw findings the verdict report (<related work item>) structures and archives. It never deploys, seeds, or destroys the stage (that is @kampus/audit-stage), and never runs against production.
---
...
You run **against an ephemeral audit stage, never production.** The `the adopting repository-authorship-loop`
flag is an all-or-nothing kill-switch with no targeting — flipping it on in production releases
v1 to the public — so the rite is only walkable where <related work item> forces the flag on: the dedicated
`audit` deploy class. The stage is provisioned, seeded, and torn down by
[`@kampus/audit-stage`](<repository URL>)
(<related work item>); **you are the run hook it invokes**, not the lifecycle. You receive the live stage's
...
- **`@kampus/audit-stage` (<related work item>) owns the stage.** It deploys the `audit` stage (flag forced
  on by <related work item>), preview-seeds it, mints a login-able test-mod, calls **you** through its
...
- **The verdict report (<related work item>) owns the archive.** You emit *raw* findings (the union of every
  dimension's `Finding`s); <related work item> structures them into the dated, run-over-run-comparable verdict
...
Grounded in [`apps/web/src/App.tsx`](<repository URL>).
...
> `/divan` **self-gates on `the adopting repository-authorship-loop`** (404 when the flag is off — `App.tsx`).
> On the audit stage <related work item> forces the flag on, so `/divan` resolves for an authorized
...
documented interface a11y (<related work item>) and sandbox-leak (<related work item>) plug into and that the verdict
report (<related work item>) aggregates; treat it as the contract, not a suggestion.
...
| `id` | File | Status |
| --- | --- | --- |
| `functional-rite` | [`dimensions/functional-rite.md`](./dimensions/functional-rite.md) | active (this child, <related work item>) |
| `accessibility` | [`dimensions/accessibility.md`](./dimensions/accessibility.md) | active (<related work item>) |
| `sandbox-leak` | [`dimensions/sandbox-leak.md`](./dimensions/sandbox-leak.md) | active (<related work item>) |
...
2. **Preflight the gate.** Navigate `${baseUrl}/divan` as the test-mod; if it 404s, the
   flag-force seam (<related work item>) is broken — record a BLOCKED precondition and stop the functional
...
4. **Emit the raw findings bundle** — the union of every dimension's `Finding`s, with the
   ...
   verdict report (<related work item>)** for structuring/archiving; do not format the dated archive here.
...
- It does **not** deploy, seed, mint, or destroy the stage — that is `@kampus/audit-stage` (<related work item>).
- It does **not** run against production — only a flag-on `audit` stage (the flag is a public
  release switch; the rule that repository-specific release-cycle behavior no-ops when the repository has no cycle document).
- It does **not** structure or archive the dated verdict — it emits raw findings; <related work item> archives.
- It does **not** implement the a11y or sandbox-leak dimensions — those are <related work item> / <related work item>, added
