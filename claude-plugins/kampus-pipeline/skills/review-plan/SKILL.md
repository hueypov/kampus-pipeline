...
Read the applicable safety invariant — it is the binding spec
...
that stops `review-code` from merging (the applicable safety invariant Decision 3).
...
attached to a PASS* — it **never** changes the pass/fail decision (the applicable safety invariant Decision 2).
...
### Route an in-scope soft-advisor finding by appending a child AC (the applicable safety invariant)
...
These two are the parts the applicable safety invariant left to the implementer; recording them here so the next
...
not a second program, not an LLM call baked into `epic-ledger`.** Rationale: the applicable safety invariant Decision 2 draws the line at *determinism* — the floor is code because it must be
...
wire a runner the repo doesn't define (the orchestrator is deliberately out-of-repo, the applicable safety invariant) and make the loop untestable without spawning a real agent.
...
to the child it traces to (out-of-scope → `report`, the applicable safety invariant — soft-advisor lane only, the
