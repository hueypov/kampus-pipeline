...
# (issue <related work item>). Two invariants, both runnable in CI/locally:
#
#   1. CONTROL_PLANE_RE ↔ single-source const — the §CP boundary is now single-sourced
#      as the CONTROL_PLANE_RE const in packages/pipeline-cli (issue <related work item>), emitted by
#      `pipeline-cli control-plane-paths`. The gate skills no longer carry a copy (they
#      re-resolve the boundary from origin/main at run time, <related work item>). The ONE un-importable
...
#
# The single source for invariant 1 is the pipeline-cli const (the skill-review rule that gives skills their own behavioral gate §6; <related work item>): the
...
#
# The §CP boundary is single-sourced in packages/pipeline-cli (<related work item>), emitted by
...
# line of gh-issue-intake-formats.md that the live gates re-resolve from origin/main (<related work item>).
...
# Invariant 1b: the §CP canonical GUARD_ADR_RE is present (the applicable safety invariant, <related work item>).
#
# The guard-touching-ADR content predicate (the applicable safety invariant, <related work item>) is single-sourced in §CP as one
# canonical GUARD_ADR_RE= line in gh-issue-intake-formats.md. Since <related work item> there is NO
...
if [ -z "$GA_CANONICAL" ]; then
	fail "§CP canonical GUARD_ADR_RE= not found in $FORMATS_MD — line must start with GUARD_ADR_RE= (the applicable safety invariant; the guard-content-probe verb reads this single source)"
else
	ok "§CP canonical GUARD_ADR_RE present in gh-issue-intake-formats.md (read by pipeline-cli guard-content-probe)"
fi

# Invariant 1c: HAS_*_RE class-fan copies match §CP (issue <related work item>).
#
...
# 0's class fan. A stale copy mis-classes a PR's changed-path fan (the <related work item>
# `.glossary/**→has-code` miss that emptied a review namespace and stalled ship-it),
# and that drift was caught only by hand during <related work item>/<related work item> — the exact gap this
# invariant closes. Kept as a pure-bash grep-extract-then-diff (the HAS_*_RE regexes
# are not part of <related work item>'s CONTROL_PLANE_RE single-sourcing) rather than routed through
...
# (issue <related work item>: reviewer.md's copy was unguarded — drifting it left this gate GREEN). Paths
...
for name in $HAS_NAMES; do
	...
	if [ -z "$HAS_CANONICAL" ]; then
		fail "§CP canonical $name= not found in $FORMATS_MD — line must start with $name='…' (issue <related work item>)"
		continue
	fi
	...
	for rel in $HAS_CONSUMERS; do
		...
		if [ -z "$LINE" ]; then
			fail "$rel: no $name='…' line found — consumer must carry a copy matching §CP (issue <related work item>)"
			continue
		fi
		...
		# swallowed the sibling assignment on a compound line and false-FAILed (issue <related work item>).
		VAL_CLEAN=$(printf '%s\n' "$LINE" | sed "s/.*$name='\([^']*\)'.*/'\1'/")
...
if [ "$errors" -gt 0 ]; then
	echo "validate-gate-path-drift: FAILED — $errors error(s); gate-path invariants are broken (issue <related work item>)"
	exit 1
fi
