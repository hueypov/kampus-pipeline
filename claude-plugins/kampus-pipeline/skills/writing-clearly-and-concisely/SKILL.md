...
guidance — **what to do** (Strunk) and **what to avoid** (AI tells) — plus the adopting repository
...
comments the adopting repository keeps: a local invariant at its enforcement site, a workaround plus its
...
concision** — a `// See the applicable safety invariant` line or a one-line invariant is already at its floor;
"omit needless words" never means deleting the load-bearing note.

**House idioms are exempt.** the adopting repository's own conventional phrasings — `§CP`, `Fixes #N`,
