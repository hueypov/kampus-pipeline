...
A skill is **neither product code nor prose** (the skill-review rule that gives skills their own behavioral gate). The things that
...
on top of AC-verification. This gate **supersedes the routing rule that treats skills as behavioral artifacts rather than ordinary documentation's** routing of
...
## Config-pin is mandatory — you review the skills with the BASE review-skill, not the PR's (the applicable safety invariant)
...
The mechanism is identical to `review-code` Step 2's (the applicable safety invariant refinement of
...
per namespace by timestamp, then a SHA-staleness refusal (the verdict rule that upserts one verdict per gate and accepts it only when bound to the PR current head SHA). Your verdict's first line
...
The Use gh api REST for GitHub operations so the pipeline does not require GraphQL support. Every issue/PR/review/comment read and write goes through `gh api` REST. This is not
...
*both* lists. These are the four checks the existing gates structurally miss (the skill-review rule that gives skills their own behavioral gate §1):
...
4. **Gate-invariant preservation — the catastrophic case.** Does the edit *quietly weaken a
   ...
   (the skill-review rule that gives skills their own behavioral gate §1): a PR that removes `ship-it`'s control-plane refusal, softens `review-code`'s
   conjunctive AC bar, loosens a marker matcher so a forged verdict slips through, drops the
   the authorization rule that accepts privileged actions only from repository members author-gate, weakens the verdict rule that upserts one verdict per gate and accepts it only when bound to the PR current head SHA SHA-staleness refusal, or relaxes the
   ADR-the related safeguards config-pin — can pass an AC-gate clean ("it meets the issue") while it has
   ...
   - the formats contract (`gh-issue-intake-formats.md`): the §CP canonical set, the §5/§6/§6.5
     matcher contracts, the verdict rule that upserts one verdict per gate and accepts it only when bound to the PR current head SHA SHA-binding + upsert rules — none narrowed or made
     ambiguous.
   - Out of scope (do **not** flag): a PR that merely *exercises* the rule that makes gate-critical skill changes control-plane changes's coarse
     ...
     — the skill-review rule that gives skills their own behavioral gate §4.)
...
and the four behavioral failures the skill-review rule that gives skills their own behavioral gate enumerated; together they are still blind to a
...
`review-skill` lands its verdict **only as the SHA-bound comment, never a native review** (the verdict rule that upserts one verdict per gate and accepts it only when bound to the PR current head SHA rule 4 — like `review-doc`): a native review can't carry the `@ <sha>` in the shape this
...
binding verdict with this advisory line — exactly one `review-skill` verdict per PR (the verdict rule that upserts one verdict per gate and accepts it only when bound to the PR current head SHA
rule 2). The advisory **first line** carries no `@ <sha>` by design (SHA-less, so it never enters
`ship-it`'s auto-merge namespace — the rule that keeps advisory control-plane evidence separate from merge-authorizing verdicts); the reviewed head IS recorded, once, in the body's
canonical `Reviewed-head: @ <HEAD_SHA>` line (the rule that records a control-plane advisory reviewed head in its body without making it a merge verdict), which `ship-it`'s §CP enqueue reads.
...
Post it **as a comment, never a native review** (the verdict rule that upserts one verdict per gate and accepts it only when bound to the PR current head SHA rule 4). Do **not** emit the
...
`review-skill` verdict comment per PR, the verdict rule that upserts one verdict per gate and accepts it only when bound to the PR current head SHA rule 2):
...
Do **not** post a native `REQUEST_CHANGES` review — `review-skill` is comment-only (the verdict rule that upserts one verdict per gate and accepts it only when bound to the PR current head SHA
...
the applicable safety invariant), classify blocking vs non-blocking via the canonical §CP set (Step 0), resolve the
...
appends an AC, out-of-scope to `report`, the applicable safety invariant), then land the verdict — namespaced
...
that decides whether your marker binds `ship-it` or merely advises is the control-plane rule that requires human approval for changes to automation, CI, or merge safeguards
...
separate axes, the skill-review rule that gives skills their own behavioral gate §4).
...
(`ship-it` does that) nor strays into another's namespace. You realize the skill-review rule that gives skills their own behavioral gate, superseding
the routing rule that treats skills as behavioral artifacts rather than ordinary documentation's
