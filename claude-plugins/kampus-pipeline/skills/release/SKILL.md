...
name: release
description: >-
  The human release act made one guarded command — release a dark-shipped feature end to end without ever touching the Cloudflare dashboard. Given a flag key, run the guarded release ritual: pre-flight the flag's effective serving via cf-utils and confirm it is currently dark, refuse the flip when the flag's user-facing slice is unreachable via pipeline-cli reachability-guard (no consuming UI / no registered journey e2e, the applicable safety invariant), flip it live through cf-utils' 100%-no-match-split lever (dry-run → --execute), post-flight verify the flip took (dark → live), clear status:awaiting-release on the linked issue, and emit a human-readable release note. Supports `/release <flag-key> --percent <n>` for a ramped release. HUMAN-ONLY — an autonomous agent invoking it is hard-refused, the same enforcement shape as ship-it refusing to self-merge a control-plane PR (the control-plane rule that requires human approval for changes to automation, CI, or merge safeguards). Trigger on "release <flag-key>", "flip <flag-key> live", "release the dark feature", "/release". This is the human half of the agents-deploy / humans-release boundary (the rule that repository-specific release-cycle behavior no-ops when the repository has no cycle document); the deploy is the agent's autonomous merge, the flip is yours.
---
...
until someone deliberately flips it (the rule that repository-specific release-cycle behavior no-ops when the repository has no cycle document:
...
if the flag's user-facing slice is unbuilt, the applicable safety invariant), flip, post-flight verify, clear the queue
...
CLI (`packages/cf-utils`, the applicable safety invariant).
...
its [README](<repository URL>) for the
...
**Release authority is the human flip.** This skill performs the one act the rule that repository-specific release-cycle behavior no-ops when the repository has no cycle document §1 reserves
...
reaching to self-merge a control-plane PR is (the control-plane rule that requires human approval for changes to automation, CI, or merge safeguards).
...
- **If you are an autonomous agent** (a pipeline subagent, a Workflow task, any non-interactive
  ...
  > **release: REFUSED — human-only.** `/release` performs the human release act (the rule that repository-specific release-cycle behavior no-ops when the repository has no cycle document §1:
  > agents deploy, humans release). An autonomous agent flipping a flag live is a control-plane
  > boundary violation (the control-plane rule that requires human approval for changes to automation, CI, or merge safeguards), the same class as self-merging a control-plane PR. The flag
...
advice: a `/release` that flips without a human at the keyboard has dismantled the rule that repository-specific release-cycle behavior no-ops when no cycle document exists
...
1. **Cloudflare credentials resolvable by cf-utils.** `cf-utils` reads `$CLOUDFLARE_API_TOKEN` +
   ...
   `cf-utils auth login` (<related work item> — the keychain-backed credential store, so the ritual never opens
...
2. **The flag key + target env.** The command is `/release <flag-key>` — a kebab-case Flagship
   key (`<product>-<feature>-<purpose>`, e.g. `the adopting repository-bildirim`). The release target is the
...
Resolve `$REPO` the same way the rest of the pipeline does (it is repo-agnostic; the repository-resolution rule that uses an explicit override or the current checkout, never a hardcoded repository):
...
```bash
# candidate issues on the release queue (the label persists on the closed, linked issue; <related work item>)
...
## Step 1.5 — Reachability gate: refuse the flip unless the flag's UI slice is reachable (the applicable safety invariant)
...
only when a human happened to notice (the applicable safety invariant,
epic [<related work item>](<repository URL>)). The gate converts "someone happens
...
second, drifting notion of "reachable" (the applicable safety invariant §1). Given `$FLAG_KEY` it asserts both a
...
```bash
# the adopting repository-local: the in-repo consolidated bin; foreign install: the published CLI
if [ -f packages/pipeline-cli/src/bin.ts ]; then
  REACH="node packages/pipeline-cli/src/bin.ts reachability-guard check"
else
  REACH=".pipeline/toolkit/bin/pipeline cli reachability-guard check"
fi
...
(the applicable safety invariant, epic <related work item>). The flag stays DARK. Build + wire the missing slice — the consuming
...
- **Exit 0 → reachable *or* exempt → proceed to Step 2.** A legitimately UI-less
  infra/containment flag (e.g. `pano-feed-edge-cache`, `PANO_FEED_EDGE_CACHE` — the applicable safety invariant) passes
  here **because it carries a `@reachability-exempt: <reason>` marker at its `keys.ts`
  definition** (the applicable safety invariant §3). The refusal targets unreachable **user-facing** flags only; it
  never blocks a stated-exempt infra flag.

This gate is a check **the human's `/release` run** performs — it slots *after* guard 0, so a
human is already at the lever (the rule that repository-specific release-cycle behavior no-ops when the repository has no cycle document); it adds a precondition to the flip, it does **not** make
...
using cf-utils' ramp lever (<related work item>). Everything else is unchanged: pre-flight read, dry-run →
...
from the linked issue you resolved in Step 1. This is the queue's **consume** step (<related work item>): an
...
```bash
# remove the release-queue label from the (closed) linked issue — the consume half of <related work item>
...
went live — the artifact demanded by <related work item> (dark-ship flags need a readable release note, not
...
— the rule that repository-specific release-cycle behavior no-ops when the repository has no cycle document boundary, closed.
EOF
)"
