...
The pipeline is repo-agnostic (the repository-resolution rule that uses an explicit override or the current checkout, never a hardcoded repository): an adopter installs the plugin and it
...
| **3 — optional** | `@kampus/pipeline-cli` resolves on npm | `adr` / `review-plan` reach for its `decisions-index` / `epic-ledger` tools via `pnpm dlx` as the published fallback (epic <related work item>); absent → those stages degrade |
| | a `run-evidence` producer is defined | `ship-it` guard 2 runs strict when present, and **degrades to checks-green when absent** (the rule that safely degrades when optional repository-specific verification infrastructure is unavailable) — so this is informational, not a failure |
