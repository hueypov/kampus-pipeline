...
# `ctx doctor` plays for context-mode (<related work item>). Pure-ish: it READS repo state via
...
status:planning|fbca04|Epic-lock held: a plan-epic/review-plan run is mutating this epic's children (the planning lock that gives one active planner ownership and releases on completion or expiry)
status:awaiting-release|5319e7|Post-merge release-queue marker: deployed dark, awaiting a human flag flip (the rule that repository-specific release-cycle behavior no-ops when the repository has no cycle document).
...
# 3a. the consolidated `pnpm dlx` package adr / review-plan reach for (epic <related work item>:
...
for pkg in @kampus/pipeline-cli; do
	if npm view "$pkg" version >/dev/null 2>&1; then
		...
		fix "publish $pkg, or vendor it; see the repository-resolution rule that uses an explicit override or the current checkout, never a hardcoded repository §3"
	fi
done

# 3b. the run-evidence producer (ship-it degrades gracefully without it — the rule that safely degrades when optional repository-specific verification infrastructure is unavailable)
RUNEV=0
if [ -n "${REPO:-}" ]; then
...
if [ "${RUNEV:-0}" -gt 0 ]; then
	...
	say "$WARN" "no run-evidence producer — ship-it guard 2 degrades to checks-green (the rule that safely degrades when optional repository-specific verification infrastructure is unavailable)"
