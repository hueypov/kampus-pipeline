...
committed index (the applicable safety invariant); the `NNNN-slug` filenames are the map, and each file's frontmatter
...
### 5. Consolidate, dedup against open issues, then file one issue per finding (the applicable safety invariant)
