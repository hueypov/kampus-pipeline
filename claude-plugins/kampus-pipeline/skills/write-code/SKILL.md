...
([`../gh-issue-intake-formats.md`](../gh-issue-intake-formats.md), the repository-resolution rule that uses an explicit override or the current checkout, never a hardcoded repository §1), defaulting
to `<owner>/<repository>` with no config.
...
gh api "repos/$REPO/pulls?state=open&per_page=100" \
  ...
  # stays inline. Author-gate the FAIL markers to write+ collaborators (the authorization rule that accepts privileged actions only from repository members, supersedes 0051)
...
- **Author-gated verdicts (the authorization rule that accepts privileged actions only from repository members).**
...
read it for what a milestone *is* and its REST surface — the applicable safety invariant).
...
## Step 3 — Claim by writing your session-id claim marker (the agent-distinguishable claim, the claim-ownership rule that gives work to the earliest authorized session-stamped claim)
...
the `CLAIM_RE` matcher, the write+ ACL trust root (the authorization rule that accepts privileged actions only from repository members), and the earliest-authorized-claim
...
**already yours** — the orchestrator posted it on your behalf (the claim-ownership rule that gives work to the earliest authorized session-stamped claim §3, Delegated
...
guard **reuses the §1 claim-marker read** (the claim-ownership rule that gives work to the earliest authorized session-stamped claim §1/§2); it introduces **no second claim
mechanism**.
...
- **Direct path** — `write-code` invoked directly: `MY_CLAIM` is the coder's own
  `CLAUDE_CODE_SESSION_ID` (the same token the Step-3 claim writes — the claim-ownership rule that gives work to the earliest authorized session-stamped claim §3 direct path).
- **Orchestrated path** — dispatched by the orchestrator, which **pre-claims before spawn**
  (`.claude/workflows/drive-issue.js`, the claim-ownership rule that gives work to the earliest authorized session-stamped claim §3) and **threads its winning claim token into
...
# fall back to the bare assignee login (that login degeneracy is the exact defect the claim-ownership rule that gives work to the earliest authorized session-stamped claim removes).
MY_CLAIM="${THREADED_CLAIM_TOKEN:-$CLAUDE_CODE_SESSION_ID}"
: "${MY_CLAIM:?mis-attribution guard: no claim token — CLAUDE_CODE_SESSION_ID absent and none threaded; refusing to claim or mutate (the claim-ownership rule that gives work to the earliest authorized session-stamped claim Consequences — never a login fallback)}"
```
...
as the owner — the assignee is a coarse availability gate only (the claim-ownership rule that gives work to the earliest authorized session-stamped claim §1), and a login-keyed
...
the same detect-and-tiebreak shape (the claim-ownership rule that gives work to the earliest authorized session-stamped claim §2), here read-only and on the *acting* side.
...
### Step 4 preflight — assert you're in a worktree, fail closed if not (the applicable safety invariant)
...
the unsafe state** (the applicable safety invariant).
...
When it **does** fire, ship dark: keep the old path as the safe default, validate the new path
separately, and enable it only through an explicit release step.
...
- **Newly-declared in this diff** — no suitable flag exists yet, so you mint one: declare a
  default-off flag, gate the new path so an unset key or evaluation failure takes the old path,
...
comment-discipline gate** (Step 3d of `review-code`, the applicable safety invariant).
...
- **Error copy** — the English source string a user reads when an operation fails, in
  ...
  use an override only when the user-facing wording genuinely differs. These rules govern the
...
in-repo, copy-pasteable form of the out-of-repo orchestrator's authorization — the applicable safety invariant keeps
...
- **Do not self-assign a reviewer, re-trigger the gate, or merge.** The gate is **stateless and
  ...
  for a control-plane `.claude`/`.github` PR a **human** does (the control-plane rule that requires human approval for changes to automation, CI, or merge safeguards).
...
changes-requested`, and `review-skill: FAIL @ <sha> — changes-requested`, SHA-bound per the verdict rule that upserts one verdict per gate and accepts it only when bound to the PR current head SHA). You take a PR that came back failed, apply exactly the enumerated
...
# The write+ author-set (the authorization rule that accepts privileged actions only from repository members) — `verdict read` computes it internally for the marker resolution,
...
# only a review prefix-matching the current head participates (the verdict rule that upserts one verdict per gate and accepts it only when bound to the PR current head SHA).
#
...
#### A review-appended AC is an ordinary `[FAIL]` row — no special parser (the applicable safety invariant)
...
- a **`write+` repo collaborator** — the same GitHub-ACL floor the authorization rule that accepts privileged actions only from repository members
...
**Head-binding (the verdict rule that upserts one verdict per gate and accepts it only when bound to the PR current head SHA staleness, applied to inline comments).** A comment whose anchor
...
This keeps the audit trail of the time-varying AC contract complete (the applicable safety invariant Consequences):
...
- **Never merge.** Repair mode pushes and hands back to the gate; the merge is `ship-it`'s
  ...
  *refuses* to auto-merge blocking-set PRs and `review-doc` is advisory-only on them (the control-plane rule that requires human approval for changes to automation, CI, or merge safeguards). **This very edit is such a PR:
...
- **All three namespaces.** Handle `review-code: FAIL @ <sha>` (§5), `review-doc: FAIL @ <sha>`
  ...
  not just `review-code`. A skill PR's FAIL lands in the `review-skill` namespace (the skill-review rule that gives skills their own behavioral gate).
- **Author-gated verdicts.** A marker counts only from a `write+` repo collaborator —
  ...
  mask a real verdict (the authorization rule that accepts privileged actions only from repository members).
- **Inline comments are additive, never the gate (Step R2).** Repair also folds in
  ...
  a **`write+`** author (the authorization rule that accepts privileged actions only from repository members floor) **or** the explicitly-named
  ...
  the verdict rule that upserts one verdict per gate and accepts it only when bound to the PR current head SHA staleness test applied to inline anchors. Addressed threads get a REST reply
  (resolve is GraphQL-only → out of reach here).
- **Review-appended ACs drain like any `[FAIL]` row — no new parser (the applicable safety invariant,
...
no committed index (the applicable safety invariant) — the ADR PR is purely additive: it lands the new file (plus
...
#### Bounded collapse — when the fix is trivial, open one PR instead of residue (the applicable safety invariant)
...
3. **Route durable knowledge to `.decisions/` / `.patterns/`.** If the investigation
   ...
   goes to `.patterns/` (add or extend a doc only when it meets the repository's stated criteria).
...
- **Repair** (PR number): resolve the PR's latest verdict per namespace (Step R1) and, if
  ...
  as an ordinary `[FAIL]` row (the applicable safety invariant, §2)** — **plus the in-scope line-anchored inline
