...
#
# ZERO-SCOPE = FAIL (the applicable safety invariant): this gate fails closed when it scans nothing. The cycle doc
# MUST be present in the adopting repository (the present path must actually be exercised), and the static
...
for entry in "${CYCLE_SKILLS[@]}"; do
	...
	if ! grep -qiE "$action_re" "$md"; then
		fail "$skill: no present-path action found (expected /$action_re/i) — the cycle's present branch must DO something (the applicable safety invariant), not just no-op"
	else
...
if [ "$(probe_cycle_doc "$tmp_root")" != "present" ]; then
	...
	ok "hermetic probe resolves 'present' on a the adopting repository-shaped install (with $CYCLE_DOC_PATH)"
fi
...
# Zero-scope guard (the applicable safety invariant): the present path MUST actually be exercised.
...
#   2. the adopting repository's real cycle doc is missing — then the present path is never exercised here,
#      so there is nothing to prove and this gate would be a silent no-op (the exact rot
#      the applicable safety invariant forbids). In the adopting repository the doc MUST be present; absent ⇒ FAIL, not skip.
if [ "$scanned_skills" -eq 0 ]; then
	fail "zero scope: no cycle-aware skills were scanned (skills dir moved?) — a zero-scope run is a FAIL, never a silent pass (the applicable safety invariant)"
fi
if [ ! -f "$repo_root/$CYCLE_DOC_PATH" ]; then
	fail "zero scope: the adopting repository has no $repo_root/$CYCLE_DOC_PATH — the present path cannot be exercised; this gate fails closed rather than no-op (the applicable safety invariant)"
else
	ok "the adopting repository's real cycle doc is present at $CYCLE_DOC_PATH (present path is live)"
fi

# Emitted scope (the applicable safety invariant): every run states what it looked at.
echo "scanned scope: ${scanned_skills} cycle-aware skill(s) [${scanned_paths[*]}]; the adopting repository cycle doc: $([ -f "$repo_root/$CYCLE_DOC_PATH" ] && echo present || echo MISSING)"

if [ "$errors" -gt 0 ]; then
	echo "validate-cycle-presence: FAILED — $errors error(s); the adopting repository's present cycle-doc path is not real (a present-branch action is missing, or the gate scanned zero scope — the applicable safety invariant)"
	exit 1
fi

echo "validate-cycle-presence: OK — $checks checks; the adopting repository's present cycle-doc path is real (probe⇒present, all four present-path actions wired + exercised)"
