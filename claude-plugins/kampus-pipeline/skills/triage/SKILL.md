...
resolution** ([`../gh-issue-intake-formats.md`](../gh-issue-intake-formats.md), the repository-resolution rule that uses an explicit override or the current checkout, never a hardcoded repository §1),
defaulting to `<owner>/<repository>` with no config:
...
### Intake dedup — is this issue itself a duplicate? (the applicable safety invariant)
...
by hand. It resolves the target repo itself (the repository-resolution rule that uses an explicit override or the current checkout, never a hardcoded repository §1), so it needs no `$REPO`.
...
- **bug vs investigation** — if the wrong behavior is understood and the fix is
  ...
  *might* turn out to be a trivial fix is **still `type:investigation`** — do **not**
  ...
  [`../gh-issue-intake-formats.md`](../gh-issue-intake-formats.md) §8, the applicable safety invariant);
  the collapse is owned by `write-code`, not a triage re-type (the applicable safety invariant rejected the
...
2. **Re-query before you create.** Report agents run concurrently with your sweep
   ...
   **intake-dedup tool** (the applicable safety invariant; the one Step 1 and the `report` skill use) on the
   ...
   itself (the repository-resolution rule that uses an explicit override or the current checkout, never a hardcoded repository §1), so it needs no `$REPO`.
...
- **Acceptance-shaped clarity.** Make "done" legible. For a typed-and-pickable issue
  ...
  §2, the applicable safety invariant). Write
...
**The filing-provenance signal is the report footer, not authorship (the applicable safety invariant).** The
...
roadmap that projects each arc onto a GitHub milestone (the applicable safety invariant;
...
- **Existing open milestones only — triage NEVER creates a milestone.** Read the repo's open
  ...
  or curating the milestone set is a roadmap (human) act, deliberately not autonomous (the applicable safety invariant
...
- **Preserve freeze-by-absence.** A deliberately-unmilestoned surface — a frozen new-product
  ...
  issue "complete" (contract §Milestone, the applicable safety invariant §4).
...
three applies is out of triage's scope (the applicable safety invariant
