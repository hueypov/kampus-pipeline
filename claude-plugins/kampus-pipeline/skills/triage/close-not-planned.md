...
these mechanics (the fan-out-economics split, [<related work item>](<repository URL>)).
...
2. Post a **reason comment** — *why* it's unsalvageable, specifically (e.g. "Duplicate of
   <related work item>, which already tracks this hang" or "The function this references was removed in <related work item>;
...
# reads back cleanly as another run's body, preserving the WRONG original (<related work item>). Keyed on the
# session id so composing dup-comment.md in a later Bash call still resolves this same directory:
RUN_SCRATCH="${TMPDIR:-/tmp}/kampus-run/${CLAUDE_CODE_SESSION_ID:?§SP: session id unset — refusing a shared path (<related work item>)}/triage-close-<N>"
mkdir -p "$RUN_SCRATCH" || { echo "§SP: no per-run scratch dir — refusing a shared path (<related work item>)." >&2; exit 1; }
