...
the adopting repository already has homes for the things big docblocks try to be (CLAUDE.md, "Doc surfaces"):
...
- A multi-paragraph docblock re-explaining a *why* that an ADR/pattern already owns → `// See the applicable safety invariant` / `// Keyset order follows the connection ordering rule.` One line, pointing at the single source of truth.
...
- **Never invent an ADR number.** Derive the next monotonic number from the `.decisions/NNNN-*.md` filenames (`ls .decisions/`); there is no committed index (the applicable safety invariant). The `/adr` skill does this.

## Running it at scale

For a whole-codebase pass, not a single file:

1. **Isolate.** Work in a per-run git worktree so nothing touches the user's tree *and* two concurrent decomment passes never collide on the branch or the dir. Derive the branch prefix from this checkout's git identity (never a hardcoded literal, else every agent's branch lands under one person's handle), choose the consumer's configured base ref when one is documented, and otherwise use the checkout's current commit. Then uniquify branch + dir with a per-invocation suffix:

   ```bash
   PREFIX="$(git config user.name | tr '[:upper:] ' '[:lower:]-')"
   : "${PREFIX:?set git user.name to derive a branch prefix}"
   SUF="$(date +%s)-$$"
   BASE_REF="${PIPELINE_BASE_REF:-$(git rev-parse HEAD)}"
   git worktree add -b "$PREFIX/decomment-$SUF" "../the-adopting-repository-decomment-$SUF" "$BASE_REF"
   ```

   A hardcoded branch name or worktree directory makes the second run fail with `already checked
   out` or clobber a shared directory. Everything — this cleanup, plus any new ADR/pattern docs —
   lands on that one branch as one reviewable PR.
2. **Partition + fan out (phase 1, parallel).** Split the target files into disjoint groups (no two agents share a file) and spawn one agent per group. Subagents don't inherit skills — **paste the CUT/COLLAPSE/MIGRATE/KEEP rules into each agent's prompt, or have it Read this file.** Each phase-1 agent: CUTs, COLLAPSEs against *existing* ADRs/patterns, KEEPs — and for any **orphaned why** (migrate candidate with no home), it does **not** delete; it leaves the comment and appends an entry to an `ORPHANS.md` **at the root of this run's worktree** (`../the adopting repository-decomment-$SUF/ORPHANS.md`) — run-local because the worktree dir is per-run, so two concurrent passes never append to the same file: file path + 1-line summary of the homeless knowledge. This keeps phase 1 safe to parallelize.
...
4. **Verify.** Start with `git diff --check`; comments-only changes should be a no-op, but
   JSDoc removal near `@ts-expect-error` can still shift a language tool's interpretation. Read
   the adopting repository's contributor guidance and package/build metadata for its declared
   typecheck, test, and formatting commands, then run the relevant declared checks. Do not assume
   a Node package manager, a formatter, or a command name. If the repository declares no
   applicable command, report that explicitly with the `git diff --check` result rather than
   guessing. Report a `git diff --stat` for review — never auto-merge.
