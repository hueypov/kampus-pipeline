...
1. **Isolate.** Work in a per-run git worktree so nothing touches the user's tree *and* two concurrent decomment passes never collide on the branch or the dir — derive your branch prefix from this checkout's git identity (never a hardcoded literal, else every agent's branch lands under one person's handle) and uniquify branch + dir with a per-invocation suffix: `PREFIX="$(git config user.name | tr '[:upper:] ' '[:lower:]-')"; : "${PREFIX:?set git user.name to derive a branch prefix}"; SUF="$(date +%s)-$$"; git worktree add -b "$PREFIX/decomment-$SUF" "../the adopting repository-decomment-$SUF" main`. (A hardcoded `$PREFIX/decomment` / `../the adopting repository-decomment` makes the second run fail with `already checked out` or clobber the shared dir.) Everything — this cleanup, plus any new ADR/pattern docs — lands on that one branch as one reviewable PR.
