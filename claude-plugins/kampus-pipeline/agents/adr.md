...
- **Claim the number under the in-flight reservation lock, fail closed (the ADR-number rule that reserves numbers across merged decisions and open decision PRs, failing rather than guessing).** Compute the
...
- **One decision per file, additive-only PR.** One decision per `.decisions/NNNN-slug.md`; a sprawling
  ...
  is **no committed index** to regenerate or stage (the applicable safety invariant); discovery is the CLAUDE.md contract
...
- **Always resolve the vocabulary-impact outcome (required, not silently skippable).** An ADR is a
  ...
  is off the fail-closed `review-code` gate by construction (the vocabulary rule that captures newly coined or redefined terms at their source and through a backstop sweep prong (c)).
- **Cross-link slugs resolve off disk — never guess from the title.** A target ADR's slug is not
  ...
  "links resolve" FAIL (<related work item>) fixed at authoring time.
