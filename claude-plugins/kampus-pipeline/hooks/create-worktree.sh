...
# (the non-deterministic fall-back-to-primary of <related work item>/<related work item>). See the applicable safety invariant.
#
# Contract (Claude Code WorktreeCreate mechanism) — grounded in a CAPTURED REAL payload
# (the applicable safety invariant), not the incomplete docs that produced <related work item>:
...
#      NOT carry `worktree_path` or `base_ref` — <related work item> shipped a hook built to those inferred
...
# (the applicable safety invariant) — we REUSE it, never reimplement it, so 0109's provision-not-share
...
# /usr/bin — <related work item>/the applicable safety invariant); if that stripping applies here too, `bootstrap-deps` finds no
...
# PATH (kept last) — OS/standard dirs only, never a per-machine volta/fnm shim (the applicable safety invariant).
...
# Freshen the remote tip BEFORE branching — the <related work item> fix. The primary checkout's `origin/main`
...
# a clean/green PR silently reverts the sibling's merged work (<related work item>). The fetch NEVER moves the
# primary's local `main` HEAD (it advances only remote-tracking refs), so the <related work item>/<related work item>
...
if ! git fetch --quiet origin main >&2; then
	echo "create-worktree: git fetch origin main failed — refusing to branch from a possibly-stale base (fail-closed, <related work item>)." >&2
	exit 1
fi
...
# HEAD is throwaway. A detached checkout at FETCH_HEAD still fires post-checkout (the applicable safety invariant).
