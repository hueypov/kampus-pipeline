...
| `review-skill` | The behavioral-artifact sibling: verify a `skills/**` PR against its criteria plus a rigor checklist (behavioral correctness, trigger quality, cross-skill shadowing, gate-invariant preservation); config-pinned to the base (the skill-review rule that gives skills their own behavioral gate). |
...
| `wayfinder` | The ideation-layer front door, upstream of the pipeline: chart a fuzzy destination into a living `wayfinder:map` issue, then work its open frontier of investigation/decision tickets — recording answers, graduating cleared fog, surfacing founder-decision-forks — until a concrete plan is ready for `triage` / `plan-epic` (epic <related work item>). |
...
`<owner>/<repository>` is the marketplace repository; `kampus-pipeline@kampus` is the plugin
...
repo (the repo whose issues/PRs it operates on) in this order (the repository-resolution rule that uses an explicit override or the current checkout, never a hardcoded repository §1):
...
The plugin carries **no per-plugin `version`** (neither in the marketplace plugin entry nor in [`.claude-plugin/plugin.json`](.claude-plugin/plugin.json)): Claude Code content-addresses the install by git commit SHA, so every commit is a new version and skill additions reach installed users on the normal update path. A fixed version pin can freeze the cached source and serve stale skills. This omission is **deliberate, not a defect — do not add a `version`**.

The whole plugin surface was audited against the official Claude Code plugin spec ([plugins reference](https://docs.claude.com/en/docs/claude-code/plugins-reference), [marketplaces](https://docs.claude.com/en/docs/claude-code/plugin-marketplaces)). Its deliberate deviations — no `version`, the root-level `hooks.json`, absent `commands/`, and the `.claude/skills` discovery symlink — are intentional compatibility choices, not open defects.
...
the private local pipeline CLI. Shared pipeline mechanics live in one tested CLI, so an
in-repository run and an installed copy use the same implementation:
a `SessionStart` hook verifies `.pipeline/toolkit`, and
both the adopting repository and a foreign install invoke the **same** `pipeline-cli epic-ledger` — so a
foreign repository install **runs** the gate (validates the ledger, flips `status:planned →
...
view`, and stops rather than guessing if it cannot resolve a repository. This lets an
installed copy target any repository without relying on repository-local state.

The **`adr`** skill is portable on the same shape. There is no committed
`.decisions/index.md` and **no `SessionStart` ADR-map hook**: discovery is the CLAUDE.md
contract alone — list `.decisions/` and read each file's frontmatter. The **`pipeline-cli
decisions-index compact`** subcommand emits the one-line-per-ADR map (`id · title · status`)
from frontmatter to stdout **on demand**, never by automatic injection. The local pipeline CLI
(SessionStart-installed by the deps-install hook)
is invoked the same way in the adopting repository and a foreign install; the CLI operates on the local
`.decisions/` tree, so it needs no repo resolution. On a PR, `decisions-index validate` rejects a duplicate
or mismatched `id`. The ADR PR is purely additive — it adds only the new file (plus
...
To re-prove a skill's gate outside its source repository, exercise it in a throwaway foreign
git repository — **not** the source repository itself:

1. `mkdir` a scratch dir outside the adopting repository, `git init`, and create the inputs the skill
   ...
   local `.pipeline/toolkit/bin/pipeline` command is what runs.
2. Run the exact subcommand the skill uses (`adr`:
   `.pipeline/toolkit/bin/pipeline cli decisions-index generate`) from the scratch repo root.
...
4. Run the gate (`.pipeline/toolkit/bin/pipeline cli decisions-index check`): exit `0` on the fresh
...
- **External doc references** in the skills (to the adopting repository's `.decisions/`, `.patterns/`,
  `CLAUDE.md`) are rewritten to **stable repository GitHub permalinks** — they are
  repository-specific rationale, not bundled into your repo (the repository-resolution rule that uses an explicit override or the current checkout, never a hardcoded repository §4). Intra-suite links
  stay relative and travel inside `skills/`.
- **Don't install the plugin into the adopting repository itself** (the repository-resolution rule that uses an explicit override or the current checkout, never a hardcoded repository §5). the adopting repository already carries
...
## For the adopting repository maintainers: layout and discovery

The sections below are repository-internal — they explain how the one source of truth serves
...
subtree** — the packaging fix from the applicable safety invariant: a `"./"` source copies the **whole monorepo**
...
offers no manifest field to redirect it. the adopting repository's **local** discovery reads
...
- **External doc-references** (`.decisions/*`, `.patterns/*`, `CLAUDE.md`) are
  repository-specific rationale and are written as **stable GitHub permalinks**, not relative
  paths (the repository-resolution rule that uses an explicit override or the current checkout, never a hardcoded repository §4) — so they resolve from an adopter's install, where those trees don't
  exist.

### In-repo discovery doubling — accept the doubles (the repository-resolution rule that uses an explicit override or the current checkout, never a hardcoded repository §5)

When the plugin is installed at **user scope** and a maintainer works **inside the adopting repository**,
every skill surfaces twice in the picker: bare `report` (project-scope `.claude/skills/`)
and `the adopting repository:report` (plugin scope). Claude Code does not dedupe skill names across scopes
and has no per-project plugin disable (upstream anthropics/claude-code#53923).

**Disposition: accept the doubles for v1.** The recommendation is that **the adopting repository
maintainers rely on the local `.claude/skills/` discovery and do _not_ install the plugin
into the adopting repository itself** — they already have the canonical suite locally; the plugin exists
for *other* repos. This is a documentation cost, not a functional one. Revisit if/when
upstream ships a per-project plugin toggle. See the repository-resolution rule that uses an explicit override or the current checkout, never a hardcoded repository §5 for the full rationale.
