...
| [Explanation](./docs/explanation.md) | understanding | The coordination model — stigmergic claim map, pull-not-push, two-keyspace design (→ the claim-lifecycle rule: presence leases and resource claims use separate keyspaces; claims live only while their holder is present and release on completion or holder expiry). |
...
- **`src/tracker/`** — the rendezvous registry where peers announce and discover each other,
  ...
  by resource — the claim-lifecycle rule: presence leases and resource claims use separate keyspaces; claims live only while their holder is present and release on completion or holder expiry).
...
- **`src/crew/`** — the only crew-coupled module and the composition root: the Role enum (the
  ...
  (enforcing the role-uniqueness lease the generic peer does not, the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes), and the runnable
...
[Explanation](./docs/explanation.md), which points to the claim-lifecycle rule: presence leases and resource claims use separate keyspaces; claims live only while their holder is present and release on completion or holder expiry.
...
rendezvous (the canonical-rendezvous rule: derive the tracker location from the repository shared Git directory so every worktree joins one tracker; fail rather than fall back when Git cannot identify it: derive the tracker location from the repository shared Git directory so every worktree joins one tracker; fail rather than fall back when Git cannot identify it — keyed on the repo's shared git dir, so the repo root, a nested subdir, and any
