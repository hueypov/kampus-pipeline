 ...
 * used to fake (locked in the role-addressing rule: send to a role inbox, and treat delivery as queued rather than read), never seen-by-model. `InboxAck` is the peer data plane's
 * own ack (the crew message catalog carries no ack kind — nothing sends one, the inbox-routing rule: resolve role-addressed messages to the correct inbox); it and
 ...
 * not a crew message catalog kind — the catalog has no ack kind because nothing sends one (the inbox-routing rule: resolve role-addressed messages to the correct inbox).
 */
