 ...
 * holds the presence for its scope's lifetime — connection-is-lease (the role-addressing rule: send to a role inbox, and treat delivery as queued rather than read); scope close
...
export const make = Effect.fn("Peer.make")(function* (config: PeerConfig) {
	...
	// Announce holds the role lease for this scope's lifetime — connection-is-lease (the role-addressing rule: send to a role inbox, and treat delivery as queued rather than read).
