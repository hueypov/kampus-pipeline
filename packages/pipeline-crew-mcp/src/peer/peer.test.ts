 ...
 *
 * The tracker is a per-project registry (the peer-lifecycle rule: scope ownership controls peer startup, heartbeat, and shutdown); here it is an in-memory fake whose
 * `announce` is scoped, so lease-lifetime is observable: present while the peer's scope is
 * open, gone once it closes (connection-is-lease, the role-addressing rule: send to a role inbox, and treat delivery as queued rather than read).
 */
