/**
 * standup/single-role — dynamic single-member membership ops (the membership rule: add, remove, or respawn one role without restarting the whole crew). These tests pin the two
 ...
 *     no boot turn, the on-demand cartographer rule); it fails closed when no crew window is up.
...
describe("standup/single-role — spawn-role (the membership rule: add, remove, or respawn one role without restarting the whole crew)", () => {
	it.effect(
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				// a self-driving engine carries its boot turn (the boot-turn rule: a spawned member receives its role-specific initial prompt) — one positional prompt
				assert.strictEqual(plan?.bind.bootPromptArg.length, 1);
			}),
	);

	it.effect(
		"spawns the on-demand HITL cartographer (not autoboot-gated) and it boots idle (the on-demand cartographer rule)",
		() =>
			Effect.gen(function* () {
...
describe("standup/single-role — retire-role (the membership rule: add, remove, or respawn one role without restarting the whole crew)", () => {
