 ...
 * values is out of scope here (the validated-config rule: parse channel settings, role tiers, and CLI version once before launch, decision on the launcher design decision).
 *
 ...
 * The lone exception is `cliVersion`: it is OPTIONAL (the optional CLI-version rule) — an omitted pin decodes
 ...
 * `LaunchConfig` extracts the launch dimensions from the one-role-map seam shape (the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes): the
 * engine count is folded into `roles["engineering-manager"].count`, and each role's optional `tier`
 * — the session model, the optional role-tier setting — is folded into `roleTiers`. Remaining seam keys
 ...
 * `plugin:<plugin>:<server>` shape (the validated-config rule: parse channel settings, role tiers, and CLI version once before launch) — no `@` — is rejected by the real runtime; the
 ...
 * different surface from this `--channels` allowlist grammar.) See the fail-closed config rule: reject malformed channel references and unsafe combinations before starting.
 */
 ...
 * (the pre-launch CLI-version check consumes it). A `major.minor.patch` core with an optional pre-release suffix — the
 * shape `claude --version` reports — so a non-version placeholder or a partial pin is rejected.
 * The pin itself is optional (the optional CLI-version rule — see `LaunchConfig.cliVersion`); this regex constrains
...
/**
 * A role's model tier — the operator's per-role cost/capability control (the optional role-tier setting), decoded so the
 ...
 * alias is one edit here, never a scatter of string literals across the launcher (the optional role-tier setting).
 */
 ...
 * name a channel the runtime silently drops — fail closed here instead (the fail-closed config rule: reject malformed channel references and unsafe combinations before starting).
 */
 ...
 *
 * `cliVersion` is an EXACT-optional key (`Schema.optionalKey`, the optional CLI-version rule): absent ⇒ the field
...
/** A role entry's optional `tier` — the launch dimension the optional role-tier setting promotes from ignored excess key. */
const RoleTierEntry = Schema.Struct({tier: Schema.optionalKey(Tier)});

/**
 * The one-role-map seam shape on disk (the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes): a `roles` map keyed by the crew role slugs. The
 * launch reader takes the engine pool size at `roles["engineering-manager"].count` AND — as of
 * the optional role-tier setting — each role's optional `tier` (its session's `--model`), no longer an ignored excess key.
 ...
 * `LaunchConfig`'s exact-optional shape (the optional CLI-version rule): omit ⇒ unpinned.
 */
...
/** The default operator-owned config path when `$CREW_CONFIG` is unset (the repository-resolution rule: honor an explicit repository override, otherwise derive the working Git repository / PERSONALIZATION.md). */
 ...
 * The engine count is folded out of the role map — `roles["engineering-manager"].count` (the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes)
 ...
 * omitted `cliVersion` stays omitted through the fold (exact-optional, the optional CLI-version rule) — the
...
> =>
	Effect.gen(function* () {
		...
		// substitutes it in place of the real disk. A read fault is
