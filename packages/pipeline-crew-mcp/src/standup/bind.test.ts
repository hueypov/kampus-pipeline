 ...
 * which the launcher writes into `~/.claude.json` local scope, the channel-probe rule: distinguish a connected server, a brief boot delay, and a persistent failure) plus the channel-registration
 ...
 * that the argv NO LONGER carries `--mcp-config` (the resolver never read it, the channel-probe rule: distinguish a connected server, a brief boot delay, and a persistent failure), and the two
...
const PROJECT_ROOT = "/work/the adopting repository";
const SERVER_NAME = "pipeline-crew";

// The bind constructor reaches the platform through the `FileSystem`/`Path` seam
// and is discharged in-test by the same NodeServices.layer the bin
...
// from so `--agent <role>` resolves the persona instead of general-purpose (the role-persona rule: launch each pane with the crew plugin and its crew-role agent definition).
const EXPECTED_PLUGIN_DIR = "/work/the adopting repository/claude-plugins/pipeline-crew";
...
// never the bare unlinked package bin name (the resolvable-server rule: bind each session to an absolute existing channel-server entry point).
...
describe("standup/bind — per-session bind constructor", () => {
	it.effect(
		"allowlist mode: persisted-scope serverConfig + --channels naming the crew server, NO inline --mcp-config (AC 1,2,3; the channel-probe rule: distinguish a connected server, a brief boot delay, and a persistent failure)",
		() =>
			Effect.gen(function* () {
				...
				// carries the visible name (the display-identity rule: name panes by role and engine instance so operators can distinguish them), closes with the positional boot-turn prompt (the boot-turn rule: a spawned member receives its role-specific initial prompt), and
				// carries NO `--mcp-config` — the crew server now registers via the persisted local scope (the channel-probe rule: distinguish a connected server, a brief boot delay, and a persistent failure).
	...
	it.effect(
		"boots each pane AS its role persona: --plugin-dir <crew plugin> + --agent crew-<role>, collision-free (the role-persona rule: launch each pane with the crew plugin and its crew-role agent definition)",
		() =>
			Effect.gen(function* () {
				const channels: ChannelConfig = {
					mode: "development",
				...
				// the bare role), so a personal `~/.claude/agents/<role>.md` def can't shadow it (the role-persona rule: launch each pane with the crew plugin and its crew-role agent definition
	...
	it.effect(
		"bakes the launcher-assigned per-instance identity into the session server config (seam 3, the registration rule: every launched session registers its channel server with its stable role and instance identity)",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"the bound server command is a RESOLVABLE invocation: node + an existing absolute bin.ts (the resolvable-server rule: bind each session to an absolute existing channel-server entry point)",
		() =>
			Effect.gen(function* () {
				const channels: ChannelConfig = {
					mode: "development",
	...
	it.effect(
		"fails closed when the crew server bin.ts does not resolve on disk (the fail-closed server-path check, the resolvable-server rule: bind each session to an absolute existing channel-server entry point)",
		() =>
			Effect.gen(function* () {
				const channels: ChannelConfig = {
					mode: "development",
	...
	it.effect("emits --model <tier> at the front of the argv when the role has a tier (the optional role-tier setting)", () =>
		Effect.gen(function* () {
			const channels: ChannelConfig = {
				mode: "development",
				servers: ["server:pipeline-crew"],
			...
			// --model leads the argv; then the persona flags (the role-persona rule: launch each pane with the crew plugin and its crew-role agent definition), the channel fragment, the name
			// (the display-identity rule: name panes by role and engine instance so operators can distinguish them), and the tail boot-turn prompt (the boot-turn rule: a spawned member receives its role-specific initial prompt). No --mcp-config.
			assert.deepStrictEqual(
				[...bind.argv],
				[
					MODEL_FLAG,
	...
	it.effect(
		"cartographer's tier:fable yields --model fable (the planning-tier bridge, the optional role-tier setting)",
		() =>
			Effect.gen(function* () {
				const channels: ChannelConfig = {
					mode: "development",
	...
	it.effect(
		"emits NO --model when the role has no tier — preserves the CLI-default boot (the optional role-tier setting)",
		() =>
			Effect.gen(function* () {
				const channels: ChannelConfig = {
					mode: "development",
				...
				// No tier ⇒ no --model; argv is the persona flags (the role-persona rule: launch each pane with the crew plugin and its crew-role agent definition) + the channel fragment + the name
				// (the display-identity rule: name panes by role and engine instance so operators can distinguish them), closed by the tail boot-turn prompt (the boot-turn rule: a spawned member receives its role-specific initial prompt).
				assert.deepStrictEqual(
					[...bind.argv],
					[
						...bind.pluginDirArg,
	...
	it.effect(
		"emits --name <role> for a bridge (no instance) — the visible pane identity (the display-identity rule: name panes by role and engine instance so operators can distinguish them)",
		() =>
			Effect.gen(function* () {
				const channels: ChannelConfig = {
					mode: "development",
				...
				// prompt (the boot-turn rule: a spawned member receives its role-specific initial prompt) — so it's the last-but-one pair, with BOOT_PROMPT at the very tail.
	...
	it.effect(
		"engine sessions get --name <role>-<instance> so the N engine panes are distinctly named (AC2, the display-identity rule: name panes by role and engine instance so operators can distinguish them)",
		() =>
			Effect.gen(function* () {
				const channels: ChannelConfig = {
					mode: "development",
	...
	it.effect(
		"hands the session its boot turn: a single positional prompt at the argv tail, no -p/--print (the boot-turn rule: a spawned member receives its role-specific initial prompt)",
		() =>
			Effect.gen(function* () {
				const channels: ChannelConfig = {
					mode: "development",
				...
				// cold-start fires from boot instead of idling (the boot-turn rule: a spawned member receives its role-specific initial prompt).
	...
	it.effect(
		"every SELF-DRIVING role (bridge + engine) gets the same role-agnostic boot turn (the boot-turn rule: a spawned member receives its role-specific initial prompt)",
		() =>
			Effect.gen(function* () {
				const channels: ChannelConfig = {
					mode: "development",
	...
	it.effect(
		"a human-in-the-loop role (the cartographer) gets NO boot turn — it boots idle (the on-demand cartographer rule)",
		() =>
			Effect.gen(function* () {
				const channels: ChannelConfig = {
					mode: "development",
