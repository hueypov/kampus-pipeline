/**
 * standup/single-role — dynamic single-member membership ops (the membership rule: add, remove, or respawn one role without restarting the whole crew): add, remove, or respawn
 ...
 *     `launchSessionInTmux`, extracted not forked, the membership rule: add, remove, or respawn one role without restarting the whole crew AC3), but SPLITS the pane into the RUNNING
 ...
 *     precisely because this is the explicit human spawn, and bind.ts already boots it idle (the on-demand cartographer rule).
...
export interface SpawnRoleInput {
	...
	/** The role to spawn one member of — any roster role, including a human-in-the-loop one (the on-demand cartographer, the on-demand cartographer rule). */
	...
	/** The project-scope collaborator (the channel-probe rule: distinguish a connected server, a brief boot delay, and a persistent failure). Default: `productionProjectScopeRegistrar`. */
...
/**
 * Add ONE member to the running crew (the membership rule: add, remove, or respawn one role without restarting the whole crew): assert the same launch preconditions stand-up does
...
/**
 * Retire ONE crew member (the membership rule: add, remove, or respawn one role without restarting the whole crew): validate the instance against the role's kind, find its pane,
