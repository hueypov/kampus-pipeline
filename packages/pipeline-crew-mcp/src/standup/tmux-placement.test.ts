 ...
 * engine's per-instance id) rather than a config `tmux` dimension (which died with the one-role-map seam, * 0189 / the role-map rule: store all role configuration in one map keyed by role kind). The single crew window the panes tile in is named at launch, and the SESSION it opens under
 * is resolved at launch to the caller's current tmux session (founder ruling the tmux-host rule: derive placement from the caller session at launch rather than persisted config/the rollback rule: clean launcher-owned artifacts after a failed or retired launch) — so a target carries
...
// carrying its per-instance identity from the role-binding rule: each session receives the launcher-assigned role and engine-instance identity). Bridges label their pane by role slug.
