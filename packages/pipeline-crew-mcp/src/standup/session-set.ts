 ...
 * roster + its per-kind cardinality contract (the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes): one instance per bridge kind, N instances
 * of the engine kind (N from the config engine-count dimension). Pure over the roster contract — no
 * process spawn, no argv (the bind constructor; the roster-set rule: derive one session per bridge and the configured number per engine from the role map turns each `{role, address}` into argv, the
 ...
 * (the on-demand cartographer rule, the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes).
 *
 ...
 * has no standing loop to run, so it is never stood up here (the on-demand cartographer rule). Total over the autobooted roles:
 * every one is kinded, so none is dropped or double-counted.
 */
/**
 * Derive ONE session for a single role — the on-demand membership op's session (the membership rule: add, remove, or respawn one role without restarting the whole crew), the same
 ...
 * boot prompt, so it comes up idle waiting for the human), the on-demand cartographer rule.
 */
...
export const deriveSessionSet = (input: SessionSetInput): readonly CrewSession[] => {
	...
	for (const role of CREW_ROLES) {
		...
		// cartographer) is on-demand — spawned by a human when wanted — so it is skipped here (the on-demand cartographer rule).
