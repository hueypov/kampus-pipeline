 ...
 * crew session (the crew-start rule: derive and launch the configured roster from one validated config) comes up with. For a role + project root it derives the launch inputs
 * for ONE session — no process spawn here (the orchestration step; the atomic stand-up rule: launch the complete roster or fail without leaving a partial crew spawns), pure derivation
 * over the already-resolved `LaunchConfig` channels dimension (the validated-config rule: parse channel settings, role tiers, and CLI version once before launch).
 *
 ...
 *      invisible to the resolver and the channel comes up inert crew-wide (the channel-probe rule: distinguish a connected server, a brief boot delay, and a persistent failure). The command
 *      is the launcher's own node (`process.execPath`) + the ABSOLUTE `bin.ts` path — a bare unlinked
 *      package bin name never resolves on the launched session's PATH (the resolvable-server rule: bind each session to an absolute existing channel-server entry point); this mirrors
 ...
 * crew server's `bin.ts` not resolving on disk (`CrewSessionBinUnresolvableError`, the resolvable-server rule: bind each session to an absolute existing channel-server entry point), the crew
 ...
 * not resolve on the launched session's PATH (the resolvable-server rule: bind each session to an absolute existing channel-server entry point). Same idiom as ensure-tracker.ts's `SELF_PATH`.
 */
...
/**
 * Binds the launcher-assigned per-instance identity (the role-binding rule: each session receives the launcher-assigned role and engine-instance identity) into the session command, so the
 * launched engine comes up on THAT address instead of `crew/session.ts` re-minting its own runtime
 * instance — the C5 handoff the role-binding rule: each session receives the launcher-assigned role and engine-instance identity left for bind to turn into argv (the registration rule: every launched session registers its channel server with its stable role and instance identity, seam 3).
 */
export const CREW_SESSION_INSTANCE_FLAG = "--instance";
/** Sets the launched session's model to the role's configured tier (the optional role-tier setting); omitted when no tier. */
 ...
 * the operator-legibility symptom (the display-identity rule: name panes by role and engine instance so operators can distinguish them).
 */
 ...
 * through to the generic general-purpose default (the fallback-to-generic-agent failure, the role-persona rule: launch each pane with the crew plugin and its crew-role agent definition). The pipeline-crew
 ...
 * the personal persona instead (the role-persona rule: launch each pane with the crew plugin and its crew-role agent definition collision). The `crew-<role>` names are collision-free, so
 * `--agent` passes `crew-<role>` (the bare role mapped at the argv site below); the bare role stays the
 * key everywhere else — `CREW_ROLES`, the channel role map, model tiering, `--name` (the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes, the role-persona rule: launch each pane with the crew plugin and its crew-role agent definition).
 */
export const PLUGIN_DIR_FLAG = "--plugin-dir";
/** Boots the session AS its role persona by resolving the plugin agent-def named `crew-<role>` (see PLUGIN_DIR_FLAG, the role-persona rule: launch each pane with the crew plugin and its crew-role agent definition). */
 ...
 * missing piece for the crew engine's cold-start self-drain (the boot-turn rule: a spawned member receives its role-specific initial prompt): the role def (the bind-construction rule: build complete session argv deterministically from validated config) already
 ...
 * idle with no prompt instead (the drive branch in `buildSessionBind`, the on-demand cartographer rule).
 */
 ...
 * carries, so the channel resolver (which reads persisted scopes, the channel-probe rule: distinguish a connected server, a brief boot delay, and a persistent failure) sees the server.
 */
 ...
 * command. When an engine's launcher-assigned `instance` (the role-binding rule: each session receives the launcher-assigned role and engine-instance identity) is present it is baked as
...
export interface SessionBind {
	...
	/** The crew server's persisted-scope config value the launcher writes to the pane's project-scope `.mcp.json` (the channel-probe rule: distinguish a connected server, a brief boot delay, and a persistent failure). */
	...
	/** `["--model", "<alias>"]` when the role has a configured tier (the optional role-tier setting), else `[]` (CLI default). */
	readonly modelArg: readonly string[];
	/**
	 * `["--name", "<display name>"]` — the visible per-session identity (the display-identity rule: name panes by role and engine instance so operators can distinguish them). A bridge is the
	 ...
	 * PLUGIN_DIR_FLAG, the role-persona rule: launch each pane with the crew plugin and its crew-role agent definition).
	 */
	 ...
	 * hand-kick (the boot-turn rule: a spawned member receives its role-specific initial prompt; see BOOT_PROMPT). It rides the argv TAIL, after the non-variadic `--name <name>`,
	 ...
	 * it comes up idle waiting for the human rather than manufacturing work (the drive branch, the on-demand cartographer rule).
	 */
	 ...
	 * launcher passes to `claude`. It boots the role persona (`--plugin-dir` + `--agent`, the role-persona rule: launch each pane with the crew plugin and its crew-role agent definition), then
	 * gives the session its boot turn via the tail positional prompt (the boot-turn rule: a spawned member receives its role-specific initial prompt), and no longer carries
	 * `--mcp-config`: the crew server now registers via the pane's project-scope `.mcp.json`
	 * (`serverConfig`), which is what the channel resolver actually reads (the channel-probe rule: distinguish a connected server, a brief boot delay, and a persistent failure).
	 */
...
export interface SessionBindInput {
	...
	readonly serverName: string;
	/**
	 * The launcher-assigned per-instance identity (the role-binding rule: each session receives the launcher-assigned role and engine-instance identity) an engine session binds — baked into the
	 * session argv as `--instance <id>` so the launched engine comes up on that address rather than
	 * re-minting its own runtime instance (the registration rule: every launched session registers its channel server with its stable role and instance identity, seam 3). A bridge is a singleton and omits it.
	 */
	readonly instance?: string | undefined;
	/**
	 * The role's configured model tier (the optional role-tier setting) — emitted as `--model <alias>` so the launched session
	...
	/** The resolved channels dimension of the crew `LaunchConfig` (the validated-config rule: parse channel settings, role tiers, and CLI version once before launch), consumed read-only. */
 ...
 * to `CrewServerNotRegisteredError` that let the resolvable-server rule: bind each session to an absolute existing channel-server entry point be SILENT (registration was guarded, resolvability
...
> =>
	Effect.gen(function* () {
		// The platform is the swappable seam: `FileSystem` probes
		...
		// the launched `<node> <bin.ts>` fails to spawn and the channel is inert (the resolvable-server rule: bind each session to an absolute existing channel-server entry point). A probe fault
		...
		// The role's tier boots the session on its `--model` (the optional role-tier setting); a role with no tier emits none,
		...
		// The persona boot (the role-persona rule: launch each pane with the crew plugin and its crew-role agent definition): --plugin-dir loads the pipeline-crew plugin so --agent resolves the
		...
		// The launcher's boot turn (the boot-turn rule: a spawned member receives its role-specific initial prompt): a tail positional prompt so the spawned session fires its
		...
		// role with no standing loop confabulate work, so it boots idle instead (the on-demand cartographer rule). The drive is a
