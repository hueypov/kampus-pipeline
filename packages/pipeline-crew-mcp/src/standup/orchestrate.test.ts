/**
 * standup/orchestrate — the one stand-up command (the atomic stand-up rule: launch the complete roster or fail without leaving a partial crew), booting against the post-the role-map rule: store all role configuration in one map keyed by role kind
 * one-role-map config (the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes): the engine count folds into `roles["engineering-manager"].count`
...
/**
 * A post-the role-map rule: store all role configuration in one map keyed by role kind one-role-map crew config (raw, on-disk shape): the engine count lives at
 ...
 * the tmux-host rule: derive placement from the caller session at launch rather than persisted config: the caller's current session, not a hardcoded one). It also records each launch's `intoWindow`, so a
 * test can prove the FIRST session opens the crew window (`intoWindow` undefined) and every later one splits into
 * the window id the first returned (founder ruling the rollback rule: clean launcher-owned artifacts after a failed or retired launch). Neither touches tmux: the launcher captures its plans,
...
/** A fake per-pane cwd the recording registrar mints — distinct per pane so each leaf `.mcp.json` is isolated (the channel-probe rule: distinguish a connected server, a brief boot delay, and a persistent failure). */
 ...
 * captured in-memory (the channel-probe rule: distinguish a connected server, a brief boot delay, and a persistent failure).
 */
 ...
 * `resolveTargetTmuxSession` run their exit-code branches with no real tmux — the async-exit seam the tmux-host rule: derive placement from the caller session at launch rather than persisted config closed. */
...
// The autobooted bridges — a HITL bridge (the cartographer) is on-demand, not stood up (the on-demand cartographer rule).
const bridgeRoles = CREW_ROLES.filter((r) => kindOf(r) === "bridge" && isAutobooted(r));

describe("standup/orchestrate — the one stand-up command (the atomic stand-up rule: launch the complete roster or fail without leaving a partial crew)", () => {
	it.effect(
	...
	it.effect(
		"launches the crew as PANES of ONE window: first opens it, every later pane splits into that id (the rollback rule: clean launcher-owned artifacts after a failed or retired launch AC1,AC2)",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"registers each pane's crew server as a project-scope leaf — reap → register → launch, distinct isolated cwds (the channel-probe rule: distinguish a connected server, a brief boot delay, and a persistent failure)",
		() =>
			Effect.gen(function* () {
				const N = 2;
				...
				// and carries exactly the bind's server config — the value the resolver reads (the channel-probe rule: distinguish a connected server, a brief boot delay, and a persistent failure).
	...
	it.effect(
		"aborts fail-loud when the project-scope register fails — nothing launched (AC3, the channel-probe rule: distinguish a connected server, a brief boot delay, and a persistent failure)",
		() =>
			Effect.gen(function* () {
				const {input, launched} = baseInput({
					localScope: {
	...
	it.effect("threads each role's config tier into its launch --model (the optional role-tier setting, end to end)", () =>
		Effect.gen(function* () {
			...
			// cartographer is not autobooted (the on-demand cartographer rule), so it never launches here; its tier is asserted at the
	...
	it.effect(
		"resolves the target session BEFORE placing any pane, and opens every pane into it (the tmux-host rule: derive placement from the caller session at launch rather than persisted config AC1)",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"aborts fail-loud when the target session cannot be resolved (outside-tmux fallback create fails) (the tmux-host rule: derive placement from the caller session at launch rather than persisted config AC3)",
		() =>
			Effect.gen(function* () {
				const {input, launched} = baseInput({
					resolveTargetSession: () =>
	...
	it.effect(
		"reports no launch count for a session that never started — a failed launch fails loud (the tmux-host rule: derive placement from the caller session at launch rather than persisted config AC4)",
		() =>
			Effect.gen(function* () {
...
/**
 * The real launcher primitives against a scripted tmux runner — the async-exit path the tmux-host rule: derive placement from the caller session at launch rather than persisted config closed: a
 * non-zero exit is inspected and fails closed, not counted as success. No real tmux, no `claude`.
 */
describe("standup/orchestrate — launch-liveness + ensure-session (the tmux-host rule: derive placement from the caller session at launch rather than persisted config)", () => {
	...
	it.effect(
		"launchSessionInTmux (first pane) opens the crew window into the resolved session and captures its id (the rollback rule: clean launcher-owned artifacts after a failed or retired launch)",
		() =>
			Effect.gen(function* () {
				...
				// the crew window opens into the RESOLVED session (the caller's, the tmux-host rule: derive placement from the caller session at launch rather than persisted config), not a hardcoded one,
				// and `-c <cwd>` boots the pane in its distinct launch cwd (the isolated projects[] key, the channel-probe rule: distinguish a connected server, a brief boot delay, and a persistent failure).
	...
	it.effect(
		"launchSessionInTmux (later pane) splits into the crew window and re-tiles on exit-0 (the rollback rule: clean launcher-owned artifacts after a failed or retired launch AC1,AC2)",
		() =>
			Effect.gen(function* () {
				const plan = yield* captureOnePlan;
	...
	it.effect(
		"launchSessionInTmux (first pane) fails closed on an async non-zero exit — the swallowed failure the tmux-host rule: derive placement from the caller session at launch rather than persisted config",
		() =>
			Effect.gen(function* () {
				const plan = yield* captureOnePlan;
	...
	it.effect(
		"launchSessionInTmux (later pane) fails closed when the split-window exits non-zero (the rollback rule: clean launcher-owned artifacts after a failed or retired launch AC3)",
		() =>
			Effect.gen(function* () {
				const plan = yield* captureOnePlan;
				const {runner} = scriptedTmux([exited(1)]);
	...
	it.effect(
		"launchSessionInTmux (later pane) fails closed when select-layout exits non-zero (the rollback rule: clean launcher-owned artifacts after a failed or retired launch AC3)",
		() =>
			Effect.gen(function* () {
				const plan = yield* captureOnePlan;
...
// The dev-channel dialog fix (the startup-order rule: use an interactive login shell so a required channel-approval dialog can complete before the pane exits): `claude` runs THROUGH an interactive login shell so the startup
...
// TaggedError is tag-only, so both `reason`-carrying union members must surface their rich fields (the command-surface rule: expose stand-up as a single CLI subcommand).
...
// The symmetric teardown (the channel-probe rule: distinguish a connected server, a brief boot delay, and a persistent failure) delegates entirely to the injected reap — which removes the
