/**
 * standup/config — the launch-dimension reader (the validated-config rule: parse channel settings, role tiers, and CLI version once before launch), reconciled to the one-role-map seam
 * shape (the topology rule: ; the role-map rule: store all role configuration in one map keyed by role kind). Covers path resolution, the JSONC strip (comments + trailing commas,
 ...
 * allowlist accepts (a bare `server:` ref can't ride `--channels`; the fail-closed config rule: reject malformed channel references and unsafe combinations before starting).
 */
const validLaunch = {
	cliVersion: "2.1.207",
	roles: {
 ...
 * config (the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes): every bridge role entry, each role's `tier`/`wipCap`, the operator +
 * notification + controlPlaneApprover blocks, and — post-the role-map rule: store all role configuration in one map keyed by role kind — NO tmux key. All of it is excess
...
describe("standup/config — decodeLaunchConfig (happy path, one-role-map shape)", () => {
	...
	it.effect(
		"folds each role's tier into roleTiers instead of dropping it as an excess key (the optional role-tier setting)",
		() =>
			Effect.gen(function* () {
				const cfg = yield* decodeLaunchConfig(fullCrewConfig, DEFAULT_CONFIG_PATH);
	...
	it.effect("an omitted cliVersion decodes cleanly to an unpinned launch (the optional CLI-version rule)", () =>
		Effect.gen(function* () {
	...
	it.effect("accepts the development channel mode carrying a top-level server: ref", () =>
		Effect.gen(function* () {
			...
			// representable only under development mode — not allowlist (the fail-closed config rule: reject malformed channel references and unsafe combinations before starting).
			const cfg = yield* decodeLaunchConfig(
				{
					...validLaunch,
					channels: {
...
describe("standup/config — channel-ref grammar matches Claude Code 2.1.212", () => {
	...
	// plugin:<plugin>:<server> shape (the validated-config rule: parse channel settings, role tiers, and CLI version once before launch) has no @ and is rejected (the fail-closed config rule: reject malformed channel references and unsafe combinations before starting).
	it.effect("accepts a plugin:<name>@<marketplace> ref under allowlist", () =>
		Effect.gen(function* () {
			const cfg = yield* decodeLaunchConfig(
				{...validLaunch, channels: {...validLaunch.channels, servers: ["plugin:notes@acme"]}},
				DEFAULT_CONFIG_PATH,
			);
			assert.deepStrictEqual([...cfg.channels.servers], ["plugin:notes@acme"]);
		}),
	);
	it.effect("rejects the old plugin:<plugin>:<server> shape", () =>
		Effect.gen(function* () {
			const err = yield* decodeErr({
				...validLaunch,
				channels: {...validLaunch.channels, servers: ["plugin:acme:notes"]},
			});
...
describe("standup/config — role tier fails closed (the optional role-tier setting)", () => {
	it.effect(
...
describe("standup/config — fails closed naming the offending dimension", () => {
	// NB: an ABSENT cliVersion is NOT a failure — it decodes to the unpinned launch (the optional CLI-version rule),
