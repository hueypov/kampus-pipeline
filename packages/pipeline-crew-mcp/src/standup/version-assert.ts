 ...
 * (the pre-launch CLI-version check). At stand-up it reads the installed Claude Code CLI version and compares it to the
 * pinned `cliVersion` the operator config carries (the validated-config rule: parse channel settings, role tiers, and CLI version once before launch's `LaunchConfig`, consumed read-only).
 *
 ...
 * match proceeds silently. The pin is OPTIONAL (the optional CLI-version rule): when it is ABSENT the assert is
 ...
 * launcher (the atomic stand-up rule: launch the complete roster or fail without leaving a partial crew) wires the real `claude --version` while tests stub it; running the subprocess
 ...
 *
 * The pin is optional (the optional CLI-version rule): an ABSENT `cliVersion` is the "unpinned" launch — the assert is
