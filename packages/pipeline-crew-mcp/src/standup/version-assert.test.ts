/**
 * standup/version-assert — the pre-launch pinned-CLI-version assert (the pre-launch CLI-version check). Covers the
 * decisions the assert makes with the launch side stubbed via an injected version reader: an
 * absent pin skips the assert unread (the unpinned launch, the optional CLI-version rule), installed == pinned
...
describe("standup/version-assert — assertPinnedCliVersion", () => {
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				// The unpinned launch (the optional CLI-version rule): with no pin the assert is skipped entirely and the
