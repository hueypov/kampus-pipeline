/**
 * standup/session-set — the roster-driven session set (AC 1–4, the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes; the on-demand cartographer rule). These tests pin the
...
// (the cartographer) is excluded: on-demand, never in the standing drain crew (the on-demand cartographer rule).
...
describe("standup/session-set — roster-driven session set (the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes)", () => {
	...
	it("excludes a human-in-the-loop role (the cartographer) from the stand-up set (the on-demand cartographer rule)", () => {
		...
		assert.isFalse(roles.has("cartographer"), "cartographer must not be autobooted (the on-demand cartographer rule)");
