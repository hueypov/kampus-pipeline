 ...
 * on (the channel-probe rule: distinguish a connected server, a brief boot delay, and a persistent failure). `fileExists` is injected so the guard is unit-testable without planting real files.
 */
...
export const withLockedJsonFile = (
	...
	return Effect.acquireUseRelease(
		...
		// to `never`. A bare `Effect.promise` here would turn a rejection into an uncatchable defect (the effect-isolation rule: model failures in the effect flow rather than letting a rejected promise escape as a defect).
		(release) =>
			Effect.tryPromise({
				try: () => release(),
...
/** Ensure the crew's git-root folder is trusted so the panes boot without the trust dialog. Idempotent; the adopting repository root is likely already trusted. */
