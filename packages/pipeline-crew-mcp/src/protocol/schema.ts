 ...
 * string, got 3621` with nothing surfacing the shape ahead of the send (the message-schema contract). `PrNumber` is the
...
/** A GitHub pull-request number — a positive integer, branded distinctly from an issue number (see `IssueNumber`, the message-schema contract). */
 ...
 * (the safe-release rule: only the claim holder may release it, and releasing a missing claim is a no-op). No reply: releasing is idempotent.
 */
 ...
 * authoritative pull-source), and a dropped/offline nudge is log-and-continue. See the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes.
 */
