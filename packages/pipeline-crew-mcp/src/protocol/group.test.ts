...
describe("protocol/group catalog", () => {
	...
	it("Release is fire-and-forget: no reply schema, so releasing is idempotent (the claim-lifecycle rule: presence leases and resource claims use separate keyspaces; claims live only while their holder is present and release on completion or holder expiry)", () => {
		assert.strictEqual(Release.successSchema, Schema.Void);
	});

	it("payloadSchemaForKind resolves a catalog kind to its payload schema (the advisory-message rule: valid catalog messages may nudge but never assign work)", () => {
	...
	it("payloadSchemaForKind returns undefined for a kind outside the catalog (the advisory-message rule: valid catalog messages may nudge but never assign work)", () => {
