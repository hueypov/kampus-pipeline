/**
 * The discoverable kind→shape surface (the message-schema contract): a peer resolves each kind's payload shape and the
...
describe("protocol/describe — the discoverable kind→shape surface (the message-schema contract)", () => {
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				// legible as the accepted shape without triggering the send-time reject that hid it (the message-schema contract).
