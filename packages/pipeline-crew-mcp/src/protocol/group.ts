...
/** Kind 1b — release a held resource claim (fire-and-forget; the claim lifecycle's free, the claim-lifecycle rule: presence leases and resource claims use separate keyspaces; claims live only while their holder is present and release on completion or holder expiry). */
 ...
 * the crew catalog. Advisory only — never command authority or lane-assignment (the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes).
 */
 ...
 * so the 7-kind catalog is enforced at the wire rather than advisory (the advisory-message rule: valid catalog messages may nudge but never assign work). Derived straight
