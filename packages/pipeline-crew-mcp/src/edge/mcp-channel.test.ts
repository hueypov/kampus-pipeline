 ...
 * (the wake-notification rule: emit a channel wake without double encoding, the MCP-channel rule: crew roles communicate through MCP channels instead of a terminal relay; the local dependency-patch rule: rely on released dependencies or committed local pnpm patches, never forks, git URLs, or unmerged upstream work pnpm-patch idiom). The patch adds, to effect's
 ...
 *
 * This IS the effect `@patch-pin` behavior pin: behavior-pinning tests guard the local dependency
 * patch. Two layers:
 ...
 *      patch can't rot silently under a bump (behavior-pinning, the delivery rule: queue each valid message to the recipient inbox).
...
const makeInitializedClient = Effect.gen(function* () {
	...
	// whose rejection escapes as an uncatchable defect: model failures in the effect flow rather than letting a rejected promise escape as a defect).
...
describe("effect MCP edge patch — behavior pin (claude/channel capability + notification)", () => {
	...
	// Pin the 2.1.214 CLIENT contract, not just server union membership (the peer-liveness rule: connect to live peers and treat a disconnected peer as unavailable). The Claude Code
