/**
 * Defect (a) — the send-tool boundary normalizes `body` to a struct once (the single-encoding rule: encode each notification exactly once before emission). The MCP
...
describe("defect(a): channel_send normalizes body to a struct at the boundary (the single-encoding rule: encode each notification exactly once before emission)", () => {
