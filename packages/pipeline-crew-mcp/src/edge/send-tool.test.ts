 ...
 * a silent drop (the role-addressing rule: send to a role inbox, and treat delivery as queued rather than read). Driven against a real in-memory `McpServer.layerHttp` +
...
describe("edge/send-tool — the channel edge server (ACs 1, 3)", () => {
	...
	// The EngineNudge drop path (the offline peer behavior): an advisory chief-of-staff → engine nudge to an OFFLINE
	...
	// correctness (the board stays the authoritative pull-source; the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes).
	it.effect(
	...
	// The shape gate (the advisory-message rule: valid catalog messages may nudge but never assign work): an unknown kind or a body that fails its kind's schema is rejected
	...
	// the offline-send rule: report an unreachable recipient instead of silently dropping the message: the MCP tools/call client serializes an unconstrained `body` as a JSON *string*, so a
	...
	// the offline-send rule: report an unreachable recipient instead of silently dropping the message AC#4: a genuine decode failure must surface InvalidMessageError.reason in the rendered
