...
/**
 * Behavior pin for the (the drain rule: emit each queued notification once) hunk of `patches/effect@4.0.0-beta.92.patch` (the local dependency-patch rule: rely on released dependencies or committed local pnpm patches, never forks, git URLs, or unmerged upstream work): the McpServer
...
describe("effect McpServer drain patch (the drain rule: emit each queued notification once) — a wake reaches an initialized client", () => {
	it.live(
