	...
	it.effect("a delivery emits a channel wake with the message as a <channel> tag", () =>
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				// delivered-to-inbox, stamped by this edge peer — never seen-by-model (the role-addressing rule: send to a role inbox, and treat delivery as queued rather than read).
