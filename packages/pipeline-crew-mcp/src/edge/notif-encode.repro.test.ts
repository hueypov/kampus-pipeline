...
/**
 * Behavior pin for the (b1) hunk of `patches/effect@4.0.0-beta.92.patch` (the single-encoding rule: encode each notification exactly once before emission, the local dependency-patch rule: rely on released dependencies or committed local pnpm patches, never forks, git URLs, or unmerged upstream work): the
...
describe("effect notification encode patch (b1) — by-tag, not blind union (the single-encoding rule: encode each notification exactly once before emission)", () => {
