...
/**
 * Behavior pin for the (b2) hunk of `patches/effect@4.0.0-beta.92.patch` (the optional-id rule: omit an absent identifier instead of serializing null, the local dependency-patch rule: rely on released dependencies or committed local pnpm patches, never forks, git URLs, or unmerged upstream work): the
...
describe("effect notification encode patch (b2) — omit id for notifications (the optional-id rule: omit an absent identifier instead of serializing null)", () => {
