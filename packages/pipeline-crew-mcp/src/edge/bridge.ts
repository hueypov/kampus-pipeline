 ...
 * with no addressing or acknowledgements, so this does last-mile delivery only; addressing
 ...
 * inbox contract (the role-addressing rule: send to a role inbox, and treat delivery as queued rather than read); it never means seen-by-model (the channel has no delivery acks).
 */
