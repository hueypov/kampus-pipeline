 ...
 * role may send/receive) instead of discovering the shape by triggering a send-time reject (the message-schema contract).
 ...
 * crew's role topology (the crew composition root does, the composition-boundary rule: generic channel edges consume ports while the crew root owns peer construction). The crew resolves the contract on its
