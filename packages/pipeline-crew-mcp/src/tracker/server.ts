 ...
 *
 * The socket path is the canonical per-repo rendezvous (`./rendezvous.ts`, the canonical-rendezvous rule: derive the tracker location from the repository shared Git directory so every worktree joins one tracker; fail rather than fall back when Git cannot identify it: derive the tracker location from the repository shared Git directory so every worktree joins one tracker; fail rather than fall back when Git cannot identify it), so every peer of
 ...
 *
 * Crash recovery lives here too (the stale-socket safety rule: reclaim a socket only after a failed connection proves no tracker is live): a unix socket *file* survives an ungraceful host exit —
 ...
 * under it (the stale-socket safety rule: reclaim a socket only after a failed connection proves no tracker is live AC — never an unconditional unlink); an absent file (`ENOENT`) is the normal
...
export const reclaimStaleSocket = (
	...
	probeStaleSocket(socketPath).pipe(
		Effect.flatMap((isStale) =>
			isStale
				? Effect.gen(function* () {
						// Unlink via the `FileSystem` seam, discharged by
 ...
 * Grounded in node `net` connect semantics (verified against the real platform, the stale-socket safety rule: reclaim a socket only after a failed connection proves no tracker is live): a live
 ...
 * is cleared before the bind — the crash-recovery contract in the module docblock (the stale-socket safety rule: reclaim a socket only after a failed connection proves no tracker is live). A live
