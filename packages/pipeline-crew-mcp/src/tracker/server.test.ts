 ...
 * (the stale-socket safety rule: reclaim a socket only after a failed connection proves no tracker is live — reclaim a crashed host's orphaned socket, never reclaim a live one).
 */
 ...
 * then refuses a connect with `ECONNREFUSED`, the stale signal `reclaimStaleSocket` keys on (the stale-socket safety rule: reclaim a socket only after a failed connection proves no tracker is live).
 */
...
describe("tracker stale-socket crash recovery (the stale-socket safety rule: reclaim a socket only after a failed connection proves no tracker is live)", () => {
