...
export class Registry extends Context.Service<
	Registry,
	{
		...
		/** Free `resource`'s claim iff `claimant` holds it — steal-release is a no-op (the safe-release rule: only the claim holder may release it, and releasing a missing claim is a no-op). */
		readonly releaseClaim: (input: {
			readonly resource: string;
			readonly claimant: string;
		}) => Effect.Effect<void>;
		/** Refresh the TTL window for every role lease `peer` holds (never a claim — the heartbeat rule: heartbeats refresh presence leases only and never refresh resource claims). */
