 ...
 *
 * Two keyspaces that never share a map (the claim-lifecycle rule: presence leases and resource claims use separate keyspaces; claims live only while their holder is present and release on completion or holder expiry): presence leases keyed by `peer`, and
 ...
 *     (the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes) — so the presence registry stays role-agnostic and never collapses a pool.
 ...
 *     presence is live (the presence-derived liveness rule: a resource claim is live only while its holder has a live presence lease). One liveness clock (presence); claims ride it. So a
 ...
 * (the presence-derived liveness rule: a resource claim is live only while its holder has a live presence lease). `claimantRole` records which role holds the claim so tooling can read it
...
/** True iff `holder` holds a live presence lease — the claim-liveness clock (the presence-derived liveness rule: a resource claim is live only while its holder has a live presence lease). */
 ...
 * presence never rejects (role uniqueness is the crew cardinality claim's job, the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes), it only
 ...
 * claim whose holder is no longer present is treated as free here (the presence-derived liveness rule: a resource claim is live only while its holder has a live presence lease).
 */
 ...
 * claim it does not hold (steal-release is unrepresentable, the safe-release rule: only the claim holder may release it, and releasing a missing claim is a no-op); releasing a claim
 ...
 * claims are never touched — they have no TTL to bump, and their liveness rides presence (the claim-lifecycle rule: presence leases and resource claims use separate keyspaces; claims live only while their holder is present and release on completion or holder expiry
 ...
 * has one entry; for an engine the set carries every live instance in the pool (the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes). An empty
 * array is the explicit not-present result (never a silent drop). Reads the presence keyspace only,
 * so it can never surface a resource-claim holder (the claim-lifecycle rule: presence leases and resource claims use separate keyspaces; claims live only while their holder is present and release on completion or holder expiry).
 */
export const lookup = (
	state: RegistryState,
	role: string,
 ...
 * claims are stale and dropped with it (the presence-derived liveness rule: a resource claim is live only while its holder has a live presence lease).
 */
 ...
 * has a live presence lease (the presence-derived liveness rule: a resource claim is live only while its holder has a live presence lease — a claim never outlives its holder's presence).
