 ...
 * `Registry` service across the two keyspaces (the claim-lifecycle rule: presence leases and resource claims use separate keyspaces; claims live only while their holder is present and release on completion or holder expiry). `AnnouncePresence` / `LookupRole` /
