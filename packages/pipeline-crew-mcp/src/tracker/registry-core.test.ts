 ...
 * and the explicit not-present result — plus the resource-claim lifecycle (the claim-lifecycle rule: presence leases and resource claims use separate keyspaces; claims live only while their holder is present and release on completion or holder expiry): keyspace
...
describe("registry-core — keyspace separation (the separate-keyspaces rule: role presence and resource claims cannot be read through the same lookup)", () => {
...
describe("registry-core — presence-derived claim liveness (the presence-derived liveness rule: a resource claim is live only while its holder has a live presence lease)", () => {
...
describe("registry-core — Release + reaping (the claim-lifecycle rule: presence leases and resource claims use separate keyspaces; claims live only while their holder is present and release on completion or holder expiry facets 2 + 3)", () => {
...
describe("registry-core — heartbeat refreshes presence only (the heartbeat rule: heartbeats refresh presence leases only and never refresh resource claims)", () => {
