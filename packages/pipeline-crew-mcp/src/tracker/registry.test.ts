...
describe("tracker registry — wire round-trips (RpcTest in-memory)", () => {
	...
	it.effect("Claim grants a free resource and collides on a second acquire (holder present)", () =>
		Effect.gen(function* () {
			const client = yield* RpcTest.makeClient(TrackerRegistry);
			// The incumbent's claim is live only while its presence is — announce it first (the claim-lifecycle rule: presence leases and resource claims use separate keyspaces; claims live only while their holder is present and release on completion or holder expiry).
	...
	it.effect("Release by a non-holder is a no-op — a peer cannot steal-release (the claim-lifecycle rule: presence leases and resource claims use separate keyspaces; claims live only while their holder is present and release on completion or holder expiry)", () =>
		Effect.gen(function* () {
			const client = yield* RpcTest.makeClient(TrackerRegistry);
			yield* client.AnnouncePresence({peer: "peer-a", role: "builder", at});
			yield* client.Claim({resource: "issue-3054", claimant: "peer-a", role: "builder", at});
