/**
 * crew/session — THE runnable stdio entry the cutover binds (the session-start rule: start one role-bound MCP server and peer for each session): stand up one live crew
 ...
 *
 * the composition-boundary rule: generic channel edges consume ports while the crew root owns peer construction built the composition (`makeCrewChannel`) but deliberately left this live entry out; this
 ...
 * absent) — the peer-liveness rule: connect to live peers and treat a disconnected peer as unavailable assembly defect. See `assembleCrewSession` for the mechanism (merge the
 * registrations + provide the transport once; claim the peer FIRST so `ChannelSend` is an instant
 * binding when the toolkit registers). The shared session scope owns the registered transport and services.
 *
 ...
 * role admits N instances, and presence frees on teardown (connection-is-lease, the role-addressing rule: send to a role inbox, and treat delivery as queued rather than read). A bridge's
 ...
 * `inboxAddressFor` (the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes).
 *
...
export interface CrewSessionConfig {
	...
	/**
	 * The launcher-assigned per-instance identity (the role-binding rule: each session receives the launcher-assigned role and engine-instance identity) an engine session binds, threaded from the
	 ...
	 * or a direct run) ⇒ `sessionInstance` mints one. See `sessionInstance` (the launcher/session contract: the launcher and session CLI agree on role and instance arguments).
	 */
	readonly instance?: string;
}

/**
 * This session's dialable inbox address, keyed by the role's KIND (the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes). A BRIDGE keeps the
 ...
 * bound by standup/bind.ts's `--instance`, the role-binding rule: each session receives the launcher-assigned role and engine-instance identity), else a freshly minted UUID. The fallback preserves
 * the launcher/session contract for the no-instance case — a bridge singleton, which folds no instance into
 ...
 * merged and provided their `transport` exactly ONCE. This is the load-bearing composition (the peer-liveness rule: connect to live peers and treat a disconnected peer as unavailable),
 * and it must win TWO races or the live session advertises no tools (`/mcp` Capabilities: none).
 *
 * Race 1 — the single instance (fixed the disconnected-peer behavior). `McpServer.toolkit` INTERNALLY self-provides its own
 ...
 *
 * Race 2 — claim BEFORE serve (the peer-liveness rule defect this build fixes). `McpServer.layerStdio` forks
...
export const assembleCrewSession = <RIn, RSub = never>(
	...
	// (`inboxServerSocketLayer`) reclaims a stale inbox socket through the `FileSystem` seam (the role-socket rule: each role uses its deterministic channel-server socket without collision),
	...
	Layer.unwrap(
		Effect.gen(function* () {
			...
			// Startup invariant (the message-schema contract): resolve the full discoverable channel contract BEFORE serving.
			...
			// deconfliction: the channel_claim toolkit (the duplicate-work failure: two engines can otherwise open competing lanes for the same resource), its ChannelClaim an INSTANT bind to the
			...
			// discovery: the channel_kinds toolkit (the message-schema contract), its ChannelDescribe an INSTANT bind to the
...
export const crewSessionLayer = (config: CrewSessionConfig) => {
	...
	// the session is live (the presence-only heartbeat rule: refresh role presence on a cadence below its TTL and never refresh resource claims). Shares `substrate`, so the beats reach the registry announced on.
