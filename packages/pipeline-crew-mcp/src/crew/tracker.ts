...
export class CrewTracker extends Context.Service<
	CrewTracker,
	{
		 ...
		 * claim via `Release` iff it was granted — the lane-finish fast path (the safe-release rule: only the claim holder may release it, and releasing a missing claim is a no-op). The
		...
		/** Free a resource claim this session holds — the explicit lane-finish release (the safe-release rule: only the claim holder may release it, and releasing a missing claim is a no-op). */
		 ...
		 * sends one `Heartbeat {peer, ttlSeconds}` and never touches a resource claim (the presence-lease rule: a live peer holds a TTL-bound role lease). Driven
