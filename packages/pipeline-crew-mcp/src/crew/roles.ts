 ...
 * → N), so a bridge-with-cardinality-2 is unrepresentable. See the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes.
 */
import {Schema} from "effect";

/**
 * A role's KIND governs its cardinality. A bridge owns a unique factory↔outside seam, so it is
 * singleton; an engine owns no seam and is fungible throughput, so it scales by count. See the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes.
 */
 ...
 * stops compiling. See the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes.
 */
 ...
 * than confabulating work. See the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes (roster law) + the on-demand cartographer rule.
 */
 ...
 * human-in-the-loop role (a HITL ideation partner with nothing to autorun, the on-demand cartographer rule); the rest self-drive.
 */
 ...
 * is on-demand — spawned by a human when wanted — and is never part of the standing drain crew (the on-demand cartographer rule).
 */
 ...
 * cardinality is through the kind, a bridge with cardinality 2 does not typecheck. See the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes.
 */
