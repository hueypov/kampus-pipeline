/**
 * crew/heartbeat — the presence keepalive keeps a live session's lease from aging out (the presence-only heartbeat rule: refresh role presence on a cadence below its TTL and never refresh resource claims).
 *
...
describe("crew/heartbeat — a live session outlives the TTL with the loop, ages out without it", () => {
	...
	it.effect(
		"without the loop, the same presence ages out past DEFAULT_TTL_SECONDS (the presence-only heartbeat rule: refresh role presence on a cadence below its TTL and never refresh resource claims bug)",
		() =>
			Effect.gen(function* () {
				const client = yield* RpcTest.makeClient(TrackerRegistry);
				yield* client.AnnouncePresence({peer, role, at: new Date().toISOString()});
