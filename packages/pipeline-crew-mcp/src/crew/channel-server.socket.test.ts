 ...
 *   - the stale-socket bind (the role-socket rule: each role uses its deterministic channel-server socket without collision): a crashed prior process's orphaned socket file at
 *     `inboxSocketPathFor(address)` is reclaimed (`reclaimStaleSocket`, the stale-socket safety rule: reclaim a socket only after a failed connection proves no tracker is live), so the server
 ...
 * keys on (the stale-socket safety rule: reclaim a socket only after a failed connection proves no tracker is live). Mirrors `tracker/server.test.ts`'s `leaveStaleSocket`.
 */
...
describe("crew/channel-server — REAL unix-socket inbox transport", () => {
	...
	it.live(
		"reclaims a crashed prior process's STALE socket and still binds (not SocketServerError: Open, the role-socket rule: each role uses its deterministic channel-server socket without collision)",
		() =>
			Effect.gen(function* () {
				const address = freshAddress();
				const socketPath = inboxSocketPathFor(address);
	...
	it.live(
		"does NOT reclaim a genuinely-live listener at the path — a second bind fails (the role-socket rule: each role uses its deterministic channel-server socket without collision guard)",
		() =>
			Effect.gen(function* () {
				const address = freshAddress();
				const socketPath = inboxSocketPathFor(address);
