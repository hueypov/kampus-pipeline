 ...
 * the ONE place cardinality is enforced, off `kindOf` (the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes); the generic tracker/protocol
 ...
 * tool's `ChannelSend` bound to this session's peer) is assembled by the bin/cutover (the session-start rule: start one role-bound MCP server and peer for each session,
 ...
 * cardinality (the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes). A `bridge` (cardinality 1) leases the bare role, so a second bridge
 ...
 * socket, decode error) collapses to `PeerUnreachableError`, never a silent drop (the role-addressing rule: send to a role inbox, and treat delivery as queued rather than read).
 */
 ...
 * the stale-socket safety rule: reclaim a socket only after a failed connection proves no tracker is live): the deterministic per-instance path is unlinked before bind IFF a connect proves it a
 ...
 * `SocketServerError: Open`, crashing the whole crew session (the role-socket rule: each role uses its deterministic channel-server socket without collision). A genuinely-live listener is
