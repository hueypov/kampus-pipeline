 ...
 * never ages out. Without it the substrate only functions for the first ~30s window (the presence-only heartbeat rule: refresh role presence on a cadence below its TTL and never refresh resource claims).
 *
 ...
 * presence-only is what stops a live session's beats from holding a stale claim alive (the presence-lease rule: a live peer holds a TTL-bound role lease).
 * The tracker-side keyspace split that makes the registry's own refresh presence-only is the heartbeat cadence rule: send heartbeats before the presence TTL expires,
