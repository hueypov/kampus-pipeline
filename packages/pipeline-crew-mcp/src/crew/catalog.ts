 ...
 * `releaseClaim` is the resource claim's free counterpart (the safe-release rule: only the claim holder may release it, and releasing a missing claim is a no-op).
 */
export const CrewSeams = {
	...
	// chief-of-staff → engine advisory nudge; scoped like intakePing, never routing (the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes).
