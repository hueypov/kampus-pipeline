/**
 * crew/roles — the kind-typed roster seam (the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes): the roster is three bridges + the engine
...
describe("crew/roles — the kind-typed roster (the topology rule: unique-seam bridges are singletons, while seam-free engines may scale and bridges never execute engine lanes)", () => {
	...
	it("kinds each role's drive: cartographer is human-in-the-loop, the rest self-drive (the on-demand cartographer rule)", () => {
	...
	it("isAutobooted is exactly the self-driving roster — the cartographer is on-demand (the on-demand cartographer rule)", () => {
		for (const role of CREW_ROLES) {
