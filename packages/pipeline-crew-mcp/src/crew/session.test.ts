/**
 * The cutover end to end (the session-start rule: start one role-bound MCP server and peer for each session): the live session's `ChannelSend`-from-peer binding stands up and
...
describe("crew/session — sessionInstance honors the launcher-assigned identity (the launcher/session contract: the launcher and session CLI agree on role and instance arguments AC 2)", () => {
	const engineRole = CREW_ROLES.find((role) => kindOf(role) === "engine");
	...
	it("mints a fresh UUID when absent (the bridge/singleton or direct-run case)", () => {
		...
		assert.match(minted, UUID_RE, "absent ⇒ a minted UUID, preserving pre-the launcher/session contract: the launcher and session CLI agree on role and instance arguments behavior");
...
describe("crew/session — the session-mode server wins the async-ChannelSend build race (the peer-liveness rule: connect to live peers and treat a disconnected peer as unavailable defect B)", () => {
...
describe("crew/session — the session-mode server advertises the channel edge (the peer-liveness rule: connect to live peers and treat a disconnected peer as unavailable defect B)", () => {
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				// on the same instance the transport serves. Regression guard for the peer-liveness rule: connect to live peers and treat a disconnected peer as unavailable completing bar
