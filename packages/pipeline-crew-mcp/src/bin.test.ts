/**
 * The `session` subcommand's flag surface (the launcher/session contract: the launcher and session CLI agree on role and instance arguments): the seam standup/bind.ts (the producer) bakes
...
describe("bin.ts — the session subcommand declares --instance (the launcher/session contract: the launcher and session CLI agree on role and instance arguments)", () => {
