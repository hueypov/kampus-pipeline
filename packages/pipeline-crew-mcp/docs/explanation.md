...
design. This quadrant states the governing invariants directly, so the package does not depend
on historical decision records. The claim lifecycle — keyspace split, presence-derived liveness,
and the `Release` mechanics — is explained here together with the tradeoffs those choices produce.
...
single most load-bearing choice in the model: a resource claim is live only while its holder has
a live presence lease. The shape it produces:
...
leave the lookup structurally able to read a claim; the separate-keyspaces rule: role presence and resource claims cannot be read through the same lookup records why it was
...
(the crew-MCP boundary: coordination infrastructure is outside the automation control plane because it grants no merge or approval authority); the coordination model
...
- Claim lifecycle, two-keyspace design, claim-liveness-rides-presence, the `Release` mechanics:
  the claim-lifecycle rule: presence leases and resource claims use separate keyspaces; claims live only while their holder is present and release on completion or holder expiry.
