/**
 * Legible failure for the in-repo-first bin path when a workspace dep isn't linked (the originating work item).
 *
 ...
 * (`ABORTED_REMOVE_MODULES_DIR_NO_TTY`) rather than wiping the modules farm (the originating work item).
 *
 ...
 * repo-root `node_modules` is a **symlink**. That is the documented failure corruption vector — a
...
/**
 * Bounded, idempotent self-heal for the unlinked-workspace-dep condition (the originating work item).
 *
 * `main-sync` (and every pipeline-cli tool) runs from source as `node src/bin.ts …`, so a
 * `pipeline-cli` workspace-dependency change can leave the primary checkout's `node_modules` stale and the dynamic
 ...
 * the caller falls back to the documented failure fail-fast remediation — the legible message is preserved as the
