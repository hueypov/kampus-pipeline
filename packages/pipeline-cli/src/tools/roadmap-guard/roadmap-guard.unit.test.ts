/**
 * Pure-core tests for `roadmap-guard` (the originating work item, roadmap map the originating work item): the parse of
 * ROADMAP.md's `## Arcs`/`## Campaigns` tables and the I1–I5 verdict (each invariant's
 * pass + every failure mode, plus the zero-scope fail-closed of the zero-scope fail-closed invariant and the
 * active↔done state symmetry of the originating work item). No IO — the `gh api`/filesystem seam is crossed
...
describe("judge — I4 zero-scope fail-closed (the zero-scope fail-closed invariant)", () => {
...
describe("renderReport", () => {
	...
	it("explains the fail-closed zero-scope verdict", () => {
		...
		expect(r).toContain("the zero-scope fail-closed invariant");
	});
