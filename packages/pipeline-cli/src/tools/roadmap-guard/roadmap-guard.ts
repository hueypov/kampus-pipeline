 ...
 * REST projection (roadmap map the originating work item, guard ruling the originating work item). IO-free and total: parsing
 ...
 * against it (the related failure modes). The invariants (I1–I5, extended to campaign rows):
 ...
 *   I4 — FAIL-CLOSED ON ZERO SCOPE (the zero-scope fail-closed invariant): zero arc rows or zero milestones ⇒
 *        a non-passing verdict, never a vacuous green (the readme-guard precedent).
 *   I5 — STATE SYMMETRY (the active↔done campaign lifecycle, the originating work item): an `active` row's
 ...
 * names the offending row/milestone so the report can print it on stderr (the zero-scope fail-closed invariant §1).
 */
...
export type RoadmapGuardVerdict =
	...
	/** Zero arc rows or zero milestones in scope — fail closed, never a vacuous pass (the zero-scope fail-closed invariant, I4). */
	| {
			readonly pass: false;
...
export const judge = (
	...
	// I5 — state symmetry (the active↔done lifecycle, the originating work item): an active row's milestone is
...
export const renderReport = (verdict: RoadmapGuardVerdict): string => {
	...
	if (verdict.reason === "zero-scope") {
		return (
			...
			"zero scope on one side, fail-closed (the zero-scope fail-closed invariant, I4). Is ROADMAP.md's `## Arcs` table present and " +
