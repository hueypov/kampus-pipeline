...
describe("Github.post — the SHA-bound, one-verdict-per-gate contract rule-2 upsert over a mock gh spawner", () => {
	...
	it.effect("no prior marker → POST a fresh verdict comment", () =>
		Effect.gen(function* () {
			...
			provide(effect, {
				...
				// the documented failure read-back: post re-fetches the landed comment and re-runs emissionDefect
	...
	// The the originating work item emission guard: a polarity-bearing (PASS/FAIL) body with an empty/malformed SHA is
	...
	// The the originating work item emission tightening: the observed leak substituted a full `mktemp` scratch path into
	...
	// The actual the originating pull request leak site: a §CP advisory (SHA-less first line, so it passes the polarity
...
// The the originating work item defense-in-depth self-verify: after post writes the comment it re-fetches the LANDED body
