 ...
 * the SHA-bound, one-verdict-per-gate contract SHA-bound gate verdicts over `gh api` REST, driving the IO-free
...
// SOURCE shared with the `Tracker` service and `epic-lock` — no longer re-copied here (the originating work item AC 5).
...
// module — the single-source classes now live in `../tracker/gh-io.ts` (the originating work item AC 5).
...
/**
 * The post self-verify (the originating work item): re-fetch the comment `post` just upserted and assert its LANDED body
 ...
 * empty-SHA `@-` marker the read side refuses, the originating work item); and every SHA field it carries — the
 * first-line `@ <sha>` and the §CP advisory `Reviewed-head:` anchor — must be a clean full 40-hex,
 * not a partial/non-hex/path-glued value (rejects the `mktemp`-path leak of the originating work item). An advisory
 ...
 * comment and re-runs `emissionDefect` on its LANDED body — the defense-in-depth self-verify (the originating work item)
...
/**
 * `Github` — the IO shell over `gh api` REST for the SHA-bound, one-verdict-per-gate contract verdict read/post glue. `read`
 ...
 * deferred to first use (`Effect.cached`, the repository-resolution contract §1): the layer build is side-effect-free, and
