...
describe("isBoundToHead — SHA-staleness prefix-match, fail-closed on empty", () => {
	...
	it("empty head is never current (the unguarded-glob bug, the SHA-bound, one-verdict-per-gate contract rule 3)", () =>
		assert.isFalse(isBoundToHead(HEAD, "")));
});

describe("resolveVerdict — the SHA-bound verdict decision (table-driven, the SHA-bound, one-verdict-per-gate contract rule 3)", () => {
	const cases: ReadonlyArray<{
		readonly name: string;
		...
		{
			name: "a forged PASS from a non-collaborator is dropped (author-gate, the runtime ACL authorization rule)",
			comments: [
				marker({
					id: 1,
					author: "attacker",
...
describe("malformedEmittedSha — the post full-40-hex emission guard (#2683)", () => {
	...
	it("flags a §CP advisory whose `Reviewed-head:` anchor is an mktemp path", () =>
		assert.isNotNull(
			malformedEmittedSha(
...
describe("emissionDefect — the one gate `post` and `validate` share (#2683/#2772/#2796)", () => {
	...
	// The the related failure modes recurrence: a /var/folders mktemp path in the @<sha> field, refused loudly.
	...
	// The the originating work item case: the whole body is a bare @filepath — its first line is not a marker.
