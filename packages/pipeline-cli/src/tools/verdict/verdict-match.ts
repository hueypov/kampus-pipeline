 ...
 * as one deterministic, table-tested function (the SHA-bound, one-verdict-per-gate contract, the SHA-bound verdict contract).
 *
 * The marker grammar, the emphasis-tolerant anchor, and the `@ <sha>` capture are
 * single-sourced from gh-issue-intake-formats.md §5/§6 and the SHA-bound, one-verdict-per-gate contract; this core is the
 * deterministic decision the IO shell (`github.ts`) drives at the boundary. The author-gate
 * (the runtime ACL authorization rule write+ trust root) is resolved by the shell and handed in as `authorizedAuthors`,
...
/** The four PR-layer gate namespaces (the SHA-bound, one-verdict-per-gate contract §Scope + the agent-definition control-plane boundary/0162). */
 ...
 * NOT match, exactly as `ship-it`'s capture refuses it (the originating work item).
 */
 ...
 * either side may be abbreviated (§the SHA-bound, one-verdict-per-gate contract rule 3). A `null`/empty bound SHA, or an empty
...
export interface ResolveVerdictInput {
	...
	/** The write+ collaborator logins — the runtime ACL authorization rule trust root (resolved by the IO shell). */
 ...
 * (the originating work item) — that the fail-closed read side then refuses (`sha-less`), false-BLOCKing a legitimate
 ...
 * abbreviated SHA (the SHA-bound, one-verdict-per-gate contract rule 3), but at EMISSION the full head SHA is always in hand, so an
 * emitted marker binds it exactly. Anything else is a defect the post guard refuses (the originating work item).
 */
...
/**
 * The post-emission SHA-shape guard (the originating work item). A verdict body about to be POSTed must bind the FULL
 * 40-hex head SHA in every SHA field it carries; a partial/non-hex/path-glued value (the observed
 * `mktemp` scratch path leaked into the `@ <sha>` field on §CP the originating pull request) both false-blocks ship-it
 ...
 * anchor line (the advisory reviewed-head format) — the latter is where the documented failure leak actually landed, invisible to the
...
export const malformedEmittedSha = (body: string, gate: VerdictGate): string | null => {
	...
	if (anyReviewedHeadLineRe.test(body) && !cleanReviewedHeadRe.test(body)) {
		return `the 'Reviewed-head: @ <sha>' anchor line is not a clean 40-hex head SHA — the §CP advisory head binding (the advisory reviewed-head format) leaked a non-hex/path value, the #2683 leak observed on the originating pull request`;
	}
	return null;
};
 ...
 *   1. cross-namespace — the first line must be *this* gate's marker (the SHA-bound, one-verdict-per-gate contract §Scope); this also
 *      refuses the whole-body bare `@filepath` case, whose first line is not a marker (the originating work item);
 *   2. unbound polarity — a PASS/FAIL first line must carry a bindable `@ <sha>` (the `@-` bug, the originating work item);
 *   3. malformed emitted SHA — every SHA field (first-line marker + `Reviewed-head:` anchor) must be
 *      a clean full 40-hex, never a partial/non-hex/path-glued value (the mktemp-path leak, the related failure modes);
 ...
 *      otherwise post a local path into a public comment (the related failure modes). The verdict body must inline
...
export const emissionDefect = (body: string, gate: VerdictGate): string | null => {
	if (!isNamespaceMarker(body, gate)) {
		return `the body's first line is not a ${GATE_KEYWORD[gate]}: marker — a verdict must carry its own gate's marker on line one (the cross-namespace emission bug, the SHA-bound, one-verdict-per-gate contract §Scope; also the whole-body bare @filepath case, #2796)`;
	}
	if (isUnboundPolarityMarker(body, gate)) {
		return `a ${GATE_KEYWORD[gate]}: PASS/FAIL verdict must carry a well-formed '@ <sha>' (≥7 hex) — this polarity-bearing body has an empty/malformed SHA, which posts an unbindable marker the fail-closed read side refuses (the '@-' emission bug, the SHA-bound, one-verdict-per-gate contract, #2646)`;
	}
