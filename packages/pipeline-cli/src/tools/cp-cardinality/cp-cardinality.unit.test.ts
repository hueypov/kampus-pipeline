...
describe("decideCpCardinality — the deterministic control-plane approval policy §CP team-cardinality branch (#2541)", () => {
	...
	describe("N == 2 (exactly-N, the non-author control-plane approval requirement two-person control) — unchanged", () => {
		...
		it("self-approval is EXCLUDED — a self-approval marker never discharges when N>=2 (the deterministic control-plane approval policy Banned)", () => {
			const v = decideCpCardinality(
				input({
