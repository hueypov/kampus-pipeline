/**
 * The `cp-cardinality` tool — `pipeline-cli cp-cardinality decide [flags]` (the originating work item).
 *
 ...
 * (the deterministic control-plane approval policy, enforcing decision the originating work item). Reads the active `@kamp-us/control-plane` member
 ...
 *
 * IO here (the thin bin); the whole the deterministic control-plane approval policy branch lives in `cp-cardinality.ts` (the pure,
...
const decideCmd = Command.make(
	...
	Effect.fn(function* ({author, nonAuthorApproval, selfApproval}) {
		...
		// Exit code mirrors the decision so the gate bash fails closed on `stop` (the deterministic control-plane approval policy).
		if (verdict.decision === "stop") return yield* Effect.sync(() => process.exit(1));
	}),
).pipe(
	Command.withDescription(
		"Decide §CP discharge deterministically from control-plane team cardinality and current-head approvals",
	),
);
...
export const cpCardinalityCommand = Command.make("cp-cardinality").pipe(
	...
	Command.withDescription(
		"Deterministic §CP team-cardinality discharge check ship-it's control-plane gate runs (the deterministic control-plane approval policy, #2541)",
	),
);
