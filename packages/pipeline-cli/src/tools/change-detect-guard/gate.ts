/**
 * The `change-detect-guard` filesystem gate (the originating work item) — the IO seam behind the ci.yml
 * API-free-git-mode check, split from `command.ts` so it is crossable in unit tests over a
 * fake repo dir rather than only by spawning the bin (the core-in-its-own-file idiom; the originating work item).
 *
 ...
 * missing job/step/`with:` — fail-closed, the zero-scope fail-closed invariant). A file that cannot be read is an
 ...
 * (the zero-scope fail-closed invariant).
 */
