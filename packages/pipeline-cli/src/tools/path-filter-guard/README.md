...
[the originating work item](https://github.com/example-org/example-repo/issues/2372); the invariant landed in issue
[the originating work item](https://github.com/example-org/example-repo/issues/2366) / PR
[the originating work item](https://github.com/example-org/example-repo/pull/2371)).
...
Fail closed on zero scope: a missing file, missing `changes` job or paths-filter step,
a missing `e2e:`/`deploy:` key, or an empty list is a FAILURE — a guard that extracted
nothing is broken, never a vacuous pass.
