...
// The live layer resolves its repo lazily (the repository-resolution contract §1); pin the env override so the
// fixtures keyed on `repos/example-org/example-repo/...` match without the ambient `gh repo view`.
...
const TARGET = 900;
const P = `repos/example-org/example-repo`;
...
describe("Tracker.claim — the authenticated, session-stamped claim protocol claim over a mock gh spawner", () => {
...
describe("Tracker.postVerdict — the SHA-bound, one-verdict-per-gate contract verdict/comment-post + read-back envelope", () => {
	// A full 40-hex head SHA — the ONLY shape the tightened emission guard (the originating work item) accepts on a
		...
		() =>
			Effect.gen(function* () {
				...
				provide(effect, {
					...
					// the documented failure read-back: postVerdict re-fetches the landed comment and re-runs emissionDefect
	...
	it.effect("files a needs-triage issue by default → created with its ref + url", () =>
		Effect.gen(function* () {
			...
			assert.deepStrictEqual(result, {
				_tag: "created",
				target: 4242,
				url: "https://github.com/example-org/example-repo/issues/4242",
			});
		}).pipe((effect) =>
			provide(effect, {
				[`POST ${P}/issues`]: JSON.stringify({
					number: 4242,
					html_url: "https://github.com/example-org/example-repo/issues/4242",
				}),
			}),
		),
	);
	...
	it.effect("honors an explicit --stage lifecycle stage", () =>
		Effect.gen(function* () {
			...
			assert.deepStrictEqual(result, {
				_tag: "created",
				target: 77,
				url: "https://github.com/example-org/example-repo/issues/77",
			});
		}).pipe((effect) =>
			provide(effect, {
				[`POST ${P}/issues`]: JSON.stringify({
					number: 77,
					html_url: "https://github.com/example-org/example-repo/issues/77",
				}),
			}),
		),
	);
...
describe("Tracker.graduate — the map/investigation graduation-close envelope", () => {
		...
		() =>
			Effect.gen(function* () {
				...
				const result = yield* tracker.graduate(TARGET, {
					artifact: "the originating initiative (planned by plan-epic)",
	...
	it.effect("composes `Graduated into <artifact>` with no note when none is given", () =>
		Effect.gen(function* () {
			const tracker = yield* Tracker;
			const result = yield* tracker.graduate(TARGET, {artifact: "#1, #2 → triage; the navigation placement law"});
			assert.deepStrictEqual(result, {
				_tag: "graduated",
				source: TARGET,
				artifact: "#1, #2 → triage; the navigation placement law",
				state: "closed",
			});
		}).pipe((effect) =>
			provide(effect, {
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				const tracker = yield* Tracker;
				const error = yield* Effect.flip(tracker.graduate(TARGET, {artifact: "the originating initiative"}));
				assert.isTrue(error instanceof TrackerVerifyError);
			}).pipe((effect) =>
				provide(effect, {
