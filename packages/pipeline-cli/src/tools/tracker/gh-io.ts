 ...
 * `epic-lock` lock, and the `verdict` gate each used to re-copy (the originating work item AC 5, the same-commit
 ...
 * repo resolution, the runtime ACL authorization rule write+ ACL gate, the raw comment shape), NOT the claim/verdict
 ...
 * install can't accidentally operate on example-repo.
 */
...
// The read-back GET for a post's self-verify (the originating work item): re-fetch the single comment we just upserted
...
/**
 * The write+ collaborator subset of `logins` — the runtime ACL authorization rule trust root. Each login is probed with
