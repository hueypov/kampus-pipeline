 ...
 *
 * Grown from the Wave-1 walking skeleton (the originating initiative): a `Context.Service` `Tracker`
 * tag plus its sole implementation `GithubTrackerLive`. `claim` (the authenticated, session-stamped claim protocol
 * agent-distinguishable claim), a generic `readBack`, `applyTriage` (the label-transition
 * envelope, the originating work item), `createIssue` / `createComment` (the issue/comment create envelope,
 * the originating work item), `postVerdict` (the SHA-bound, one-verdict-per-gate contract verdict/comment-post + read-back envelope, the originating work item),
 * and `graduate` (the map/investigation graduation-close envelope, the originating work item) are all live —
 * every Phase-3 verb has landed.
 *
 * Two design rules bind every signature here and are the point of the skeleton:
 *
 *  - **Design against two backends, build one (the portable tracker-interface rule).** `GithubTrackerLive` is the
 *    only implementation now, but the interface must stay implementable over the two
 *    downstream the originating work item targets *without a reshape*: **Asana** (no sub-issues, no
 ...
 *    owner (not a raw comment id). The GitHub ACL trust root (the runtime ACL authorization rule) lives entirely
 *    inside the live impl, never in the interface — local-markdown has none.
 *  - **Typed failures, never a throw.** Every infrastructure fault is in the `E` channel:
 *    a non-zero `gh` exit is `GhCommandError`, malformed output is `GhParseError`, an
 *    unresolvable repo is `RepoResolutionError`, and untrusted REST JSON is decoded at the boundary.
 *
 ...
 * follow-up (the originating work item AC 5). The pure claim-resolution decision is reused from the existing
 * IO-free, the test-tier taxonomy-tested core (`../epic-lock/claim-resolution.ts`) rather than
...
// adoption-lint (the originating work item) forbids the corpus from hand-copying.
import {
	emissionDefect,
	GATE_KEYWORD,
	GATES,
...
// per consumer (the originating work item AC 5). The error types are re-exported below so callers/tests keep importing
...
// service module — the single-source classes now live in `gh-io.ts` (the originating work item AC 5).
 ...
 * domain number so no backend's addressing leaks into a signature (the portable tracker-interface rule): GitHub reads
...
/** The agent-distinguishable claim decision (the authenticated, session-stamped claim protocol): who is claiming (the session id). */
...
// complete before it locks (the portable tracker-interface rule). Each stays domain-shaped — no label string, no
// sub-issue id, no REST field — so a later Asana/local-markdown adapter needs no reshape.

/**
 * Triage classification (judgment-as-parameter, the originating work item): the domain type + priority a
 ...
 * boundary, so no GitHub label string leaks into the signature (the portable tracker-interface rule).
 */
...
/**
 * A create-issue judgment (the originating work item): the `title` + `body` of the new entity, and the
 ...
 * so no label string or REST field leaks into the signature (the portable tracker-interface rule). The judgment
 ...
 * the originating work item names), so no GitHub marker string leaks into the signature. `GithubTrackerLive` composes
 ...
 * gate/passed/headRef it now carries. A domain result, never a raw REST comment id (the portable tracker-interface rule).
 */
...
/**
 * A graduation-close judgment (the originating work item): what the source graduated into (`artifact`, a domain
 ...
 * close that actually landed). A domain result, never a raw REST issue payload (the portable tracker-interface rule).
 */
...
// Close an issue as *completed* (the graduation-close, the originating work item) — distinct from triage's
...
/**
 * Claim `target` under `session` (the authenticated, session-stamped claim protocol). Rule-0 defer first: if an authorized claim
...
/**
 * Apply a triage classification to `target` (the portable tracker-interface rule) — the label-transition envelope
...
/** Map a domain gate name to the SHA-bound, one-verdict-per-gate contract `VerdictGate`, or `null` if it is not a known gate. */
...
/**
 * Compose the verdict comment body from the domain judgment: the SHA-bound, one-verdict-per-gate contract SHA-bound marker on line
...
/**
 * The post-write self-verify (the originating work item): re-fetch the comment `postVerdict` just upserted and assert
...
/**
 * Upsert `target`'s `gate` verdict and read it back (the SHA-bound, one-verdict-per-gate contract rule 2, the portable tracker-interface rule) — the
...
/** Add a comment (note) to `target` (the portable tracker-interface rule); the created note's ref is decoded back. */
 ...
 * optional closing note. Owning this composition is what lets the adoption-lint (the originating work item) forbid the
...
/**
 * Graduate `target` into its durable artifact(s) and close it as completed (the portable tracker-interface rule, the originating work item) — the
 ...
 * `TrackerVerifyError` rather than reporting a false graduation (the originating work item).
 */
 ...
 * map/investigation graduation-close, the originating work item). Built by `GithubTrackerLive`, whose `R` is
 * `ChildProcessSpawner`.
 */
 ...
 * carry `R = never`. Repo resolution is deferred to first use (`Effect.cached`, the repository-resolution contract
