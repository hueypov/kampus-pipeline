 ...
 *
 * The CLI surface of the Wave-1 `Tracker` service (the portable tracker-interface rule): `claim` posts the authenticated, session-stamped claim protocol
 ...
 * and drops the entity out of the needs-triage queue (the originating work item); `create-issue` files a new
 * issue and `create-comment` adds a note (the originating work item); `post-verdict` upserts a SHA-bound
 * the SHA-bound, one-verdict-per-gate contract gate verdict comment and reads it back (the originating work item); `graduate` posts the
 * source → artifact provenance record and closes the source as completed (the originating work item).
 ...
 * local-path leak — the documented failure hazard class is gone, not hand-guarded).
 *
 * `GithubTrackerLive` is baked in with `Command.provide(...)` so the registered command's
 * residual requirement is the Node platform union (the registry seam, the originating initiative).
 */
...
// Judgment-as-parameter (the originating work item): the classification is supplied, not decided by the tool.
...
// Judgment-as-parameter (the originating work item): the verdict decision (gate / PASS|FAIL / bound head / prose body)
// is supplied, never decided by the tool — the role vocabulary (which reviewer) lives in the agent
// def, not here. The verb composes the SHA-bound, one-verdict-per-gate contract `review-<gate>:` marker from these parameters so no
...
const headRefFlag = Flag.string("head").pipe(
	Flag.withDescription("the full 40-hex head SHA the verdict binds to (the SHA-bound, one-verdict-per-gate contract)"),
);
...
const postVerdict = Command.make(
	...
	Command.withDescription(
		"Upsert a SHA-bound gate verdict comment for a tracker entity and read it back (PATCH own prior marker, else POST — one per gate, the SHA-bound, one-verdict-per-gate contract)",
	),
);

// Judgment-as-parameter (the originating work item): what the source graduated into (`--artifact`) and an optional
...
export const trackerCommand = Command.make("tracker").pipe(
	...
	Command.withDescription(
		"The crew tracker service (the portable tracker-interface rule): claim a tracker entity, read back its owner, apply a triage classification, create an issue or a comment, post a gate verdict, graduate a source into its artifact",
	),
