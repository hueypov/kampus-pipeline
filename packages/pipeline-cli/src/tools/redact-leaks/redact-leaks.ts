 ...
 * machine-local path leak while preserving its evidential shape (the originating work item).
 *
 * Detection is NOT re-implemented here: `redactLeaks` consumes `findCommentLeaks`
 * (../leak-guard/leak-guard.ts) as the single leak-pattern source (the originating work item AC3) and only
 ...
 * itself a leak (the originating work item-class).
 */
 ...
 * text is returned byte-for-byte unchanged (the originating work item AC5). Longest matches are replaced first
