 ...
 * leak-guard matcher (`findCommentLeaks`); this tool only masks (the originating work item). Triage's Step-4
 ...
 * (not `Console.log`) so a leak-free body is emitted byte-for-byte, no appended newline (the originating work item AC5).
 */
...
export const redactLeaksCommand = Command.make(
	...
	Command.withDescription(
		"Redact machine-local path leaks in a body, preserving evidential shape (the originating work item)",
	),
);
