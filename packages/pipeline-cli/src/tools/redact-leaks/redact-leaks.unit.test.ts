...
describe("redactLeaks — REDACT matched leaks, preserving evidential shape (#3021 AC2)", () => {
	...
	it("redacts an absolute /Users/<name> home path, keeping the /Users root", () => {
		const out = redactLeaks("see /Users/umut/code/example-repo/apps/web for the seam");
		assert.strictEqual(out, "see /Users/<redacted>/code/example-repo/apps/web for the seam");
		assert.isTrue(isClean(out));
	});
	...
	it("redacts a ~/.claude private-home internal down to ~, dropping the tool name", () => {
		...
		// config leaves were exempted by shape (the originating work item); ~/.claude/projects/... is not one.
