...
const pull = (over: Partial<PullHeadPayload> = {}): PullHeadPayload => ({
	number: 42,
	state: "open",
	head: {sha: SHA, ref: "usirin/feature", repoFullName: "example-org/example-repo"},
	baseRepoFullName: "example-org/example-repo",
	...over,
});
...
describe("resolveHead — the latest head from the REST payload", () => {
	...
	it("takes REST's `.head.sha` verbatim as the latest head — no history to walk", () => {
		...
		const r = asResolved(
			resolveHead(pull({head: {sha: newer, ref: "b", repoFullName: "example-org/example-repo"}})),
		);
	...
	it("flags a cross-fork head (head repo != base repo) while still resolving it", () => {
		const r = asResolved(
			resolveHead(pull({head: {sha: SHA, ref: "patch-1", repoFullName: "someone/example-repo"}})),
		);
...
describe("resolveHead — missing-PR fail-safe", () => {
	...
	it("is unresolvable when the head SHA is null", () => {
		const r = resolveHead(pull({head: {sha: null, ref: "x", repoFullName: "example-org/example-repo"}}));
		assert.strictEqual(r._tag, "unresolvable");
	});

	it("is unresolvable when the head SHA is not a full 40-hex commit", () => {
		const r = resolveHead(
			pull({head: {sha: "0123abc", ref: "x", repoFullName: "example-org/example-repo"}}),
		);
		assert.strictEqual(r._tag, "unresolvable");
