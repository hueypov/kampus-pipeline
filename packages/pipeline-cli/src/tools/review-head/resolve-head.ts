 ...
 * (the originating work item / the originating work item). It answers two things from the untrusted `repos/:repo/pulls/:pr`
 ...
 * materialize (and bind a verdict to) a head it cannot name (the SHA-bound, one-verdict-per-gate contract §5).
 */
export const resolveHead = (pull: PullHeadPayload): HeadResolution => {
	...
	if (!FULL_SHA_RE.test(headSha)) {
		return {
			_tag: "unresolvable",
			reason: `PR #${pull.number} head SHA '${pull.head.sha}' is not a full 40-hex commit — refusing to bind a verdict to a partial/malformed head (the SHA-bound, one-verdict-per-gate contract §5)`,
		};
	}
	return {
		_tag: "resolved",
