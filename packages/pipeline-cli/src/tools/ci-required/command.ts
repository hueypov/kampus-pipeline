 ...
 *
 * The CI aggregator assertion for the originating work item (the zero-scope fail-closed invariant), moved into the
 * pipeline-cli registry (the originating initiative, Phase 2 / the originating work item). Reads the gating jobs'
 ...
 * prints the per-job verdicts (the zero-scope fail-closed invariant §1 "emit what you scanned"), and exits 0
 * on PASS / 1 on FAIL.
 *
 ...
 * Effect `Command`. The pure core lives alongside this command, so this CLI owns
 * its behavior without loading another workspace package. `command.ts` is the IO
 * shell: read env, print, exit.
 */
...
export const ciRequiredCommand = Command.make(
	...
	Command.withDescription(
		"Require should-have-run gating jobs to have passed; fail-closed on a silent skip (#786, the zero-scope fail-closed invariant)",
	),
);
