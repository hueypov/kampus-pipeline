/**
 * Pure-core tests for `campaign verify-trace` (the originating work item) — the fail-closed founder-approval-trace
 ...
 * input fails closed (the zero-scope fail-closed invariant). The four fail-closed paths the acceptance criteria enumerate —
...
describe("verifyTrace — fails closed on ZERO SCOPE (the zero-scope fail-closed invariant)", () => {
