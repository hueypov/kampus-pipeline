 ...
 *
 * The fail-closed founder-approval-trace verifier (the originating work item). The campaign skill is
 ...
 * absence, malformation, a non-founder author, or zero scope (the zero-scope fail-closed invariant). The IO-free decision
		...
		if (founderLogin === "") {
			return yield* fail(
				"campaign verify-trace: FAIL (zero-scope) — no founder identity configured " +
					...
					"authorization anchor rather than fall back to any implicit login (the zero-scope fail-closed invariant).",
			);
		}
