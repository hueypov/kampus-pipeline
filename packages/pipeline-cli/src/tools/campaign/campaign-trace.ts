 ...
 * human OR an agent may run it (the originating work item, story 2) — so there is no human-at-keyboard guard
 ...
 * marker; absence, malformation, a non-founder author, and zero scope each fail closed (the zero-scope fail-closed invariant).
 *
 ...
 * evidence for *why* it failed. Every non-PASS is a fail-closed refusal (the zero-scope fail-closed invariant).
 */
...
export const verifyTrace = (input: VerifyTraceInput): TraceVerdict => {
	...
	if (input.clusterSize <= 0) {
		return {
			pass: false,
			reason: "zero-scope",
			detail: `the wave label '${waveLabel}' names zero issues — an empty cluster cannot be approved (the zero-scope fail-closed invariant)`,
		};
	}
...
/** Render the human-readable report for a verdict (the zero-scope fail-closed invariant §1 "emit what you scanned/decided"). */
