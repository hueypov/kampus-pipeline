 ...
 * one central curated-hybrid index (the originating work item, the originating initiative, the descriptive/normative design firewall). IO-free and
 ...
 *
 * The descriptive/normative firewall (the descriptive/normative design firewall) is enforced in this core, not merely
 ...
 *
 * Fail-closed on zero scope (the zero-scope fail-closed invariant): zero annotated primitives discovered is a broken
 ...
 * empty list a caller could misread as "clean, nothing to document" (the zero-scope fail-closed invariant).
 */
 ...
 * a TS AST because the metadata schema (the descriptive/normative design firewall) is a lean tag vocabulary carried in the
 ...
 * order — the property a drift guard (the originating work item) depends on. Fails closed on zero primitives.
 */
...
/**
 * The firewall predicate (the descriptive/normative design firewall): the ONLY sanctioned write target is the descriptive
...
/**
 * Render the whole descriptive inventory to markdown. It inlines each component's
 * selection guidance and links to source for depth. The header states the firewall so a
 * reader (human or agent) knows this file is
 * descriptive-only and machine-generated. No timestamp: the output must be byte-stable
 * for a given source tree so a drift guard (the originating work item) reds only on real drift.
 */
export const renderInventory = (entries: ReadonlyArray<ComponentEntry>): string => {
	const header = [
		...
		"",
		"The descriptive half of the design docs: which `components/ui` primitives",
