/**
 * Pure-core tests for `design-inventory` (the originating work item, the descriptive/normative design firewall): parsing the `@component`
 * JSDoc schema off fixture source into the structured inventory, the deterministic sort,
 * the fail-closed-on-zero verdict (the zero-scope fail-closed invariant), the firewall predicate, and the render.
...
// A fixture primitive mirroring the shipped Button.tsx annotation shape (the descriptive/normative design firewall schema).
...
describe("buildInventory — deterministic sort + fail-closed on zero scope", () => {
	...
	it("FAILS zero-scope (fail-closed, the zero-scope fail-closed invariant) when no annotated primitive is found", () => {
...
describe("isDescriptiveWriteTarget — the descriptive/normative firewall (the descriptive/normative design firewall)", () => {
