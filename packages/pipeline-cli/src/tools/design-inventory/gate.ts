/**
 * The `design-inventory` filesystem gate — the IO seam behind the originating work item's descriptive
 * component-inventory extractor (the originating initiative, the descriptive/normative design firewall), split from `command.ts` so it is
 ...
 *     but the descriptive inventory (the descriptive/normative design firewall) — the manifest can never be written here.
 *
 * Fail-closed on zero scope (the zero-scope fail-closed invariant): zero annotated primitives is a `CheckFailed`, not a
 ...
 * forbids (the descriptive/normative design firewall). Unreachable on the normal path — the gate only ever writes the
...
/** The primitives directory the inventory is extracted from (the descriptive/normative design firewall / the originating initiative). */
...
/**
 * Write `content` to a repo-relative target — but only if the firewall admits it (the descriptive/normative design firewall).
 ...
 * on drift (the self-updating loop's freshness signal, wired into CI by the originating work item); `--stdout`
			...
			return yield* Effect.fail(
				new CheckFailed({
					reason:
						...
						"fail-closed (the zero-scope fail-closed invariant). Is the repo root correct, or did the @component convention drop?",
				}),
			);
		}
