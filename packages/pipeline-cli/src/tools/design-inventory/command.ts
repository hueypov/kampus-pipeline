 ...
 * [--root <d>] [--stdout] [--check]` (the originating work item, the originating initiative, the descriptive/normative design firewall).
 *
 ...
 *   pipeline-cli design-inventory check               # the CI guard: red on a stale inventory (the originating work item)
 *
 * FIREWALL (the descriptive/normative design firewall): the tool writes ONLY the descriptive inventory artifact; the
...
// design-token-guard) the CI job invokes — the self-update freshness rung of the originating work item. It reds
...
// scan lives once. The firewall half of the originating work item is a git-diff belt in the workflow (a mutation
...
export const designInventoryCommand = Command.make("design-inventory").pipe(
	...
	Command.withDescription(
		"Self-updating descriptive component inventory from components/ui JSDoc (the originating work item, the descriptive/normative design firewall)",
	),
);
