/**
 * The `patch-guard` tool — `pipeline-cli patch-guard check [--root <d>]` (the local-patch reproducibility rule).
 *
 ...
 * zero patchedDependencies in scope (fail-closed, the zero-scope fail-closed invariant). The marker grammar + the
 * two-layer discipline (version-keyed loud-fail + behavior pin) is enforced here. The scan/IO lives in `gate.ts`; this
...
export const patchGuardCommand = Command.make("patch-guard").pipe(
	...
	Command.withDescription(
		"Fail-closed gate: every pnpm patch (the local-patch reproducibility rule) carries a registered behavior-pinning test (#3051)",
	),
);
