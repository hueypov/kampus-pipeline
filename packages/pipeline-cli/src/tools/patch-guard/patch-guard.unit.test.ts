/**
 * `patch-guard` pure-core tests (the originating work item) — the verdict logic over already-gathered
 * facts: every patched dep with a matching pin passes, an unpinned patch fails, a
 * stale marker fails, and an empty patch set fails closed (the zero-scope fail-closed invariant). No disk — the IO
