/**
 * `patch-guard` pure core — decide whether every maintained `pnpm patch` (the local-patch reproducibility rule)
 ...
 *
 * The guard uses two independent protections: pnpm's version-keyed patch declaration and a
 * behavior-pinning test. A `pnpm patch` is a silent fork of a dependency's
 ...
 *
 * Fail-closed on zero scope (the zero-scope fail-closed invariant): if no `patchedDependencies` are found it is a
...
export type PatchGuardVerdict =
	...
	/** No `patchedDependencies` in scope — fail closed, never a vacuous pass (the zero-scope fail-closed invariant). */
...
// The registration marker grammar is a comment carrying `@patch-pin: <name>@<version>`.
// A test file scanned by this guard must not embed the contiguous tag as a stray marker.
 ...
 * on zero patches (the zero-scope fail-closed invariant), else reds on any patched dep with no matching pin
...
export const renderReport = (verdict: PatchGuardVerdict): string => {
	...
	if (verdict.reason === "zero-scope") {
		return (
			"patch-guard: found ZERO patchedDependencies in pnpm-workspace.yaml — fail-closed (the zero-scope fail-closed invariant). " +
	...
	for (const dep of verdict.missing) {
		lines.push(
			`  ${dep.key}: patched but has NO \`// @patch-pin: ${dep.key}\` marker — ` +
				"a pnpm patch (the local-patch reproducibility rule) with no behavior-pinning test is an unverified fork. " +
				`Fix: add a test that fails if the patched behavior regresses and tag it \`// @patch-pin: ${dep.key}\`.`,
		);
	}
