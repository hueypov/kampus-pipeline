/**
 * The `patch-guard` filesystem gate (the local-patch reproducibility rule forcing function) — the IO seam behind
 ...
 * by spawning the bin (the core-in-its-own-file idiom; the originating work item).
 *
 ...
 * scope (fail-closed, the zero-scope fail-closed invariant). A directory/file IO failure is an `IoError` (also
 ...
 * patchedDependencies in scope (the zero-scope fail-closed invariant).
 */
