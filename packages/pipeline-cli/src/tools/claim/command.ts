 ...
 *
 * The issue-scoped "is this claim mine?" resolver (the originating work item): resolve the earliest
 * authorized claim on an arbitrary issue (the authenticated, session-stamped claim protocol §2 / gh-issue-intake-formats.md
 ...
 * toward the expensive-but-correct path (the documented failure license) rather than parsing
...
export const claimCommand = Command.make("claim").pipe(
	...
	Command.withDescription(
		"Resolve issue-claim ownership (the authenticated, session-stamped claim protocol earliest-authorized-claim) — the default-deny 'is this claim mine' verb",
	),
