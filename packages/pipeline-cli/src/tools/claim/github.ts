 ...
 * (the template child): REST only (GraphQL is broken on the host organization). Every
 * infrastructure failure is a typed error in the `E` channel: a non-zero `gh` exit is `GhCommandError`, malformed output is `GhParseError`, an
 ...
 * comments, resolves the write+ authorized-author set (the runtime ACL authorization rule), and resolves the
...
/**
 * The write+ collaborator subset of `logins` — the runtime ACL authorization rule trust root. Each login is
