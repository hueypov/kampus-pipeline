 ...
 *
 * The classifier for ship-it Step 5.5's bounded post-enqueue reconcile (the originating work item).
 ...
 *
 * The IO lives in `github.ts` (the `Github` service — the `gh` REST boundary, the repository-resolution contract repo
 ...
 *
 * Fail-closed away from a false ship (preserved through the documented failure idiom remediation): an
