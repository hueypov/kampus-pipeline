...
// Spawn the handle directly (not `ChildProcessSpawner.string`, which hides the exit code), lower a non-zero exit
...
/**
 * The env half of repo resolution (the repository-resolution contract §1), read via `Config` rather than `process.env`
 * (the originating work item): `CLAUDE_PIPELINE_REPO`, else `GITHUB_REPOSITORY` (CI). `Config.option` maps an unset
 ...
 * `Unknown JSON field` on this gh/repo, the originating work item); `merged` is derived from `state == MERGED`.
 */
 ...
 * false-ship posture (the originating work item): a missing event reads as the enqueue-settle window (`pending`),
 ...
 * `repoOverride` (the CLI `--repo` flag, the repository-resolution contract §1's highest precedence) wins over the resolved
 ...
 * is deferred to first use (`Effect.cached`, the repository-resolution contract §1): the layer build is side-effect-free,
