/**
 * Unit tests for the `intake-dedup` pure core (the unified deterministic intake-dedup rule): tokenization, query building,
...
describe("searchQuery", () => {
	it("prepends the repo/is:issue/is:open qualifiers, space-joined", () => {
		expect(searchQuery("example-org/example-repo", ["retry", "abort"])).toBe(
			"repo:example-org/example-repo is:issue is:open retry abort",
		);
	});

	it("emits qualifiers-only when there are no tokens (no trailing raw space)", () => {
		expect(searchQuery("example-org/example-repo", [])).toBe("repo:example-org/example-repo is:issue is:open");
	});
});
...
describe("rankCandidates", () => {
	...
	// the originating work item — a Turkish-titled queue duplicate must surface. Before the fix its title tokenized
