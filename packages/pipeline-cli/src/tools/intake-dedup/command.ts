 ...
 *
 * The unified deterministic "is there already an open issue for this?" check, extracted from the inline
 ...
 * likely duplicate). It is advisory, not an oracle (the unified deterministic intake-dedup rule; a duplicate is cheap to close, a
 ...
 * command's residual requirement is the Node platform union (the registry seam, the originating initiative).
 */
...
const check = Command.make(
	...
	Command.withDescription(
		"List open issues that may already cover an observation (the unified deterministic intake-dedup rule pre-file / intake dedup check)",
	),
);
...
export const intakeDedupCommand = Command.make("intake-dedup").pipe(
	...
	Command.withDescription(
		"Unified intake-dedup check — one tested query shared by the report + triage seams",
	),
