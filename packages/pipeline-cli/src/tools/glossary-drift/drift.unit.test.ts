...
const TERMS_FIXTURE = `# example-repo domain vocabulary (TERMS)
...
describe("extractCandidates", () => {
	...
	it("drops docs(decisions) merges — the ADR-routed surface prong (c) covers", () => {
		const line: MergeLine = {subject: "docs(decisions): the vocabulary capture-and-backstop rule glossary triggers (#1745)"};
		assert.deepStrictEqual(extractCandidates(line), []);
	});
});
