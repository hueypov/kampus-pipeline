 ...
 *
 * The out-of-band backstop of the vocabulary capture-and-backstop rule prong (b) (the originating work item): it diffs recent
 ...
 * regular code PR that never routes through `/adr` or `plan-epic` — the documented failure
 ...
 * the deliberate the vocabulary capture-and-backstop rule price of keeping this off the fail-closed gate.
 *
...
const sweep = Command.make(
	...
	Effect.fn(function* ({window: windowOpt, terms: termsOpt, fileIssue}) {
		...
		yield* Effect.gen(function* () {
			...
			// Exit 0 whether or not drift was found — this NEVER blocks a merge (the vocabulary capture-and-backstop rule).
		}).pipe(Effect.catchTag("SweepIoError", onIoError));
	}),
).pipe(
	Command.withDescription(
		"Sweep recent merges for concept-level vocab drift vs .glossary/TERMS.md (out-of-band, the vocabulary capture-and-backstop rule (b))",
	),
);
...
export const glossaryDriftCommand = Command.make("glossary-drift").pipe(
	...
	Command.withDescription(
		"Out-of-band glossary-drift backstop: diff recent merges vs .glossary/TERMS.md (the vocabulary capture-and-backstop rule prong (b), #1748)",
	),
);
