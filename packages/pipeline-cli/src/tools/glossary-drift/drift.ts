 ...
 * file them as candidate vocabulary drift (the vocabulary capture-and-backstop rule prong (b), the originating work item).
 *
 ...
 * that never routes through `/adr` or `plan-epic`** (the grounded miss the originating work item — the
 ...
 *   2. strip trailing issue/PR backlinks (`(the originating work item)`),
 ...
 * borderline phrase over missing a real coinage, because the vocabulary capture-and-backstop rule makes a sweep hit a
 ...
 * over-pruning would cost recall, which the vocabulary capture-and-backstop rule protects (a miss is worse than a noisy
 ...
 * ("true kill switch") still surfaces. Recall-biased on purpose (the vocabulary capture-and-backstop rule: a false
 ...
 * from "the sweep didn't run" (the zero-scope fail-closed invariant §1 — emit what you scanned).
 */
...
export const renderIssueBody = (
	...
	return [
		"## What I was doing",
		`The periodic glossary-drift sweep (the vocabulary capture-and-backstop rule prong (b), \`pipeline-cli glossary-drift\`) ran over the last ${windowSize} merges to \`main\`.`,
		"",
		"## What I observed",
		...
		"",
		"## Why it matters",
		"A concept that ships un-glossaried drifts: the same idea gets named three different ways across the codebase (the one-concept-named-four-ways drift, #851 / the triageable-audit-finding rule). Each surfaced phrase is either a real term to add to `.glossary/TERMS.md`, or a false positive to dismiss — triage's call.",
		"",
		"## Pointers",
