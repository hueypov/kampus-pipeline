 ...
 * [--root <d>] [--write-baseline]` (the originating work item, the four-pillar design token rule).
 *
 ...
 * or zero CSS files in scope (fail-closed, the zero-scope fail-closed invariant). The scan/IO lives in `gate.ts`;
...
export const designTokenGuardCommand = Command.make("design-token-guard").pipe(
	...
	Command.withDescription(
		"Fail-closed gate: CSS consumes role tokens only — no raw hex/px, no undefined token ref (the originating work item, the four-pillar design token rule)",
	),
);
