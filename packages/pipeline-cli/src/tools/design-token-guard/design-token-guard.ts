/**
 * `design-token-guard` pure core (the originating work item, the four-pillar design token rule / design-system-manifest.md)
 ...
 *      the Toast `var(--surface-1)`/`var(--text)` dead-ref class (the originating work item).
 ...
 *      4px grid sanctions 1px & 2px for hairlines/nudges, the four-pillar design token rule value #1) does not
 ...
 * on zero scope (the zero-scope fail-closed invariant): zero CSS files discovered is a broken scope assumption, a
...
export type DesignTokenGuardVerdict =
	...
	/** No CSS files discovered — fail closed, never a vacuous pass (the zero-scope fail-closed invariant). */
...
export const renderReport = (verdict: DesignTokenGuardVerdict): string => {
	...
	if (verdict.reason === "zero-scope") {
		return (
			...
			"(the zero-scope fail-closed invariant). Is the repo root correct, or did the styles layout change?"
		);
	}
	...
	return (
		"design-token-guard: the design-token seam is broken (the originating work item, the four-pillar design token rule):\n" +
		`${sections.join("\n\n")}\n\n` +
 ...
 * an identifier/hex; `1px` and `2px` are the sanctioned grid exceptions (the four-pillar design token rule
