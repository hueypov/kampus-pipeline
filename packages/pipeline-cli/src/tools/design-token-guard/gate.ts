/**
 * The `design-token-guard` filesystem gate (the originating work item) — the IO seam behind the
 ...
 * idiom; the originating work item).
 *
 ...
 * ref / raw hex / raw-px regression, or on zero CSS files in scope. A missing or empty
 * scope is a failure, never a passing no-op. A directory/file IO failure is an `IoError`
 * (also non-zero — both failures,
...
const readConfig = (root: string): Effect.Effect<DesignTokenConfig, IoError | CheckFailed> =>
	Effect.gen(function* () {
		...
		if (
			...
			// pass — fail closed (the zero-scope fail-closed invariant).
			return yield* Effect.fail(
				new CheckFailed({
					reason: `design-token-guard: ${CONFIG_PATH} is missing one of externalProperties / grandfatheredMissingTokens / rawPxCeilings — the allow-list is broken, fail-closed (the zero-scope fail-closed invariant).`,
				}),
			);
		}
		return {
 ...
 * closed on zero CSS files in scope (the zero-scope fail-closed invariant).
 */
