...
[the originating work item](https://github.com/example-org/example-repo/issues/2170), the design-token rule: UI code uses declared semantic tokens rather than raw visual values /
...
for the class of defect [the originating work item](https://github.com/example-org/example-repo/issues/2167) fixed
...
2. **raw-hex** — a component CSS file carries no hex color literal. Hex lives **only**
   ...
   Issue-ref comments like `/* the originating work item */` are stripped before matching, so only real hex
   colors count.
3. **raw-px ratchet** — a file's count of raw `px` values `> 2px` must not exceed its
   ...
   nudges — the four-pillar design token rule value #1); everything else lands on the `--s-N` spacing ramp.
   `@media`/`@container`/`@supports` breakpoint values are excluded.

Fail closed on zero CSS files or a malformed config: a missing or empty scope is a
failure, never a passing no-op.
...
- `grandfatheredMissingTokens` — pre-existing dead refs, grandfathered **by name** (not
  ...
  ([the originating work item](https://github.com/example-org/example-repo/issues/2163)/[the originating work item](https://github.com/example-org/example-repo/issues/2166))
