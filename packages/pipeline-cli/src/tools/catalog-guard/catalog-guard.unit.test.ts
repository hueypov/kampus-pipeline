/**
 * `catalog-guard` pure-core tests (the originating work item) — the verdict logic over already-gathered
 * manifest facts: a `catalog:` / `workspace:` ref passes, a hardcoded version fails,
 * an allowlisted exception passes, and an empty scope fails closed (the zero-scope fail-closed invariant). No
