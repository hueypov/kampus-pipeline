 ...
 * the originating work item): every dep in every workspace `package.json` must be sourced from the pnpm
 ...
 * the originating pull request frozen-lockfile break (`@distilled.cloud/cloudflare` hardcoded):
 *
 ...
 * today) — never a silent tolerance. Fail-closed on zero manifests (the zero-scope fail-closed invariant). The
...
// disk. It mirrors `findRootDir`'s pure upward walk (dirname to the fixpoint, then fall back to the start), but the marker check is the
