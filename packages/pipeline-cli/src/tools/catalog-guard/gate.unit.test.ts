/**
 * `checkCatalog` over a fake repo dir — the filesystem-seam test (the related failure modes). The
...
describe("checkCatalog — the CI exit-code gate over a fake repo dir", () => {
	it("SUCCEEDS when every dep across root + members is on catalog:/workspace:", async () => {
		writeWorkspace();
		writeRoot({name: "example-repo", devDependencies: {turbo: "catalog:"}});
		mkPackage("a", {
			name: "@kampus/a",
	...
	it("FAILS (CheckFailed) when the ROOT manifest pins a hardcoded version", async () => {
		writeWorkspace();
		writeRoot({name: "example-repo", devDependencies: {turbo: "^2.0.0"}});
