/**
 * The `catalog-guard` filesystem gate — the IO seam behind the originating work item's "every
 ...
 * manifests are in scope (fail-closed, the zero-scope fail-closed invariant). A directory/file IO failure is an
 ...
 * the real disk; a filesystem fault folds `PlatformError` → the `IoError` this gate already carries. This is the one why-note for the file's
 ...
 * `CheckFailed`. Fails closed on zero manifests in scope (the zero-scope fail-closed invariant).
 */
