 ...
 * ref), with no hardcoded version string (the originating work item). IO-free and total: every
 ...
 * the originating pull request failure mode (`@distilled.cloud/cloudflare` hardcoded). The convention was
 * written but unenforced, so it held only by reviewer vigilance; this guard closes
 * the gap deterministically.
 *
 * Fail-closed on zero scope (the zero-scope fail-closed invariant): if the scan finds zero manifests it is a
...
/**
 * An explicit, reasoned exception to the catalog rule (the originating work item: "a genuinely
 ...
 * exception, each with a recorded `reason`; a silent tolerance is a bug (the originating work item).
 */
...
export type CatalogGuardVerdict =
	...
	/** No manifest in scope — fail closed, never a vacuous pass (the zero-scope fail-closed invariant). */
 ...
 * (the zero-scope fail-closed invariant), else reds on any dep whose value is neither `catalog:` / `workspace:`
...
/** Render the human-readable failure report for a non-passing verdict (the zero-scope fail-closed invariant §1 "emit what you scanned"). */
export const renderReport = (verdict: CatalogGuardVerdict): string => {
	...
	if (verdict.reason === "zero-scope") {
		return (
			"catalog-guard: scanned ZERO package.json manifests — fail-closed (the zero-scope fail-closed invariant). " +
			"Is the repo root correct, or did the workspace shape change?"
		);
	}
	// Each line follows the `<what's wrong> — <why + issue ref>. Fix: <step>.` convention (the originating work item).
...
/** The manifest fields the catalog rule governs (the originating work item acceptance criteria). */
