...
describe("parseMapBody — decision entries", () => {
	...
	it("folds a wrapped item's continuation line, extracting its `— from #N` (#2426)", () => {
		// The the originating work item §worked-example map: a well-formed decision whose `— from #N`
...
describe("parseMapBody — the wrapped worked example validates clean (#2426)", () => {
	it("no MALFORMED_DECISION_ENTRY when refs sit on continuation lines", () => {
		// Grounded repro of the FAIL: parsing the verbatim the originating work item worked example, whose
...
describe("parseMapBody — CHART-time seed attribution `— from #<MAP>` (#3405)", () => {
	...
	// (`— from the originating work item` for map the originating work item). That form already parses to a resolvable origin,
...
describe("parseMapBody — fog entries", () => {
	...
	it("a fog line whose only ref is a spawned ref still names its subject", () => {
		// `the originating work item → spawned the originating work item`: the originating work item is the subject, the originating work item the follow-on.
