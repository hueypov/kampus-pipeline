/**
 * Unit tests for the `epic-splice` pure core (the originating work item): the epic-body splice extracted from
 * `plan-epic`'s inline Step-5 splice (the originating work item). IO-free — no `gh` boundary. The headline invariants:
...
// exactly what a naive reconstruct-from-memory splice would "tidy" and thereby clobber (the originating work item).
