 ...
 *     It **fails closed**: an off-allowlist model is a DENY, never a silent
 ...
 *     the originating work item). The inherit decision used to hinge on the `WORKFLOW_MODEL` env pin being
 *     allowlisted — but that pin is uncommitted and operator-shell-only, so a fresh
 *     clone / CI / cron / new operator without it re-hit the documented failure fail-closed-on-unset
 *     symptom. So an **absent** pin falls back to a committed `DEFAULT_PIN`: unset spawns
 *     resolve to allow-inherit durably, independent of the launching
 ...
 * (the originating work item). Only the Opus-4.8 family has an entry — `sonnet` / `haiku` are deliberately
 * absent, so they never canonicalize onto the allowlist and stay a fail-closed DENY
 * (the zero-scope fail-closed invariant). Mapping resolves an alias for the allowlist *check* only; the guard never
 * rewrites the request to the canonical id (the originating work item — the Task tool rejects the full id).
 */
...
/**
 * The committed, allowlisted pin an unset `WORKFLOW_MODEL` falls back to (the durable default model-pin rule).
 * Durable in source, so an unset spawn resolves to allow-inherit regardless of the
 * launching shell — the fix for the documented failure fragility where the inherit path depended on
...
export type SpawnDecision =
	...
	| {
			...
			// because the Task tool rejects the full pin id (the originating work item), so the pin can't override it.
 ...
 *
 * Rules (fail-closed, the zero-scope fail-closed invariant; durable default pin, the durable default model-pin rule). This core owns the full
 ...
 * `DEFAULT_PIN` — so an *absent* pin no longer fails closed (the documented failure fragility), but a
 ...
 *   rejects the full id `claude-opus-4-8[1m]`, the originating work item), so an unset request rides the
 ...
 *   pin exists (the pin can't override it, the originating work item; `explicitOffAllowlist` flags this), and a
...
export const decideSpawn = (
	...
	if (isOnAllowlist(effectivePin)) {
		...
		// model; an explicit off-allowlist request is denied — the pin can't rewrite it in (the originating work item).
		if (req === null) {
