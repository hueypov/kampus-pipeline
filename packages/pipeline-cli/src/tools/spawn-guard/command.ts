 ...
 *
 * The harness-config slice for the originating work item (the zero-scope fail-closed invariant), moved into the pipeline-cli
 * registry (the originating initiative, Phase 2 / the originating work item). Three harness-event surfaces:
 *
 ...
 *     absent pin falls back to the committed `DEFAULT_PIN`, the durable default model-pin rule), and renders the
 *     `decideSpawn` outcome as a permission decision: **allow** an allowlisted model,
 *     **allow** an unset request to inherit the session model (the related failure modes), or **deny**
 *     an off-allowlist request. Per the zero-scope fail-closed invariant it still fails closed on a *present* bad
 ...
 *   - `freshness`  — a SessionStart freshness check (the originating work item): when the hook-pack's
 ...
 * additionalContext contract. The package's the originating work item stale-tree shim (`bin.ts` /
...
const guard = Command.make(
	...
	Effect.fn(function* () {
		...
		switch (decision.kind) {
			...
			case "allow-inherit":
				...
				return;
			case "deny":
				// the zero-scope fail-closed invariant: fail closed. An off-allowlist or unset model is a DENY, and the
				// reason emits what the guard checked so the refusal is observable, not silent.
				// `explicitOffAllowlist` (an explicit bad model the pin can't override, the originating work item)
	...
	Command.withDescription(
		"PreToolUse spawn-model allowlist guard (Task/Workflow), fail-closed (the zero-scope fail-closed invariant)",
	),
);
...
/**
 * The proactive SessionStart freshness signal (the originating work item): when the hook-pack's runtime dep
...
const freshnessSignal = (dep: string = RUNTIME_DEP): string | null =>
	depsInstalled(dep)
		? null
		: `Hook deps not installed — run \`pnpm install\`. The example-repo hook-pack's runtime ` +
			...
			`are installed (the originating work item). Tell the user to run \`pnpm install\` from the repo root.`;
