...
describe("spawn-guard guard CLI — PreToolUse envelope", () => {
	...
	it("DENIES an off-family alias (`sonnet`) — fail-closed on the caller's vocabulary too (the zero-scope fail-closed invariant)", async () => {
		const {stdout} = await run(["guard"], envelope("sonnet"));
		const out = JSON.parse(stdout);
		assert.strictEqual(out.hookSpecificOutput.permissionDecision, "deny");
	}, 30_000);

	it("DENIES an off-allowlist model with no pin and emits what it checked (the zero-scope fail-closed invariant)", async () => {
