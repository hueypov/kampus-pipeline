...
describe("isOnAllowlist — only the opus-4.8 family", () => {
	...
	it("rejects off-family aliases — sonnet / haiku have no allowlist mapping (the zero-scope fail-closed invariant)", () => {
...
describe("decideSpawn — allowlist guard (allow / allow-inherit / deny)", () => {
	...
	it("ALLOW-INHERITS an unset request with NO env pin via the committed DEFAULT_PIN (#943 durable default, the durable default model-pin rule)", () => {
		// A fresh clone / CI / cron with no WORKFLOW_MODEL in-shell no longer re-hits the
		// the originating work item fail-closed-on-unset symptom — the absent pin falls back to the committed default.
	...
	it("DENIES an off-allowlist request even with no env pin (never a silent allow; #943 durable default reclassifies the reason)", () => {
		...
		assert.strictEqual(d.kind, "deny");
		// Post-the originating work item: the absent env pin falls back to the valid DEFAULT_PIN, so effectivePin is
	...
	it("every decision emits what it checked (the zero-scope fail-closed invariant observability)", () => {
		for (const d of [
	...
	it("DENIES off-family aliases (sonnet / haiku / fable-5 / garbage) — fail-closed preserved (the zero-scope fail-closed invariant)", () => {
		for (const alias of ["sonnet", "haiku", "fable-5", "opusplan", "not-a-model"]) {
			...
			// explicit-off-allowlist deny (the pin can't override it, the originating work item) — not the no-valid-pin one.
			assert.strictEqual(d.kind === "deny" ? d.explicitOffAllowlist : false, true);
		}
	});

	it("an unset request still resolves to allow-inherit — alias support is request-only (the durable default model-pin rule unchanged)", () => {
		const d = decideSpawn(null, null);
		assert.strictEqual(d.kind, "allow-inherit");
	});
});
