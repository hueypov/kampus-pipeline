 ...
 * mutation into the worktree, but the documented failure/the originating work item detach class is a bare op escaping
 ...
 * `stash`/`merge` mutate the primary working tree (the originating work item: a review-doc agent ran
 ...
 * ruling on the arm-vs-refuse posture (the originating work item) is REFUSE, scoped to guarded
 * agents: the refusal fires when `$WORKTREE_ROOT` names a managed worktree —
 * i.e. this hook is active for an `isolation:worktree` subagent — OR, per the expected-worktree isolation fail-closed rule, when
 * isolation was EXPECTED (a direct isolation-asserting agent-type, OR a nested crew spawn whose
 * inherited agent-type masks that — detected via `git-dir == git-common-dir`; the originating work item) yet
 * `$WORKTREE_ROOT` is unset (the documented failure harness no-op that disarms the root-keyed branch and leaves
 * this the sole layer).
 ...
 * either refusal. The guard route is enforced here.
 * A second, distinct refusal is the **auto-stage-all containment** (the originating work item): `git add -A/--all/.`
...
// many-slash input; mirrors `bash-attribution.ts`'s `stripTrailingSlashes`, the related failure modes). The
 ...
 * shared primary is the documented failure detach (`checkout`/`switch`/`reset`/`rebase` move HEAD) or the documented failure
 ...
 * (the `-am` bundle included). Containment for the originating work item: a fresh worktree can come up dirty (an
...
// (the originating work item, mirroring the rewrite `bash-attribution.ts` already carries from the originating work item).
...
// The the related failure modes detach: an isolation-expecting agent (coder/reviewer/shipper) whose harness
// no-op'd worktree provisioning (the originating work item) has NO worktree, so `$WORKTREE_ROOT` is unset — which ALSO
...
// of `-C` scoping — this preflight is then the surviving layer (the expected-worktree isolation fail-closed rule).
const REFUSE_REASON_ISOLATION_NO_ROOT =
	...
	"to route around it — that hides the failure and collapses the primary-corruption defense (the expected-worktree isolation fail-closed rule, #2270).";

/**
 * Was worktree isolation EXPECTED for this run — the gate that arms the {@link pinBash}
 * no-worktree refusal (the expected-worktree isolation fail-closed rule)?
 *
 ...
 * `engineering-manager`), so the agent-type regex alone goes inert for it and the expected-worktree isolation fail-closed rule's loud-fail
 * never arms (the originating work item). So we corroborate with an **env-independent** signal — `onPrimaryCheckout`
 ...
 * human `/write-code`) matches neither clause, so it never over-refuses (the expected-worktree isolation fail-closed rule's standalone path).
 */
 ...
 *   the originating work item no-op / the originating work item detach), and the $WORKTREE_ROOT-keyed branch below is disarmed, so this is
 *   the surviving refusal (the expected-worktree isolation fail-closed rule). A non-head-move command still allows (no over-refusal).
 ...
 *   whole tree captures unauthored bleed into the commit — stage by explicit path only (the originating work item).
 ...
 *   scoped to the worktree → **refuse** (the originating work item; `stash`/`merge` added the originating work item).
...
export const pinBash = (args: {
	...
	if (!worktreeRoot || !isManagedWorktree(worktreeRoot)) {
		...
		// no-op'd provisioning (the originating work item) and a mutating git op here hits the PRIMARY checkout: a
		// head-move detaches it (the originating work item), and a stage-all captures the owner's tree state (the originating work item).
	...
	// unauthored bleed, and `-C "$WT"` scoping doesn't make it safe (the originating work item) — so it fires regardless.
