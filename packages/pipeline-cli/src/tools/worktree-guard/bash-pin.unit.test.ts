...
describe("pinBash — refuse a bare HEAD-moving git op in a guarded worktree (#1571)", () => {
	...
	// the originating work item: `stash`/`merge` corrupt the shared primary's WORKING TREE (the review-doc
...
// the expected-worktree isolation fail-closed rule / the originating work item: when isolation was EXPECTED (coder/reviewer/shipper) but the harness no-op'd
// provisioning (the originating work item), $WORKTREE_ROOT is unset — which used to disarm the guard entirely and let
// the documented failure/the originating work item `git -C "$WT" checkout` primary-checkout detach through. isolationExpected closes it.
describe("pinBash — isolation expected but NO provisioned worktree (the expected-worktree isolation fail-closed rule, #2453)", () => {
...
// the expected-worktree isolation fail-closed rule amendment / the originating work item: re-key the isolation gate off $CLAUDE_CODE_AGENT alone onto the
...
describe("isIsolationExpected — the expected-worktree isolation fail-closed rule gate, re-keyed onto git-dir == common-dir (#2462)", () => {
...
// the originating work item containment: a fresh worktree can come up dirty, so an auto-stage-all captures unauthored
