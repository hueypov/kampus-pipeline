/**
 * `adoption-lint` core — the pure, IO-free matcher behind the documented failure adoption
 ...
 * the originating work item: the lint lands FIRST so the verb-extraction sweep cannot grow the
 * unreferenced-tool pile (the `verdict read` / `merge-queue-classify` divergent-copy
 * class named in the originating work item / the originating work item).
 *
 ...
 * The declared exemptions are themselves linted (never a blanket skip, the originating work item):
 ...
 *
 * Fail-closed on zero scope (the zero-scope fail-closed invariant): the caller pairs this with `isZeroScope`,
...
export interface AdoptionResult {
	...
	/** Every corpus file the lint considered — its scope (the zero-scope fail-closed invariant). */
...
const lintExemption = (
	...
	// migration already fixed it and this entry is stale, so fail to force its removal (the originating work item).
 ...
 * and the two scopes the caller fails-closed on (the zero-scope fail-closed invariant).
 */
...
/**
 * Zero-scope test (the zero-scope fail-closed invariant): a lint that looked at NO corpus file, or a manifest
