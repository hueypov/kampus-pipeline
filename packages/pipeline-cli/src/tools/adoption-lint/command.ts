 ...
 *
 * The the originating work item adoption corpus-lint: flags a crew-corpus file that inline-re-derives a
 * tool-owned decision instead of citing the owning `pipeline-cli` verb, the governing
 * AC of the originating initiative (it lands first so the verb sweep can't grow the unreferenced-tool
 * pile). Follows the same IO boundary — an IO-free core (`lintAdoption`)
 * over a declared manifest, with a thin CLI that reads the handed corpus files and
 * maps the verdict to a fail-closed exit contract (the zero-scope fail-closed invariant):
 *
 ...
 * contract survives the fold into `pipeline-cli` — mirroring the other subcommand adapters.
 */
...
const judge = (result: AdoptionResult): Effect.Effect<void, ZeroScope | FindingsFound> =>
	Effect.gen(function* () {
		// the zero-scope fail-closed invariant: zero scope is a FAIL, never a silent PASS.
		if (isZeroScope(result)) {
			yield* Console.error(
				`adoption-lint: FAIL — scanned ${result.scanned.length} file(s) against ${result.decisionCount} declared decision(s); a zero scope on either axis is fail-closed (the zero-scope fail-closed invariant).`,
			);
...
const check = Command.make(
	...
	Effect.fn(function* ({files}) {
		...
		// the zero-scope fail-closed invariant: emit what was scanned (scope is observable), THEN judge.
		yield* Console.log(
...
export const adoptionLintCommand = Command.make("adoption-lint").pipe(
	...
	Command.withDescription(
		"Adoption corpus-lint: every extraction's decision must be cited, not re-derived (#3254 / the originating initiative)",
	),
);
