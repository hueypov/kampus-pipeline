 ...
 * `(created_at, comment id)` (the authenticated, session-stamped claim protocol §2) — among comments whose body matches the
 * canonical `CLAIM_RE` and whose author holds write+ on the repo (the runtime ACL authorization rule
 ...
 * gh-issue-intake-formats.md §7 (see the authenticated, session-stamped claim protocol); this core is the deterministic
