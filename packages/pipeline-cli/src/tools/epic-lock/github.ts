 ...
 * and releases the epic mutation lock `status:planning` epic-lock over `gh api` REST, driving
 ...
 * resolves over.
 *
 * The lock is **two layers** (the authenticated, session-stamped claim protocol / gh-issue-intake-formats.md §7): the coarse
 ...
 * exception. See the epic mutation lock (the epic-plan lock) and the authenticated, session-stamped claim protocol (the claim marker).
 */
import {Context, Effect, Layer} from "effect";
import * as Schema from "effect/Schema";
import {ChildProcessSpawner} from "effect/unstable/process";
// The `gh api` REST IO seam (runGh / resolveRepo / authorizedAuthors / RawComment) is the SINGLE
// SOURCE shared with the `Tracker` service and `verdict` — no longer re-copied here (the originating work item AC 5).
...
/** The canonical epic-plan lock label (the epic mutation lock). */
...
// The epic-lock label envelope (the epic mutation lock `status:planning` read/add/remove) is the arg-builder
 ...
 * the exact catastrophe the epic mutation lock prevents — so a non-404 fault propagates as
 ...
 * `R = never`. Repo resolution is deferred to first use (`Effect.cached`, the repository-resolution contract
 * §1, the originating work item): the layer build is side-effect-free, and `RepoResolutionError` lives in
