 ...
 *
 * The `status:planning` epic mutation lock, extracted from the ~50-line inline jq
 * glue the `plan-epic` / `review-plan` / `write-code` skills each hand-rolled (the originating work item).
 * `acquire` runs the two-layer coarse-label + agent-distinguishable-claim protocol
 * (the authenticated, session-stamped claim protocol) and **exits 0 only when the lock is ours**; every fail-closed back-off —
...
export const epicLockCommand = Command.make("epic-lock").pipe(
	...
	Command.withDescription(
		"Acquire/release the epic mutation lock status:planning epic-lock (the authenticated, session-stamped claim protocol claim protocol)",
	),
