/**
 * The `settings-env-guard` filesystem gate — the IO seam behind the originating work item's "no
 ...
 * rather than only by spawning the bin (the core-in-its-own-file idiom; the originating work item).
 *
 ...
 * an `IoError` (also non-zero) — fail-closed on the file we can't scan (the zero-scope fail-closed invariant).
 */
...
/** A read/parse failure: the guard couldn't scan the settings file — fail-closed (the zero-scope fail-closed invariant). */
 ...
 * missing/unreadable/unparseable settings file (the zero-scope fail-closed invariant).
 */
