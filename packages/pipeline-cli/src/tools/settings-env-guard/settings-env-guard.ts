 ...
 * VALUE carries an unexpanded `${...}` token (the originating work item). IO-free and total: a
 ...
 * `${CLAUDE_PROJECT_DIR}` directory at the repo root and clobbered PATH (the originating work item).
 ...
 * concern (the zero-scope fail-closed invariant) is the FILE — an unreadable/unparseable settings.json — and
 ...
 * entries (its evidence — the zero-scope fail-closed invariant §1 "emit what you scanned").
 */
