/**
 * `checkSettingsEnv` over a fake repo dir — the filesystem-seam test (the related failure modes).
...
describe("checkSettingsEnv — IO gate", () => {
	...
	it("SUCCEEDS on a settings.json with no env block (the post-#2495 example-repo shape)", () => {
		writeSettings({enabledPlugins: {"kampus-pipeline@acme": false}, hooks: {}});
		return run(checkSettingsEnv(root)).then((exit) => expect(Exit.isSuccess(exit)).toBe(true));
	});
	...
	it("FAILS CheckFailed on an env value carrying an unexpanded brace token (the #2495 regression)", () => {
		...
		// placeholder (biome noTemplateCurlyInString) — it is exactly the documented failure value.
		writeSettings({
	...
	it("FAILS IoError (fail-closed, the zero-scope fail-closed invariant) when settings.json is missing", () =>
