 ...
 * a user-local filesystem path into a SHARED artifact (the originating work item), moved into the
 * pipeline-cli registry (the originating initiative, Phase 2 / the originating work item).
 *
 ...
 * main — see the originating work item). `findLeaks(filePath, text)` returns every leak in `text` when
 ...
 * `~/.claude` config-file carve-out (the related failure modes); the `/tmp` arm carries no carve-out,
 * so it fail-closes on any bare `/tmp/…` (the originating work item Option 1) — live in the single shared
 * `path-matcher.ts` module that both this doc/comment scanner and `crew-leak.ts` import,
 * so the two detectors can never
 * drift (the documented failure root bug). This module owns only the doc-surface scoping (which
...
const DOC_SELF_EXEMPT = [
	...
	// patterns (moved out of leak-guard.ts per the originating work item), so it is exempt like its host was.
	...
	// machine-local shapes it forbids in the return summary (the originating work item) — illustrative rule
 ...
 * mktemp path onto public PRs (the related failure modes/the related failure modes). A comment body is UNCONDITIONALLY a
 ...
 * (which fail-closes on any bare `/tmp/…`, no carve-out — the originating work item Option 1). Generic path shapes only (the originating work item).
 */
