 ...
 * resolved once per the repository-resolution contract §1, every infra fault a typed error) — a deliberately separate, minimal
...
/** Resolve the target repo (`owner/name`) per the repository-resolution contract §1: env override → CI env → `gh repo view`. */
 ...
 * use (`Effect.cached`, the repository-resolution contract §1) — the layer build is side-effect-free.
 */
