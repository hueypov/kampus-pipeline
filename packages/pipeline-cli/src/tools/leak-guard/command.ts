 ...
 *
 * The CI-callable scan for the originating work item, moved into the pipeline-cli registry (epic
 * the originating work item, Phase 2 / the originating work item):
 *
 ...
 * fail-closes (any non-zero fails the gate). See the originating work item.
 *
 ...
 * per-tool catch. The package's the originating work item stale-tree shim (`bin.ts` / `preflight.ts`)
...
// CI treats as failure (the originating work item).
...
const scan = Command.make(
	...
	Effect.fn(function* ({files}) {
		const run = Effect.gen(function* () {
			...
			yield* Console.error(
				"leak-guard: blocked — user-local path(s) in shared doc surface(s) (the originating work item):",
			);
...
// The repeatable pipeline-crew sanitization sweep (the originating work item). Where `scan` takes explicit
...
// data" contract bans — fail-closed on any hit and on a zero-file scope (the zero-scope fail-closed invariant),
...
// The pre-post net for a PR/issue COMMENT body (the originating work item). Where `scan` takes doc FILES and
...
const scanComment = Command.make(
	...
	Effect.fn(function* ({bodyFile}) {
		const run = Effect.gen(function* () {
			...
			yield* Console.error(
				"leak-guard: blocked — machine-local path(s) in a PR/issue comment body (the originating work item):",
			);
			for (const leak of leaks) {
...
// The LANDED-comment scan (the originating work item). Where `scan-comment` nets a body BEFORE it is posted (an
...
const scanPr = Command.make(
	...
	Effect.fn(function* ({pr}) {
		const run = Effect.gen(function* () {
			...
			yield* Console.error(
				`leak-guard: blocked — machine-local path(s) in landed comment(s) on PR #${pr} (the originating work item):`,
			);
