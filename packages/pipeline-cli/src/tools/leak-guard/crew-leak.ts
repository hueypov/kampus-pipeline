 ...
 * pipeline-crew sanitization contract (the originating work item, crew the originating initiative Phase 4). Where
 ...
 * (the single source both detectors import, per the documented failure ruling) — so its config-file and
 * `/tmp` shape carve-outs can never drift from leak-guard's, which was the root bug the originating work item
