 ...
 * leaks a machine-local path onto a public PR regardless of emit path (the related failure modes). Re-scanning the
 * comments straight off the REST boundary moves that one missing check off the reviewer's transcript
 * to the ship-it preflight every merge crosses (the originating work item).
 */
