...
describe("~/.claude public-config-file carve-out (#3475 — narrowed by shape, both surfaces)", () => {
	...
	// The end-to-end guard-4 case from the originating work item: one crew-mcp comment naming all three at once passes.
...
describe("findCommentLeaks — PR/issue comment body scan (#2796, stricter than the file surface)", () => {
	...
	// the originating work item — the drive-letter carve-out: a Windows `C:/Users/...` file URL is not a macOS-home
	// leak, so quoting one in a verdict comment must not fail-closed-block enqueue (the documented failure FP);
	...
	it("allows a GitHub PR URL (not a machine-local path)", () =>
		assert.isFalse(
			hasCommentLeak("cross-linked with https://github.com/example-org/example-repo/pull/2796"),
		));
...
describe("surface predicates", () => {
	...
	it("isSelfExempt: the guard's own files (old package + moved tool) and the path-hygiene skills", () => {
		...
		// shapes as illustrative text (the originating work item), so it is self-exempt like the skills.
