/**
 * The `leak-guard sweep` filesystem gate — the IO seam behind the originating work item's repeatable
 * pipeline-crew sanitization check (crew the originating initiative Phase 4). Split from `command.ts`
 ...
 * personal-data hit OR when zero files are in scope (fail-closed, the zero-scope fail-closed invariant — an empty
 ...
 * `CREW_DIR`). Fails closed on zero files in scope (the zero-scope fail-closed invariant).
 */
	...
	Effect.gen(function* () {
		...
		if (files.length === 0) {
			return yield* Effect.fail(
				new CheckFailed({
					reason:
						`leak-guard sweep: scanned ZERO files under ${dir} — fail-closed (the zero-scope fail-closed invariant). ` +
