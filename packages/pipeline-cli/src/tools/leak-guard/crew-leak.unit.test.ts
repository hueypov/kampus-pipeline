 ...
 * sweep (the originating work item). Covers every GENERIC personal-data class the crew contract bans
...
describe("findCrewLeaks — match classes", () => {
	describe("path class (machine-local / home / absolute)", () => {
		...
		// the originating work item — the drive-letter carve-out drops the Windows-file-URL FP while keeping the
		...
		// the originating work item — the shared path-matcher's config-file carve-out now applies HERE too (it used to
		...
		it("does NOT flag the plugin-root env var or relative ../../.decisions paths", () => {
			...
			expect(findCrewLeaks("see decision record .decisions/0001-sample.md")).toEqual([]);
		});
