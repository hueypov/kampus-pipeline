 ...
 * founder ruling on the originating work item (Option C — extract, don't copy): the two detectors each carried their
 * own copy of the same path arm, and the originating work item's `~/.claude` shape carve-out landed on leak-guard's
 * copy only — so the copies drifted, which is the exact root bug the originating work item records. With the arm (and
 ...
 *
 * Two arms, both GENERIC structural shapes and never a named operator allow-list (the originating work item):
 *
...
export const MACHINE_LOCAL_PATH_PATTERNS: ReadonlyArray<PathPattern> = [
	{
		...
		// carries no operator PII, yet its FP fail-closed-blocked legitimate PRs (the originating work item).
	...
	{
		// The `~/.claude` home-dir detector, NARROWED by shape (not a named allow-list; the originating work item
		// and the originating work item): it still flags any descent into the private agent home tree
		...
		// exact public leaf. See the threat-model note on the originating work item for what this can no longer catch.
...
// (the documented failure/the originating work item scratchpad-`@filepath` and the related failure modes mktemp-in-`@sha` leaks). Generic path
// shapes, NOT a named deny-list (the originating work item). The `(?<![\w.])` lookbehind stops a `/private/tmp/...`
