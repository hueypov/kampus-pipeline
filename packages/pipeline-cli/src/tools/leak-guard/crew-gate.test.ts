/**
 * `sweepCrew` over a fake crew dir — the filesystem-seam test for the originating work item. The pure
 ...
 * scope `CheckFailed`s (fail-closed, the zero-scope fail-closed invariant). Seeded fixtures use FAKE data only
	...
	it("SUCCEEDS on a clean tree (placeholder-only content, fictional examples)", async () => {
		writeCrewFile(
			...
			'# crew\nFill `operator.handle` with your own, e.g. `"@robin"`.\n' +
				...
				`Copy \`$${"{CLAUDE_PLUGIN_ROOT}"}/crew.config.template.jsonc\`.`,
		);
