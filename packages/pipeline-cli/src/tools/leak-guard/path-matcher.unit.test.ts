/**
 * `path-matcher` unit tests — the single source both leak-guard detectors import (the originating work item). These
 * pin the one SHAPE carve-out that must apply uniformly to every consumer: the `~/.claude` public
 * config-file exemption (the related failure modes). The `/tmp` arm has NO carve-out (the originating work item Option 1 — the socket
