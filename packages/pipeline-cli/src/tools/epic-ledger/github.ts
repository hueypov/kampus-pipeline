 ...
 *
 * Decode the JSON at this boundary, where genuinely untyped REST responses enter — and not past
 ...
 *
 * Every mutation is scoped to exactly what the gate may touch (the verified-plan transition): a
 ...
 *
 * Repo resolution is **deferred to first use, not run at layer build** (the originating work item). The
 * resolution (the repository-resolution contract §1: `CLAUDE_PIPELINE_REPO` → `GITHUB_REPOSITORY` →
 ...
 * (still once per process, never a silent example-repo default — the originating work item).
 *
