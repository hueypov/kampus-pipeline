/**
 * The re-plan convergence loop with a stall-based circuit breaker (the verified-plan transition
 * Decision 3, the originating work item).
 *
 * On a FAIL verdict from the gate (the originating work item), the loop routes the epic back through
 ...
 * consumer's `catchTag` can tell a re-plan fault from a `gh` exit. It is a standalone
