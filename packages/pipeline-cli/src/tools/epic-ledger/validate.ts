 ...
 * later flips to `status:triaged` — see the verified-plan transition).
 */
...
export const validateLedger = (ledger: EpicLedger): ReadonlyArray<Defect> => {
	...
	// Zero-scope=fail self-assertion (formats §ZS / the zero-scope fail-closed invariant): the floor's scope IS the
	...
	if (children.length === 0) {
		return [
			{
				type: "ZERO_SCOPE",
				message: `Epic #${epic.number} declares no linked children; the floor scanned zero scope (a gate that scans nothing fails closed — the zero-scope fail-closed invariant).`,
				refs: [epic.number],
			},
		];
	}
	...
	for (const child of [...children].sort((a, b) => a.number - b.number)) {
		...
		// the real-consumer requirement forcing function: in a repo with a cycle doc, every `type:feature`
		...
		if (
			...
			defects.push({
				...
				message: `Child #${child.number} is \`type:feature\` but carries no \`**Containment:**\` marker; every feature child must declare \`flag (default-off)\` or \`exempt (<reason>)\` (the real-consumer requirement).`,
				refs: [child.number],
			});
		}
