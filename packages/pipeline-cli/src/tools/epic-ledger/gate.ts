/**
 * The deterministic `review-plan` gate action (the verified-plan transition, the originating work item).
 *
 ...
 * in the verified-plan transition). It is flag-not-repair: it signals; the re-plan loop (the originating work item) owns
 ...
 * across iterations to detect a stall (the originating work item). `signature` is `ledgerSignature`
 ...
 * emit-scope facet (the zero-scope fail-closed invariant): the gate states *what it looked at* (the children it
