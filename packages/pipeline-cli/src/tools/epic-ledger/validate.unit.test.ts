...
describe("validateLedger — each defect type", () => {
	...
	it("a non-child ref resolved as a cross-epic edge (externalRefs) is NOT a dangling dep", () => {
		// the originating work item is referenced via `requires:` but is not a child of this epic; the IO
	...
	it("each of the closed defect types is producible", () => {
		...
		// a childless epic produces the zero-scope=fail self-assertion (the zero-scope fail-closed invariant)
...
// The zero-scope=fail self-assertion (formats §ZS / the zero-scope fail-closed invariant), demonstrated on the floor:
...
describe("validateLedger — zero-scope=fail self-assertion (the zero-scope fail-closed invariant)", () => {
