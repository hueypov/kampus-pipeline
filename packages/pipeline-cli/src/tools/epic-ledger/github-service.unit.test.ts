...
// (the repository-resolution contract §1, deferred per the originating work item): env override first, else `gh repo view`. These
// tests pin the env override to `example-org/example-repo` so the fixtures keyed on
// `repos/example-org/example-repo/...` match without depending on the ambient `gh repo
// view`. The resolution-order describe below clears it to exercise the other
// branches explicitly.
const PINNED_REPO = "example-org/example-repo";
...
describe("Github.epicLedger — over a mock gh spawner", () => {
	it.effect("assembles a clean, story-covered EpicLedger consumable by validateLedger", () =>
		Effect.gen(function* () {
			...
			provide(effect, {
				"repos/example-org/example-repo/issues/159": issue(159, EPIC_BODY, [
					"type:epic",
					"p1",
					"status:triaged",
				]),
				"repos/example-org/example-repo/issues/159/sub_issues": JSON.stringify([
					{number: 101},
					{number: 102},
				]),
				"repos/example-org/example-repo/issues/101": issue(
					101,
					"**Stories:** 1\n### Acceptance criteria\n- [ ] ac",
					["type:feature", "p1", "status:triaged"],
				),
				"repos/example-org/example-repo/issues/102": issue(
					102,
					"**Stories:** 2\n### Acceptance criteria\n- [ ] ac",
					["type:feature", "p1", "status:triaged"],
				),
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				// the related failure modes are type:feature with no `**Containment:**` line ⇒ both flagged.
				...
				provide(effect, {
					...
					"repos/example-org/example-repo/contents/product-development-cycle.md":
	...
	it.effect("surfaces malformed gh JSON as a typed GhParseError (not a throw)", () =>
		Effect.gen(function* () {
			...
			provide(effect, {
				"repos/example-org/example-repo/issues/159": "not json {{{",
			}),
		),
	);
});
...
const xrefResponses = (epicNumber: number): Record<string, Response> => ({
	[`repos/example-org/example-repo/issues/${epicNumber}`]: issue(epicNumber, XREF_BODY, [
		"type:epic",
		"p1",
		"status:triaged",
	]),
	[`repos/example-org/example-repo/issues/${epicNumber}/sub_issues`]: JSON.stringify([
		{number: 101},
		{number: 102},
	]),
	"repos/example-org/example-repo/issues/101": issue(
...
describe("Github.epicLedger — cross-epic dependency resolution at the boundary", () => {
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				// the originating work item is referenced via `requires:` but is not a linked child; it resolves
				...
				provide(effect, {
					...xrefResponses(160),
					"repos/example-org/example-repo/issues/108": issue(108, "a cross-epic dependency", [
						"type:feature",
						"p2",
						"status:triaged",
					]),
	...
	it.effect("a `requires:` ref that 404s is left out of externalRefs and still DANGLES", () =>
		Effect.gen(function* () {
			...
			// the originating work item is unmapped → the probe 404s → it is not resolved, so it dangles.
...
// `process.env` on the first method call (deferred — the originating work item), so env is set
...
// proof that repo resolution is deferred out of the layer build (the originating work item).
...
describe("GithubLive — target repo resolution (the repository-resolution contract §1)", () => {
	// The read only succeeds if the resolved repo became the `repos/<repo>/...` URL
	// prefix the fixture map is keyed on — a wrong repo (e.g. a example-repo default)
	...
	it("CLAUDE_PIPELINE_REPO wins; gh repo view is never consulted", async () => {
		const exit = await runResolution({
			...
			repoView: {stdout: "example-org/example-repo"}, // would mis-route if ever consulted
			epicNumber: 159,
		});
		assert.isTrue(exit._tag === "Success");
	...
	it("GITHUB_REPOSITORY is used when CLAUDE_PIPELINE_REPO is unset", async () => {
		const exit = await runResolution({
			...
			repoView: {stdout: "example-org/example-repo"},
			epicNumber: 159,
		});
		assert.isTrue(exit._tag === "Success");
		if (exit._tag === "Success") assert.strictEqual(exit.value.epic.number, 159);
