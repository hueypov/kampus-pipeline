 ...
 * boundary (diff / --version / install) lives in `command.ts`. See the originating work item.
 *
 ...
 * leaves the install stale until a re-install (the originating work item).
 */
...
/**
 * The pnpm-version guard outcome (candidate 3 of the originating work item, folded into the install path). Only
...
export type PnpmGuardResult =
	...
	// leaves a stale patched dir (the originating work item), so fail closed instead of installing under it.
	| {
			readonly ok: false;
