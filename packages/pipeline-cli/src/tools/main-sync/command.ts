 ...
 * checkout (the originating work item, Unit C of the documented failure diagnosis). Replaces the hand-run
 ...
 *   - default (drain-sync, the related failure modes): AGGRESSIVE — reattaches a detached/off-`main`
 *     HEAD to `main` (clean tree only) before the merge, for the orchestrator's
 *     before/after-drain sync.
 *   - `--post-merge` (refresh, the originating work item): GENTLE — invoked by a pipeline step that knows a
 ...
 *     the originating work item); on a non-`main` branch or a tree with tracked modifications it LEAVES
 *     THE CHECKOUT ALONE and exits 0 (never moves HEAD, never errors). This closes the
 *     silent-drift hazard under the merge queue (the merge-queue base-freshness rule), where a PR lands GitHub-side
...
import {isControlPlaneDeletion, parseNameStatus} from "../primary-index-guard/tripwire.ts";
 ...
 * unreadable tree as tracked-dirty so the refresh leaves it alone). See the originating work item: the post-merge
...
/**
 * The count of staged deletions under the instruction-trust prefixes — the documented failure signature (the originating work item),
 * classified with the local tripwire detection so main-sync's guaranteed refusal and
 ...
 * change — the documented failure fix. Without it the ff advances the SOURCE patch/lockfile but not the
...
const refreshDepsAfterMerge = Effect.fn(function* (preSha: string) {
	...
	// pnpm — the wrong-major hazard the originating work item exists to close. A failed probe ⇒ unresolved ⇒ fail-closed.
...
/**
 * The gentle post-merge refresh (the originating work item) — the HEAD-preserving counterpart to the drain-sync
...
const runPostMergeRefresh = Effect.fn(function* (head: HeadState, execute: boolean) {
	...
	// the zero-scope fail-closed invariant "emit what you scanned": the HEAD state + plan are observable before any action.
	// Report the tracked/untracked distinction the refresh actually gates on (the originating work item), not bare dirt.
	...
	if (plan.action === "refuse-mass-deletion") {
		// The one refresh outcome that is LOUD (exit 1), not a silent no-op: the documented failure loaded-gun
		// state must surface, never be skipped as ordinary dirt (the originating work item).
		yield* Console.error(
	...
	// HEAD before the merge, so refreshDepsAfterMerge diffs exactly what the ff pulled (the originating work item).
	...
	// never a force. On a clean 'main' the worst case is a no-op, never a clobber (the originating work item).
...
const mainSync = Command.make(
	...
	Effect.fn(function* ({execute, postMerge}) {
		...
		if (plan.action === "blocked-mass-deletion") {
			// The guaranteed catch (the originating work item): refuse by NAME on the documented failure signature, before any reattach
		...
		if (plan.action === "blocked-dirty") {
			// Detect-and-surface, never discard: refuse the reattach on a dirty tree (the originating work item AC #3).
			yield* Console.error(
