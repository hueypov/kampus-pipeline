 ...
 *
 * The StructuredOutput conformance slice for the originating work item (the originating initiative), moved into the
 * pipeline-cli registry (the originating initiative, Phase 2 / the originating work item). Two verbs, both reading their
...
export const structuredOutputGuardCommand = Command.make("structured-output-guard").pipe(
	...
	Command.withDescription(
		"Make a subagent's final StructuredOutput call conform first-try and self-correct in one retry (the originating work item)",
	),
);
