/**
 * The pure match core for `split-guard` (the originating work item): the create-once key that makes the
 * `triage` skill's Step-3 "Split bundled reports" child-create idempotent.
 *
 * The observed defect (the related failure modes): a single split of a parent emitted two byte-identical
 ...
 * on byte-equality of the body (the originating work item AC2 — a twin emitted with a slightly different body is still
 ...
 * and only on the exact number (a `#12` boundary guard so `#12` never matches `the originating work item`).
 */
 ...
 * twin (the documented failure/the originating work item case) and a whitespace-/case-variant twin both map to the same key.
 */
 ...
 * the survivor is deterministic (the earliest-created child wins, mirroring the documented failure→the originating work item canonical
