/**
 * The GitHub boundary for `split-guard` (the originating work item): the live `Github` capability that reads the
...
export class RepoResolutionError extends Schema.TaggedErrorClass<RepoResolutionError>()(
	"split-guard/RepoResolutionError",
	{
		message: Schema.String,
	},
) {}
...
const resolveRepo = Effect.fn("Github.resolveRepo")(function* () {
	...
	const viewed = yield* runGh([
		...
		Effect.catchTag("split-guard/GhCommandError", () => Effect.succeed("")),
	);
 ...
 * deferred to first use (`Effect.cached`, the repository-resolution contract §1): the layer build is side-effect-free.
 */
