/**
 * Unit tests for the `split-guard` pure core (the originating work item): the create-once key that makes the triage
 * split idempotent. IO-free — no `gh` boundary here. The headline invariant is
 * "split runs twice → one child" (the documented failure/the originating work item double-fire).
 */
...
describe("referencesParent", () => {
	...
	it("does not match a different parent, and guards the number boundary", () => {
		...
		// the originating work item must not match when the parent is the originating work item (prefix boundary).
...
describe("findExistingChild — the create-once decision", () => {
	...
	it("split runs twice → one child: the SECOND run finds the first child and skips the POST", () => {
		// After the first emit, the read-after-write needs-triage queue carries the originating work item. A second run
		// of the split with the same (parent, title) must resolve to the originating work item — never a the originating work item twin.
