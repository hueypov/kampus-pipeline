 ...
 * drift gate (the originating work item). `gate.ts` wires it to the filesystem (read the canonical
 ...
 * `require_code_owner_review` (under-protected; bit on the related failure modes). This core reads
 ...
 * closed on a `null` (the zero-scope fail-closed invariant: can't parse the source ⇒ refuse, never pass).
 */
...
export const expandBranch = (branch: string): ReadonlyArray<CpPath> => {
	// `([^/]+/)*` is the any-depth segment prefix (the skill-`.sh` clause, the originating work item) — a
	...
	const normalize = (s: string): CpPath => {
		...
		// CODEOWNERS row can own (the bare gate-critical `skills/*.sh` guards — the executable-guard control-plane classification).
		if (base.includes("[^/]+")) {
 ...
 * empty result as a fail-closed condition (zero-scope ⇒ refuse, the zero-scope fail-closed invariant).
 */
