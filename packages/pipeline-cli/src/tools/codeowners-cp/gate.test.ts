...
// The gate derives paths from the single-source const (the originating work item); a fixture formats doc must
...
describe("checkCodeownersCp", () => {
	...
	it("fails closed when the formats-doc CONTROL_PLANE_RE has drifted from the single-source const", async () => {
		...
		// that must never diverge from the const the gate derives from (the related failure modes).
