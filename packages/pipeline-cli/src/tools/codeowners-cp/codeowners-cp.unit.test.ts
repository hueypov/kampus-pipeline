...
// so this fixture cannot drift from the boundary (the documented failure class). The lockstep test at the
...
describe("expandBranch", () => {
	...
	it("passes the group-free agents dir branch through as a single dir (the agent-definition control-plane boundary)", () => {
	...
	it("expands the biome-governance branches: an exact file + a dir prefix (the lint-governance control-plane rule)", () => {
...
describe("findUncovered — the drift check", () => {
	...
	it("flags a §CP path the regex adds but CODEOWNERS still misses (the drift it guards)", () => {
		// CODEOWNERS missing the hooks rows + pipeline-cli — the exact pre-the originating work item gap.
		const stale = [
			"/.claude/ @usirin",
			"/.github/ @usirin",
			"/claude-plugins/kampus-pipeline/skills/ship-it/ @usirin",
...
// the originating work item), because validate-gate-path-drift.sh's consumer set never included these unit
