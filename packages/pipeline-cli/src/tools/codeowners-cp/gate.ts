/**
 * The `check` IO gate — the filesystem seam behind the originating work item's §CP↔CODEOWNERS drift
 ...
 * the originating work item). `command.ts` wires this effect to the Effect CLI and maps the failures to
 * the non-zero gate exit; the exit-code contract lives there.
 *
 * The §CP boundary is the single-source const `CONTROL_PLANE_RE` in
 * `control-plane-paths/control-plane-re.ts` (the originating work item). The gate derives the §CP
 ...
 * Fail-closed (the zero-scope fail-closed invariant): a missing/unreadable source, a formats line it cannot parse, a
	...
	Effect.gen(function* () {
		...
		if (re === null) {
			return yield* Effect.fail(
				new CheckFailed({
					reason: `could not parse CONTROL_PLANE_RE='…' from ${FORMATS_PATH} — refusing (fail-closed, the zero-scope fail-closed invariant).`,
				}),
			);
		}

		// Drift guard for the un-importable prose surface: the live merge-deciding gates read
		// CONTROL_PLANE_RE from this formats line on origin/main (the originating work item), so it must stay byte-equal
		// to the single-source const. Fail-closed on any divergence (the originating work item).
		...
		if (paths.length === 0) {
			return yield* Effect.fail(
				new CheckFailed({
					reason: `CONTROL_PLANE_RE resolved ZERO §CP paths — refusing (zero-scope fail-closed, the zero-scope fail-closed invariant).`,
				}),
			);
		}
		...
		if (patterns.length === 0) {
			return yield* Effect.fail(
				new CheckFailed({
					reason: `${CODEOWNERS_PATH} has ZERO owned entries — refusing (fail-closed, the zero-scope fail-closed invariant).`,
				}),
			);
		}
 ...
 * the shared `find-root-dir` / decisions-index walk (the originating work item).
 */
