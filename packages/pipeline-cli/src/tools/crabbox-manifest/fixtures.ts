/**
 * Test fixtures derived from spike the originating work item's verified crabbox output. Plain TypeScript data,
 * with no Effect runtime: a representative run-summary (the `provider`/`leaseId`/`slug`/timing/`exitCode`/`artifacts[]`/
 * `leaseStopped` shape the originating work item captured, widened with the per-command `commands[]`
 * the adapter folds into `checks[]`) plus JUnit XML strings.
 */
import type {RunSummary} from "./crabbox.ts";

/**
 * A passing crabbox run (the documented failure happy path): a local-container lease that ran
