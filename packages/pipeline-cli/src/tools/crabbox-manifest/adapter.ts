 ...
 * `TestSummary` + the stamped commit + a logs ref → an the SHA-bound run-evidence requirement §2 `Manifest`.
 *
 ...
 * (its unit tests run it directly; the unit/integration tier boundary). It derives `checks[]` from per-command
...
export interface AdapterInput {
	 ...
	 * headless worker-bundle node-core assertion (the originating work item), which runs on the runner,
...
/** Fold crabbox's provider/lease facts into the optional the SHA-bound run-evidence requirement §2 `lease` block. */
...
/**
 * Build the SHA-bound run-evidence requirement §2 manifest. Pure: same inputs → same manifest, no IO. The
