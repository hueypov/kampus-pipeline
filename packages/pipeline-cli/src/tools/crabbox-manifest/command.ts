 ...
 *
 * The crabbox → the SHA-bound run-evidence requirement §2 run-evidence adapter (the originating work item), moved into the pipeline-cli
 * registry (the originating initiative, Phase 2 / the originating work item). Reads a crabbox run-summary JSON and
 ...
 * `NodeServices.layer`), so this stays testable off real disk; a filesystem fault folds into the
...
export const crabboxManifestCommand = Command.make(
	...
	Command.withDescription("Map a crabbox run to an the SHA-bound run-evidence requirement §2 run-evidence manifest"),
