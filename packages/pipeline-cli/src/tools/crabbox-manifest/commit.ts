/**
 * Commit stamping — close the one gap spike the originating work item found: crabbox seeds git and
 * resolves `HEAD` in-box but never surfaces the SHA, so the SHA-bound run-evidence requirement §1's
 * `bundle.commit == head SHA` binding isn't satisfied out of the box.
 *
 * `Git` is a `Context.Service` whose live layer runs `git rev-parse HEAD` over `ChildProcessSpawner`
 * (`effect/unstable/process`) and returns the trimmed SHA. A missing/empty SHA is
 * a hard `MissingCommitError` (the SHA-bound run-evidence requirement §1: `commit` is the binding key — never
...
export class MissingCommitError extends Schema.TaggedErrorClass<MissingCommitError>()(
	"crabbox-manifest/MissingCommitError",
	{
 ...
 * per the SHA-bound run-evidence requirement §1 and the originating work item's acceptance criteria). Like `epic-ledger`'s `Github`,
