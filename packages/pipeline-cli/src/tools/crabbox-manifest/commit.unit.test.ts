 ...
 * (the originating work item). The never-blank-commit guard (the SHA-bound run-evidence requirement §1) has three
 * `MissingCommitError` paths and a happy path; this crosses the IO seam with a mock spawner, so all four are
...
describe("GitLive.headSha — over a fake spawner (the SHA-bound run-evidence requirement §1: commit never blank)", () => {
