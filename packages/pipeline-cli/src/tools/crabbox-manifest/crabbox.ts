 ...
 * `TestSummary`.
 *
 * Decode it here, where genuinely untyped crabbox output enters — and not past it: the
 * pure transform (`adapter.ts`) is total over a decoded `RunSummary`. The crabbox
 * shape is the one verified in spike the originating work item (`provider`/`leaseId`/`slug`/timing/
 ...
 * legitimate run (a config-only PR runs no test step) — the SHA-bound run-evidence requirement §2 makes `tests`
...
/**
 * The crabbox machine-readable run-summary (spike the originating work item's verified shape). Only
...
export class CrabboxParseError extends Schema.TaggedErrorClass<CrabboxParseError>()(
	"crabbox-manifest/CrabboxParseError",
	{
 ...
 * throw) that keeps a no-JUnit run from crashing (the SHA-bound run-evidence requirement §2).
 */
export const parseJUnit = (xml: string | null | undefined): TestSummary => {
	...
	let root: unknown;
	// biome-ignore lint/plugin: best-effort parse — malformed XML is absorbed into ZERO_TESTS (the SHA-bound run-evidence requirement §2: a no-JUnit run must not crash), never the E channel; a total helper, not Effect-cosplay.
	try {
