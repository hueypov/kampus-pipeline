/**
 * The SHA-bound run-evidence bundle manifest, as Effect Schema — the output
 ...
 *
 * The contract is defined by its *fields*, not its producer (the SHA-bound run-evidence requirement §2):
...
/** The manifest schema this package emits; bumped when the shape changes (the SHA-bound run-evidence requirement, sibling #243). */
 ...
 * pointer to its machine-readable result (the SHA-bound run-evidence requirement §2 `checks[]`). `exitCode`
...
/**
 * The folded JUnit summary (the SHA-bound run-evidence requirement §2 `tests`): totals + each failure's suite +
...
/**
 * Producer + run metadata (the SHA-bound run-evidence requirement §2 `run`): producer id, optional run URL,
...
/** A reference to captured stdout/stderr for the run (the SHA-bound run-evidence requirement §2 `logs`). */
 ...
 * generated the bundle (the SHA-bound run-evidence requirement §2 `lease` / §5). crabbox is such a producer,
...
/**
 * The full the SHA-bound run-evidence requirement §2 run-evidence manifest. `commit` is the binding key (the
