...
describe("buildManifest (crabbox → the SHA-bound run-evidence requirement §2 manifest)", () => {
	it("happy path: emits every required field plus schemaVersion", () => {
		const m = build(passingRunSummary(), passingJUnit);

		// Every the SHA-bound run-evidence requirement §2 required field is present, plus schemaVersion.
