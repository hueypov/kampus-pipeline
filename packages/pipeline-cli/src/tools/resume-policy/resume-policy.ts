 ...
 * with a hard K-cap on how many times ONE run may be resumed (the capped transient-resume policy, the originating initiative,
 * the child work item).
 *
 * This is the capped auto-resume MECHANISM that the capped transient-resume policy main-loop discipline runs:
 ...
 *
 *   - The crash class is decided by SIBLING the originating work item's `classify()` — composed, NOT
 ...
 *     classifier is optimistically wrong — the load-bearing safety property (the capped transient-resume policy).
...
/**
 * Decide what to do with a crashed run. Composes the originating work item's `classify()` for the class, then
	...
	if (ledger.priorResumes >= RESUME_CAP) {
		return {
			...
			rationale:
				...
				`is a masked LOGIC error; the cap bounds token burn (the capped transient-resume policy).`,
		};
	}
