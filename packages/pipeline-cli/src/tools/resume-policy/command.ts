 ...
 *
 * The capped, TRANSIENT-only auto-resume decision for a crashed dynamic Workflow. It
 * resumes only recoverable failures and stops after the configured per-run limit. It
 * prints the action word (`resume` / `surface`) to
 ...
 * which composes the originating work item's `classify()` for the class and applies the K=2 per-run cap. So
...
const decideCmd = Command.make(
	...
	Command.withDescription(
		"Decide resume vs surface for a crashed workflow: TRANSIENT + under the K=2 per-run cap → resume, else surface (the capped transient-resume policy; #1759)",
	),
);
...
export const resumePolicyCommand = Command.make("resume-policy").pipe(
	...
	Command.withDescription(
		"Capped TRANSIENT-only auto-resume for crashed dynamic workflows (the capped transient-resume policy, the originating initiative)",
	),
);
