 ...
 * auto-resume of crashed overnight drains (the originating initiative, the child work item).
 *
 * The taxonomy has exactly two classes, which need OPPOSITE treatment (the originating initiative):
 ...
 * This mirrors `drive-issue.js`'s `selectReviewTier` default-deny (the diff-complexity routing rule §3) — a
 ...
 * ACTUAL resume (and the K-cap) is sibling the originating work item's job — this child produces only the
...
/**
 * TRANSIENT signatures — the crash classes that re-run cleanly (the originating initiative story 1). A
...
/**
 * LOGIC signatures — same-inputs-re-crash classes (the originating initiative story 4). These are matched
