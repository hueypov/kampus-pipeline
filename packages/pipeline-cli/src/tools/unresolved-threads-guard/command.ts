 ...
 * (the unresolved-thread merge gate enforcement, the originating work item).
 *
 * The fail-closed machine gate behind the unresolved-thread merge gate's unresolved-inline-thread merge gate, giving
 ...
 * PR through (the zero-scope fail-closed invariant's zero-scope stance at the unreadable-channel seam). `GithubLive` is
 * baked in with `Command.provide(...)` so the registered command's residual requirement is the
 * Node platform union (the registry seam, the originating initiative).
 */
...
export const unresolvedThreadsGuardCommand = Command.make("unresolved-threads-guard").pipe(
	...
	Command.withDescription(
		"Fail-closed gate: no unaccounted unresolved inline review thread reaches a merge-ready PR (the unresolved-thread merge gate, #3331)",
	),
