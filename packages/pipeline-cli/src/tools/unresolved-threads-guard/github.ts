 ...
 *
 * REST for everything EXCEPT the one read the unresolved-thread merge gate sanctions as the single GraphQL exception:
 ...
 * tell resolved from unresolved (the unresolved-thread merge gate; verified working on this org, the Projects-classic
 ...
 *   2. the PR's issue comments (REST), author-gated to write+ collaborators (the runtime ACL authorization rule), latest
...
/**
 * Resolve the target repo (`owner/name`) once, per the repository-resolution contract §1:
...
// The ONE GraphQL read the guard makes — the unresolved-thread merge gate's single sanctioned exception (REST has no
...
/**
 * The write+ collaborator subset of `logins` — the runtime ACL authorization rule trust root. Each login is probed
 ...
 * (the runtime ACL authorization rule), keep only authorized markers, take the newest by (created_at, id) — the same
 ...
 * is deferred to first use (`Effect.cached`, the repository-resolution contract §1): the layer build is side-effect
