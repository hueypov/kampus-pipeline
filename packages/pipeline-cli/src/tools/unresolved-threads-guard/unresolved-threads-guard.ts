 ...
 *
 * Gives the unresolved-thread merge gate's unresolved-inline-thread merge gate the machine teeth it lacked:
 ...
 * was the ONLY line of defense. That is exactly what happened on the originating pull request: a CodeQL/GHAS
 * inline finding reached a human merger flagged merge-ready with no flag (the originating work item).
 *
 ...
 * (the documented failure bug) leaves the thread unaccounted → red. It does NOT re-decide the unresolved-thread merge gate's
 * substantive-vs-nit policy: a genuine nit is discharged the unresolved-thread merge gate way — resolve the
 * thread with a written rationale (isResolved=true removes it from the live set), never a
 * silent whole-channel skip. See the unresolved-thread merge gate.
 *
...
/**
 * One PR review thread as the unresolved-thread merge gate's sanctioned GraphQL `reviewThreads` read surfaces it.
 ...
 * the unresolved-thread merge gate sanctioned read keys solely on `isResolved`, and an outdated-but-unresolved
 ...
 * the IO shell, which reds when it cannot READ the thread state at all (the zero-scope fail-closed invariant's
...
const renderReport = (
	...
	return [
		...
		rows,
		"",
		"Per the unresolved-thread merge gate, discharge each by EITHER resolving the thread with a written rationale (a genuine nit),",
