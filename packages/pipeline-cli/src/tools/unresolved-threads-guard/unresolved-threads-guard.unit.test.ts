/**
 * Pure-core tests for `unresolved-threads-guard` (the unresolved-thread merge gate enforcement, the originating work item). The gate
 * decision over fixtures — deterministic, no live PR state:
 *   - the documented failure exemplar reproduction: an unresolved GHAS/CodeQL thread on
 *     `.github/workflows/commands-guard.yml:35` + a review-code PASS with NO accounting row
 *     → RED, naming that thread (AC: re-running the check against the originating work item's state surfaces it);
 ...
 *   - a nit resolved-with-rationale (isResolved=true) → pass (the unresolved-thread merge gate discharge path);
...
// The the originating work item exemplar: the CodeQL/GHAS inline finding on the commands-guard workflow.
...
// A review-code PASS marker with NO unresolved-threads accounting — the documented failure bug shape.
...
describe("isAccounted", () => {
	...
	it("is true when the verdict names the exact path:line", () => {
		const verdict = `review-code: FAIL @ deadbeef\n- [FAIL] unresolved-threads — ${siteToken(codeqlThread)} @github-advanced-security: "no permissions" is substantive (the unresolved-thread merge gate)`;
...
describe("judge", () => {
	...
	it("passes when the only thread is resolved (the unresolved-thread merge gate nit discharge: resolve-with-rationale)", () => {
		const verdict = judge({
	...
	it("passes when a substantive thread is accounted-for by a FAIL unresolved-threads row", () => {
		const accounted = `review-code: FAIL @ 4da28749abc — one criterion unmet\n- [FAIL] unresolved-threads — ${siteToken(codeqlThread)} @github-advanced-security: "no permissions" → address on the branch (the unresolved-thread merge gate)`;
		const verdict = judge({threads: [codeqlThread], verdictBody: accounted});
		expect(verdict.pass).toBe(true);
	});

	it("REDS a HUMAN inline thread on the same footing as a bot thread (the unresolved-thread merge gate human-or-bot)", () => {
