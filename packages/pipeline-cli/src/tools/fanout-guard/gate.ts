/**
 * The `fanout-guard` filesystem gate (the declared mutation-publication guard) — the IO seam behind the
 ...
 * publish, or zero mutations in scope (fail-closed, the zero-scope fail-closed invariant). A directory/file IO
...
const readManifest = (
	...
	Effect.gen(function* () {
		...
		if (entries.length === 0) {
			...
			// not a vacuous pass — fail closed (the zero-scope fail-closed invariant).
			return yield* Effect.fail(
				new CheckFailed({
					reason:
						`fanout-guard: parsed ZERO rows from ${MANIFEST_PATH} — the manifest is empty or its ` +
						"row shape changed, so the classification is broken. Fail-closed (the zero-scope fail-closed invariant/0155).",
				}),
			);
		}
		return entries;
 ...
 * topic, else `CheckFailed`. Fails closed on zero mutations in scope (the zero-scope fail-closed invariant).
 */
