/**
 * The `fanout-guard` tool — `pipeline-cli fanout-guard check [--root <d>]` (the declared mutation-publication guard).
 *
 ...
 * fail-closed so the documented failure–the originating work item silent-omission class can't re-drift:
 *
 ...
 * publish, or zero discovered mutations (fail-closed, the zero-scope fail-closed invariant). The scan/IO lives in
...
export const fanoutGuardCommand = Command.make("fanout-guard").pipe(
	...
	Command.withDescription(
		"Fail-closed gate: every fanned mutation publishes its /fate/live invalidation (the declared mutation-publication guard, #1898)",
	),
);
