/**
 * `fanout-guard` pure core (the declared mutation-publication guard) — decide whether every worker `Fate.mutation`
 ...
 *      feature's `live.ts` binding. Closes the mis-aim class (the originating work item): invariant 2 only
 ...
 *
 * Fail-closed on zero scope (the zero-scope fail-closed invariant): zero discovered mutations is a
 ...
 * per the declared mutation-publication guard: a fanned mutation's FEATURE must reach the publisher, not necessarily
...
export type FanoutGuardVerdict =
	...
	/** No mutations discovered — fail closed, never a vacuous pass (the zero-scope fail-closed invariant). */
	...
	/**
	 * A fanned mutation's publish does not aim where the manifest declares (the originating work item):
...
export const renderReport = (verdict: FanoutGuardVerdict): string => {
	...
	if (verdict.reason === "zero-scope") {
		return (
			...
			"apps/web/worker/features/*/mutations.ts — fail-closed (the zero-scope fail-closed invariant). Is the repo root " +
	...
	if (verdict.reason === "drift") {
		...
		return (
			...
			"omit its /fate/live publish (the declared mutation-publication guard)."
		);
	}
	if (verdict.reason === "missing-publish") {
		...
		return (
			...
			"view goes stale. Add the publish, or reclassify " +
	...
	return (
		...
		"[...] to the topic the feature actually fans into."
	);
};
 ...
 * over the text: matches `"<entity.verb>": Fate.mutation(`. Dependency-free and sufficient for this
 ...
 * feature-scoped floor (the declared mutation-publication guard): a fanned mutation's feature must reach the
 ...
 *     view name minus its `View` suffix (`PostView.typeName === "Post"`, the declared mutation-publication guard
 *     view-sourced-typename convention, the originating work item). Anchored on `.typeName` so an unrelated
