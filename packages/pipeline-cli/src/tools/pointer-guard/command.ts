 ...
 *
 * The CI surface for the fail-closed stale-pointer gate over `**​/CLAUDE.md` (the originating work item):
 *
 ...
 *
 * `doc-links` (the originating work item) validates markdown `[text](path)` links and masks code spans;
 ...
 * repo, not just the package — same fix as decisions-index the originating work item).
 *
...
// disk. It walks to the path fixpoint, then falls back to the start; `fs.exists` yields an Effect,
