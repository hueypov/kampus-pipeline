 ...
 * stale-pointer gate for `**​/CLAUDE.md` (the originating work item). `gate.ts` wires it to the
 ...
 *
 * The gap this closes is the one the `doc-links` gate (the originating work item) cannot see by
...
/** Render the failure report: one `file:line  →  path` line per stale pointer (the zero-scope fail-closed invariant §1 "emit what you scanned"). */
