/**
 * The `pointer-guard` filesystem gate — the IO seam behind the originating work item's stale-pointer
 * scan, split out of `command.ts` so it is crossable in unit tests over a fake repo
 * dir rather than only by spawning the bin (the core-in-its-own-file idiom; the originating work item).
 ...
 *
 * Fail-closed on zero scope (the zero-scope fail-closed invariant): a scan that finds no CLAUDE.md is a
 ...
 * the bin's `NodeServices.layer`), so a gate `unit` test scripts an in-memory repo; a filesystem
 * fault folds `PlatformError` → `IoError`. The pure scanner (`findStalePointersIn`) filters
 ...
 * generated/runtime path like `apps/web/.env`); git IO stays a raw subprocess.
 */
 ...
 * scope (the zero-scope fail-closed invariant) — never a vacuous green on a mis-rooted scan.
 */
	...
	Effect.gen(function* () {
		...
		if (files.length === 0) {
			return yield* Effect.fail(
				new CheckFailed({
					reason:
						"pointer-guard: scanned ZERO git-tracked CLAUDE.md files — fail-closed (the zero-scope fail-closed invariant). " +
