 ...
 * the originating work item, the originating initiative).
 *
 * The collection layer between the corpus format (the originating work item) and the report slice (the originating work item): for
 ...
 * core, the measured no-quality-regression methodology §2), yielding a `{entry, grade, spend}` row. A per-(stage × model) collection
 ...
 * (the measured no-quality-regression methodology §2); `artifact` is the recorded decision artifact graded as-is. This is the documented
