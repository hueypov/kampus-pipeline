 ...
 *
 * the measured no-quality-regression methodology's token-economics gate is binary-per-run: it prices a stage's spend on a single
 ...
 *
 * The token inputs are sourced from the existing `token-spend` reconstruction (the measured no-quality-regression methodology §2 —
 ...
 * read-only. See the measured no-quality-regression methodology §2; this is the single meter, never a second one.
 */
