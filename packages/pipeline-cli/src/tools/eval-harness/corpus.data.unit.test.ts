...
describe("committed corpus — meaningful pass-rate, not n=1 (seed + ≥2 per stage)", () => {
	// AC1: each stage manifest seeds the measured no-quality-regression methodology §1 recorded input plus ≥2 more entries.
	const expected = [
		...
		it(`${file} carries ≥${min} ${stage} entries including the §1 seed #${seed}`, () => {
			...
			if (Result.isSuccess(result)) {
				...
				assert.isTrue(
					...
					`${stage} corpus must seed the measured no-quality-regression methodology §1 recorded input #${seed}`,
				);
			}
		});
	}
