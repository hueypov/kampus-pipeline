/**
 * eval-harness report core — the graded two-axis scorecard (the originating work item, the originating initiative).
 *
 * The top of the eval-harness vertical slice and the evidence artifact the model-tiering
 * decision (the originating work item) consumes. It aggregates the runner's graded `{entry, grade, spend}` rows
 * (`runner.ts`) into a per-(stage × model) scorecard on the measured no-quality-regression methodology §4 two-axis gate, now
 * GRADED: the token axis (billed + ex-cache-read spend per run, the measured no-quality-regression methodology §2) AND a graded
 ...
 * cell; it never selects or recommends a model. That call is the originating work item, a separate `type:decision`
...
export interface CellSpend {
	/** Mean billed tokens per run (the four-`usage`-component sum, the measured no-quality-regression methodology §2). */
	readonly billedPerRun: number;
	/** Mean ex-cache-read tokens per run (the cross-run comparator, the measured no-quality-regression methodology §2). */
...
export interface ScorecardCell {
	...
	/** `passedRuns / gradedRuns` — the graded quality axis (the measured no-quality-regression methodology §4). */
...
/**
 * Render the human-readable table the founder reads to decide the originating work item (`token-spend`/`ship-digest`
 ...
 * line states this is evidence for the originating work item, never a recommendation.
 */
