...
[the originating work item](https://github.com/example-org/example-repo/issues/1842), extending the measured no-quality-regression methodology).
...
slice ([the originating work item](https://github.com/example-org/example-repo/issues/1848)) shipped the corpus
**format + its decode/encode core**; the churn core
([the originating work item](https://github.com/example-org/example-repo/issues/1850)) is documented under
...
## The graded oracle ([the originating work item](https://github.com/example-org/example-repo/issues/1849))

`gradeEntry(entry, artifact): Grade` (`oracle.ts`) is the per-corpus-entry quality grade. the evaluation rule: grade reproducible stage outputs against frozen labels while tracking token spend with one consistent meter §3
...
a stage or call `gh` (that is the runner slice, the originating work item).

An entry passes iff the observed `artifact` reproduces its known-good `label`, per stage (the measured no-quality-regression methodology §3):
...
## The corpus runner ([the originating work item](https://github.com/example-org/example-repo/issues/1851))
...
core, the measured no-quality-regression methodology §2), producing a `{entry, grade, spend}` **row**. A per-(stage × model) collection
of those rows is the raw material the report slice ([the originating work item](https://github.com/example-org/example-repo/issues/1853))
...
- **Capture-manifest (story 7)** — `CaptureManifest` is the documented shape naming, per run, the
  transcript path (`<parent-session-id>/subagents/agent-<id>.jsonl`, the measured no-quality-regression methodology §2) + the recorded
...
([the originating work item](https://github.com/example-org/example-repo/issues/1853)), documented next.

## The report — graded two-axis scorecard ([the originating work item](https://github.com/example-org/example-repo/issues/1853))

`report.ts` is the **top of the vertical slice** and the evidence artifact used for
model-tiering decisions. It aggregates the runner's graded `{entry, grade, spend}` rows
into a per-(stage × model) **scorecard** with two axes:

- **Quality axis** — a **pass-rate** per (stage × model) over the corpus (`passedRuns / gradedRuns`).
- **Token axis** — the mean **billed** + **ex-cache-read** spend per run, plus the
...
and never selects or recommends a model — that call is the originating work item, a separate `type:decision`. Both
rendered surfaces carry a framing line pointing at the originating work item, and the JSON has no
...
```bash
# human table (default) — the founder reads this to decide the originating work item
...
```jsonc
{
  ...
  "framing": "This scorecard is measurement feeding the model-tiering decision (the originating work item); …",
...
the measured no-quality-regression methodology's apparatus grades **one** frozen input per stage with a **binary** oracle —
...
the measured no-quality-regression methodology's token-economics gate is **binary per run**: it prices a stage's spend on one
...
### Token grounding (the measured no-quality-regression methodology §2 — no second meter)
...
`agent-<id>.jsonl` transcript (the measured no-quality-regression methodology §2). `tokensFromTranscript` reuses that core
...
[`corpus/`](./corpus) (issue [the originating work item](https://github.com/example-org/example-repo/issues/1854)):
...
[the originating work item](https://github.com/example-org/example-repo/pull/1294) genuinely failed CI + earned
...
## Corpus-curation policy (the measured no-quality-regression methodology §1)

The corpus is governed by the **representative-task-set discipline** of the evaluation rule: grade reproducible stage outputs against frozen labels while tracking token spend with one consistent meter
...
- **Selection — representative, stable, reproducible-from-id.** An entry is a real
  ...
  "a recent issue"). Each stage seeds the measured no-quality-regression methodology §1 recorded input (triage
  [the originating work item](https://github.com/example-org/example-repo/issues/1227), write-code
  [the originating work item](https://github.com/example-org/example-repo/issues/1223) →
  [the originating work item](https://github.com/example-org/example-repo/pull/1224), review-code
  [the originating work item](https://github.com/example-org/example-repo/pull/1199)) and adds entries spanning the
...
**Making the tiering call** is [the originating work item](https://github.com/example-org/example-repo/issues/1576), a
