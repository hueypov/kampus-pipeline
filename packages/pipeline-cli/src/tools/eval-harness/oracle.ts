/**
 * eval-harness graded oracle — the per-corpus-entry quality grade (the originating work item, the originating initiative).
 *
 * the measured no-quality-regression methodology §3 defines a per-stage output-quality oracle: a reproducible pass/fail that asserts
 ...
 * pass-RATE can later be computed over the whole corpus (the report slice, the originating work item). See the measured no-quality-regression methodology §3.
 *
 * Two invariants hold, matching the corpus core's `decodeManifest` discipline:
 *  - Pure + total. The artifact is collected out-of-band by the runner slice (the originating work item), so it
...
// the measured no-quality-regression methodology §3 grades a finding SET, not an ordered list — dedup + sort so order and repeats
...
// Artifact schemas mirror each stage's frozen label shape in corpus.ts (the originating work item) — the OBSERVED
...
// See the measured no-quality-regression methodology §3 for every oracle definition below — do not re-invent a rubric.

/** triage passes iff the actual `{type, priority, status}` equals the label (the measured no-quality-regression methodology §3). */
 ...
 * (the measured no-quality-regression methodology §3).
 */
...
/** review-code passes iff the actual verdict + AC-finding set match the label (the measured no-quality-regression methodology §3). */
...
/** review-doc passes iff the actual verdict + doc-finding set match the label (the measured no-quality-regression methodology §3). */
...
/** ship-it passes iff the actual `{merged, mergeSha}` equals the label (the measured no-quality-regression methodology §3). */
 ...
 * fails its stage's shape grades `fail` with a stated reason rather than throwing. See the measured no-quality-regression methodology §3.
 */
