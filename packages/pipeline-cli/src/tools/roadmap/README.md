...
The **observability surface** of the steering seam (roadmap map the originating work item; part 3 of the three-part
seam settled in the originating work item — parts 1–2 are the active-milestone grounding + the arc-flip re-triage
...
▸ Four Pillars [active] → milestone #17 "…" [open]  ← ACTIVE ARC
    ◆ the originating initiative [open] p1 …
        · the originating work item [open] …
        · the originating work item [closed] …
    · the originating work item [open] …            (loose, non-epic issue)
    PRs:
        ⇢ the originating pull request build … → the originating work item
...
⚠ Drift: 2 stale p1(s) — open p1 outside the active-arc milestone #17 (…):
    the originating work item … (milestone #24)
    the originating work item … (no milestone)
```

The hierarchy is **arcs → milestones → epic trees → open PRs**, driven by:

- **ROADMAP.md's `## Arcs` / `## Campaigns` tables** — the sole parsed roadmap surface (the related failure modes).
...
decorative; surfacing them is the point of the view (the originating work item). With no single resolvable active arc,
...
is a GET. It is a *render*, not a guard: `roadmap-guard` (the originating work item) owns the fail-closed ROADMAP.md ↔
...
target repo resolves per the repository-resolution contract §1: `CLAUDE_PIPELINE_REPO` → `GITHUB_REPOSITORY` → `gh repo view`.
