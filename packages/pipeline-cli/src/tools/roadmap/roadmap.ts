 ...
 *
 * This is the observability half of the steering seam (the originating work item part 3): a render, not a guard.
 * `roadmap-guard` (the originating work item) owns the fail-closed ROADMAP.md ↔ milestone sync enforcement; this
 ...
 * keep draining after an arc flip if the lever were decorative (the originating work item).
 *
 * ROADMAP.md is the SOLE parsed roadmap surface (the related failure modes): the arc/campaign table parse is
...
/**
 * Stale p1 drift: open `p1` issues sitting outside the active-arc milestone (the originating work item). With no
