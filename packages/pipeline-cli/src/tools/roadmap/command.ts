 ...
 *
 * The observability surface of the steering seam (the originating work item part 3, roadmap map the originating work item): a read-only
 ...
 * registered command's residual requirement is the Node platform union (the registry seam, the originating work item).
 */
...
// ROADMAP.md's own tables plus the `## Dependencies` declaration, so the read is a plain file read
// (mirroring `view.ts`) rather than a `gh api` fetch. Deterministic on purpose: same tables in ⇒
// byte-identical block out, which is the property a later guard check can compare against.
const diagram = Command.make(
	"diagram",
	{root: rootFlag},
	Effect.fn(function* ({root: rootOpt}) {
		const md = readFileSync(join(resolveRoot(rootOpt), "ROADMAP.md"), "utf8");
