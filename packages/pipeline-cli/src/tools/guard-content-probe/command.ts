 ...
 * (the originating work item, founder ruling the originating work item).
 *
 ...
 *   pipeline-cli guard-content-probe classify --body-file decision.md --path .decisions/0001-sample.md
 *
 * The shared guard-touching-decision content probe, used so the review gate (review-doc /
 ...
 * and fail closed.
 *
 * IO here (the thin bin); the whole the guard-change ratification policy predicate lives in `guard-content-probe.ts` (the
...
const classifyCmd = Command.make(
	...
	Effect.fn(function* ({bodyFile, path, root}) {
		...
		yield* Effect.sync(() =>
			process.stderr.write(
				`guard-content-probe: ${label} → ${result.guardTouching ? "guard-touching (§CP, the guard-change ratification policy)" : "not-guard-touching"} [${result.reason}]\n`,
			),
		);
	...
	Command.withDescription(
		"Classify one .decisions/** ADR body as guard-touching (§CP) or not by content (the guard-change ratification policy, #3645)",
	),
);
...
export const guardContentProbeCommand = Command.make("guard-content-probe").pipe(
	...
	Command.withDescription(
		"Shared the guard-change ratification policy guard-touching-ADR content probe (review gate + driver + ship-it Step 0; #3645)",
	),
);
