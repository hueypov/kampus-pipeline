...
// gh-issue-intake-formats.md §CP block, the guard-change ratification policy).
const FORMATS_SLICE = [
...
describe("parseGuardAdrRe — single-source the §CP vocabulary (the guard-change ratification policy)", () => {
...
describe("probeGuardContent — the guard-change ratification policy content-shape predicate (#3645)", () => {
	...
	describe("guard-vocabulary match (§CP, the guard-change ratification policy)", () => {
		it("a guard-relaxing ADR is guard-touching", () => {
			const body = "# the descriptive/normative design firewall\n\nThis decision relaxes the fail-closed enforcement guard.";
