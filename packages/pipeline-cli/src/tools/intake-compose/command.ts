 ...
 * cite this verb instead of re-deriving the format by hand (the originating work item / the originating initiative):
 *
 ...
 * enforces the format-2 invariants (the ≥1-acceptance-criterion hard floor), and emits the composed body
 * to **stdout only**.
 *
 ...
 * body"; the originating work item / the originating work item / the originating pull request) is structurally unreachable. There is deliberately
...
export const intakeComposeCommand = Command.make("intake-compose").pipe(
	...
	Command.withDescription(
		"Compose an intake body per the gh-issue-intake-formats.md prose contract — one composer, not N re-derivations (#3254 / the originating initiative)",
	),
);
