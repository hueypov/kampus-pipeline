 ...
 *
 * The operable surface for the derived changelog rule (the originating work item), moved into the pipeline-cli registry
 * (the originating initiative, Phase 2 / the originating work item):
 *
 ...
 *
 * Untrusted entries JSON is decoded into `ChangelogEntry[]` at this boundary, and everything
...
export const changelogDeriveCommand = Command.make("changelog-derive").pipe(
	...
	Command.withDescription(
		"Derive CHANGELOG.md as a projection of shipped-work pipeline metadata (the derived changelog rule)",
	),
);
