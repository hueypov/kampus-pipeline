 ...
 * not the file's own prose. This module owns the `type:*` → Keep-a-Changelog category map
 * (`TYPE_CATEGORY`), unit-tested here. An entry with no recognized `type:*` is mapped
 * to the `Uncategorized` category — surfaced, never silently dropped.
 *
 ...
 * is an explicit "flag, never drop" category — an entry whose issue
