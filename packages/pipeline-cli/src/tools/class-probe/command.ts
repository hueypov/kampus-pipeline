/**
 * The `class-probe` tool — `pipeline-cli class-probe classify [flags]` (the originating work item).
 *
 ...
 * they cannot disagree on a diff's required class coverage (the originating work item, the `.glossary/**→has-code`
 * miss on the originating pull request). Reads the changed-file list from stdin (or `--files-from`), reads the
 ...
 * instead, appending `review-design`). Folding has-ui in here is the documented failure/the originating work item fix: the
