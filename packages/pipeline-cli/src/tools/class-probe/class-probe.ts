 ...
 *
 * The classification RESULT was already regex-correct on `main`, yet the originating pull request still
 ...
 * to enqueue (the originating work item). An LLM eyeball is not a probe. This core makes the probe
 ...
 * output the reviewer fan dispatches rather than an eyeball it can skip (the related failure modes).
 */
 ...
 *
 * No-class fail-closed (the originating work item): a changed file matching NONE of the three class probes —
 ...
 * zero classes, so ship-it required zero gates and it could merge un-reviewed (the originating pull request was
 ...
 * is not a fourth class or a widened regex (that regex single-source is the originating work item); it is the
 * fail-closed default of the same the zero-scope fail-closed invariant idiom the FAILCLOSED_PROBES over-dispatch uses.
 */
 ...
 * or the PR deadlocks on a phantom-empty review-design namespace (the related failure modes). A `.tsx`/`.css`
 * OUTSIDE apps/web/src is not has-ui — the scope is `^apps/web/src/` only, so the require predicate
 * never exceeds review-design's dispatch/off-ramp (the originating work item). The carve-out (the originating work item) exempts an
 * all-test/spec src diff — `*.test.tsx` / `*.spec.ts` render no surface, so a required review-design
 * on them could only ever no-op PASS (the documented failure/the originating work item ship stall); a real component or a mixed
