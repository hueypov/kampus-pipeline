...
[the originating work item](https://github.com/example-org/example-repo/issues/2434)).

## Why it exists

The reviewer's multi-class fan (the originating work item) and ship-it Step 0 both re-resolve the same
`HAS_*_RE` probes from `gh-issue-intake-formats.md` §CLASS, so on paper their class sets
agree. But on **the originating pull request** the reviewer read `.glossary/TERMS.md` as a doc surface and
...
`review-doc`; the originating work item). This tool makes the classification **executable and unit-tested** —
