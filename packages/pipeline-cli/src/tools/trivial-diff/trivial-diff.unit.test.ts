...
// The live CONTROL_PLANE_RE, IMPORTED from its single source (the originating work item) — never re-literaled
// here, so this fixture cannot drift from the boundary the way the old hand-copy did (the
// the originating work item class). The bin re-resolves it from origin/main at run time; the core takes it as a
...
// The live GUARD_ADR_RE (the guard-change ratification policy), parsed from the same single source the shared
...
describe("classify — §CP forces non-trivial (live boundary, checked first)", () => {
	...
	it("a pipeline agent-definition path (claude-plugins/…/agents/) → non-trivial (the agent-definition control-plane boundary)", () => {
		const c = classify(
...
describe("classify — a guard-touching .decisions/** ADR forces non-trivial (the guard-change ratification policy / #3645)", () => {
	it("an ADR whose added content relaxes a guard → non-trivial (never the trivial doc branch)", () => {
		const c = classify(
			fileDiff(".decisions/0001-sample.md", [
...
// codeowners-cp + validate-gate-path-drift.sh but cheap belt-and-suspenders (the related failure modes).
