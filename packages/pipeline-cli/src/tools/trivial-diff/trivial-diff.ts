 ...
 * as `trivial` or `non-trivial` for the right-sized fan-out. Only small,
 * single-concern, non-control-plane changes use the reduced independent review gate.
 *
 * This file holds the parsing + the classification over plain strings, with no disk
 * and no network (the core-in-its-own-file idiom; the originating work item). `command.ts` is the thin CLI
 ...
 *
 * The contract is **fail-closed / default-deny** (the diff-complexity routing rule §3): a diff is `trivial`
 ...
 * never a taste call), in the spirit of the bounded investigation-fix collapse rule's four bounds:
 ...
 *      touched `.decisions/**` ADR is guard-touching by content (the guard-change ratification policy / the originating work item). A
 ...
 *      the originating work item closes. The guard-touching content check is the SAME shared verb ship-it Step 0
...
export interface ClassifyOptions {
	 ...
	 * by the bin (the diff-complexity routing rule §1 bound 3 / §3). `null` means the boundary could not be
	...
	/**
	 * The live `GUARD_ADR_RE` (the guard-change ratification policy), re-resolved from `origin/main` by the bin — the SAME
 ...
 * or build/stack config. Any change here is a new-surface change (the diff-complexity routing rule §1 bound 2)
...
/** True for a `.decisions/**` ADR — the class the guard-touching content bound (the guard-change ratification policy) scopes to. */
 ...
 * new-surface markers; routes/bindings/config-keys/schemas are
 * caught by `isSurfacePath` or arrive as one of these edges. Anything else flows
 * through the single-file ≤ N bound. The reduced-review path is enabled only after
 * measured evaluation, so the conservative scope here is deliberate, never a gap.
 */
...
export const classify = (diff: string, opts: ClassifyOptions): Classification => {
	...
	if (re === null || re.trim() === "") {
		return nonTrivial("live CONTROL_PLANE_RE unreadable — fail-closed (the diff-complexity routing rule §3).");
	}
	let cp: RegExp;
	try {
		cp = new RegExp(re);
	} catch {
		return nonTrivial("live CONTROL_PLANE_RE failed to compile — fail-closed (the diff-complexity routing rule §3).");
	}
	const cpHit = files.find((f) => cp.test(f.path));
	if (cpHit !== undefined) {
		return nonTrivial(
			`control-plane path touched (${cpHit.path}) — never trivial (the diff-complexity routing rule §1.3).`,
		);
	}
	...
	if (isSurfacePath(f.path)) {
		return nonTrivial(
			`surface-bearing path (${f.path}: dependency/manifest/migration/schema/config) — not trivial (the diff-complexity routing rule §1.2).`,
		);
	}
	if (hasNewSurfaceContent(f.addedLines)) {
		return nonTrivial(
			`new module edge added (export/import/require) in ${f.path} — not trivial (the diff-complexity routing rule §1.2).`,
		);
	}

	// Bound 3 (content) — a guard-touching `.decisions/**` ADR is §CP by nature (the guard-change ratification policy) and
	// must NEVER ride the single-doc-file trivial branch below onto the lighter gate (the originating work item). Probe
	...
	if (isAdrPath(f.path)) {
		...
		if (probe.guardTouching) {
			return nonTrivial(
				`guard-touching .decisions/** ADR (${f.path}: ${probe.reason}) — §CP by content, never trivial (the guard-change ratification policy / #3645).`,
			);
		}
	}
	...
	return nonTrivial(
		`single file but ${changed} changed lines > N=${opts.lineBudget} — not trivial (the diff-complexity routing rule §1.1).`,
	);
};
