 ...
 * gate a PR takes. Only small, single-concern, non-control-plane changes use the
 * reduced independent review gate.
 *
 ...
 * (the diff-complexity routing rule §3). The workflow inlines the same one-line rule because a workflow script —
 ...
 *
 * **Default-deny (the diff-complexity routing rule §3).** The lighter path is selected ONLY on the positive
 * conjunction — the tier is enabled AND the classifier ran OK AND its verdict is exactly
 * `trivial`. ANY other state — the tier disabled (its default, pending the originating work item's
...
export interface RouteInput {
	/**
	 * Whether the trivial tier is switched on at all. It is **off by default** (the diff-complexity routing rule
	 * §4 + the measured no-quality-regression methodology): the branch is wired but a pure no-op until the child work item's two-axis
...
/**
 * Decide the review tier for a PR — default-deny (the diff-complexity routing rule §3). Returns `"lighter"` ONLY
