 ...
 *
 * The deterministic, fail-closed trivial-diff classifier: only small, single-concern,
 * non-control-plane changes use the reduced independent review gate.
 ...
 * wired into the executor (that is sibling the originating work item) and the lighter gate it routes to is
 * sibling the originating work item's measurement-gated adoption. It ships correct, tested, and dormant.
 *
 ...
 *     REST raw contents endpoint (`?ref=main`) — never a stale snapshot (the documented failure
 *     mis-classification class; the diff-complexity routing rule §1.3). An unreadable boundary resolves to
 ...
 * (the originating pull request, a `biome.jsonc` lint fix) is 5 changed lines in one file, and the
 * trivial-path motivating case (the originating work item, a one-line `CLAUDE.md` doc fix) is a single
 * line — both well within. 20 covers the measured single-file trivial class (one-line
 * stub/comment/value fixes the documented failure small-PR drain names) with headroom while staying
 * far below a reviewable multi-hunk refactor. Adoption is measurement-gated (the measured no-quality-regression methodology,
 * sibling the originating work item), so `N` is a tunable starting bound, overridable with `--max-lines`.
 */
 ...
 * (`GUARD_ADR_RE`, the guard-change ratification policy). Returns null on any failure (gh missing/unauth, repo unresolved,
...
const classifyCmd = Command.make(
	...
	Command.withDescription(
		"Classify a unified diff as trivial / non-trivial (fail-closed; the diff-complexity routing rule §1)",
	),
);
...
export const trivialDiffCommand = Command.make("trivial-diff").pipe(
	...
	Command.withDescription(
		"Deterministic fail-closed trivial-diff classifier (the diff-complexity routing rule §1, the originating initiative)",
	),
);
