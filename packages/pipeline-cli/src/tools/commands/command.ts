/**
 * The `commands` tool — `pipeline-cli commands <compact|check>` (the originating work item).
 *
 * The rot-proof tool-discovery surface, mirroring `decisions-index`: it derives from each command's own name and description.
 ...
 * purpose (AC #4), and a zero-length registry reds (fail-closed on zero scope, the zero-scope fail-closed invariant).
 *
...
const check = Command.make(
	...
	{},
	Effect.fn(function* () {
		// Fail-closed on zero scope: an empty registry is a wiring/load fault, not a pass (the zero-scope fail-closed invariant).
