/**
 * The `workflow-contract` filesystem gate — the IO seam behind the originating work item's Workflow-
 ...
 * (the zero-scope fail-closed invariant): a script that exists but cannot be read is an `IoError` (non-zero), and
...
/** The workflow-script surface this guard owns (the repository-resolution contract — repo-relative). */
 ...
 * clean; a present-but-violating (or unparseable) script reds — fail-closed (the zero-scope fail-closed invariant).
 */
