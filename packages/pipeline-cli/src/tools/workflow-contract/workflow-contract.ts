 ...
 * unit-testable over plain strings, no disk (the core-in-its-own-file idiom; the originating work item).
 *
 * THE CONTRACT (source-grounded in bug the originating work item's diagnosis — the lived defect where a
 ...
 *     documented intent, NOT a brittle must-reference-all rule (the originating work item triage note).
 *
 ...
 *
 *   1. NO `export default` (the documented failure load-breaker) — the primary reject.
...
export const judgeScript = (file: string, source: string): ScriptVerdict => {
	...
	try {
		...
		// A masking crash means the source is pathological — fail closed (the zero-scope fail-closed invariant).
		return {
			file,
			violations: [
