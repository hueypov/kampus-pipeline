...
const MAIN = "/Users/dev/example-repo";
...
describe("isManagedWorktree", () => {
	...
	it("normalizes backslash separators (windows-shaped path)", () => {
		assert.isTrue(isManagedWorktree("C:\\Users\\dev\\example-repo\\.claude\\worktrees\\agent-a"));
	});
...
describe("classifyWorktree — KEEP branches (the safety cases)", () => {
	...
	// The the originating work item liveness guard: clean+merged is NOT sufficient — a live sibling lane is
...
describe("classifyWorktree — REMOVE branches (clean AND reachable)", () => {
	...
	// The the originating work item case: a squash merge (the single merge-authority rule) rewrites the branch's commits into one
...
describe("computeWorktreeSweepPlan — partition", () => {
	it("partitions a mixed pile into removable + kept-with-reason", () => {
		const records: ReadonlyArray<WorktreeRecord> = [
			...
			// squash-merged-and-clean: tip not an ancestor, but content landed → remove (the originating work item)
			record({
				path: wtPath("e"),
