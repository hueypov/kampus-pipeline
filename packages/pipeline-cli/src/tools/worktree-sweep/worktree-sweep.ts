 ...
 * reason, for the operator's sanctioned bulk drain (the originating work item). IO-free and total:
 ...
 *
 * Three swept classes (the related failure modes):
 ...
 *   - **Gone-dir worktrees** (the originating work item) — ANY tree, managed or foreign, whose working directory
 ...
 *
 * The safety property is the whole point (MEMORY "Safe worktree prune", the originating work item AC):
 ...
 * (`squashMergedToOriginMain`: example-repo merges by squash per the single merge-authority rule, which rewrites
 * the branch's commits into one new commit, so the tip is NOT a commit-ancestor even
 * though its content is in `origin/main` — the originating work item). A dirty tree or a genuinely
 ...
 *
 * Liveness guard (the originating work item FAIL): clean-AND-merged is NOT sufficient to remove. On the
...
export interface WorktreeRecord {
	 ...
	 * flagged it `prunable` (its gitdir points at a non-existent location, the originating work item). Only
	 ...
	 * `reachableFromOriginMain` misses (the single merge-authority rule, the originating work item).
	 */
	readonly squashMergedToOriginMain: boolean;
	/**
	 * The three liveness facts (the originating work item) — each fail-safe toward KEEP, gathered at the git
...
export type RemoveReason =
	 ...
	 * via `git worktree prune` (the originating work item). No `git worktree remove` (the path is missing); the
	...
	| "squash-merged-clean"
	/**
	 * A `review-head-*` throwaway detached checkout that is clean + unlocked + idle (the originating work item). No
	 ...
	 * to PR lifetime), defeating the reclaim; the documented failure liveness triple (dirty/locked/recently-active)
 ...
 *      (the originating work item). Wins over everything, INCLUDING `not-managed`: a gone-dir tree — a foreign
 ...
 *   3. Liveness gates (the originating work item) — locked / recently-active → KEEP, for BOTH classes. A clean
 ...
 *      lane whose branch has an open PR (the originating work item) — an open-PR review-head tree never reaches this
 ...
 *      the originating work item case: a squash merge (the single merge-authority rule) leaves the tip un-ancestored but lands the content.
