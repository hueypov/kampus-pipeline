/**
 * End-to-end guard for the SessionStart cadence (the originating work item): the wiring runs
 ...
 * KEPT (the documented failure liveness guard — a live sibling lane must never be swept). The
...
describe("worktree-sweep --execute — SessionStart cadence against a REAL git repo (#2238/#2240)", () => {
	...
	// the originating work item: a tree whose working dir was cleaned out from under it (a dead session's temp root)
