 ...
 * the originating work item). It sweeps three leaked classes (the related failure modes): the harness-provisioned build trees
 ...
 *      threshold, and with no open PR (the documented failure liveness guard). Dirty / unmerged /
 ...
 *      (git does NOT refuse a clean tree a *sibling* holds as CWD — the originating work item) — the
 ...
 * (the single merge-authority rule) rewrites the branch's commits into one new commit, so the branch tip is
 * NOT a commit-ancestor of `origin/main` and `--is-ancestor` misses it (the originating work item). This
...
/**
 * Is a managed worktree still LIVE by recency (the originating work item)? Probes the worktree dir + its
...
/**
 * Does `branch` have an OPEN PR on the GitHub origin (the originating work item)? An open PR marks an
	...
	Effect.fn(function* ({execute}) {
		...
		const records: ReadonlyArray<WorktreeRecord> = parsed
			...
			.map((p) => {
				...
				// checks `prunable` first, so the other facts are never consulted for it (the originating work item).
				if (p.prunable) {
					return {
						path: p.path,
						branch: p.branch,
				...
				// A review-head tree is a swept candidate too (the originating work item), so its liveness (idle mtime)
		...
		// the zero-scope fail-closed invariant "emit what you scanned": the full plan is observable before any action.
		yield* Console.log(
