 ...
 * shared primary checkout's `refs/heads/main` (the originating work item). Wired as git's own
 ...
 * the originating work item `worktree-guard` bash-pin) structurally cannot reach: that guard only arms for
 * a `$WORKTREE_ROOT` subagent and only matches its `HEAD_MOVING` set, while the documented failure
 ...
 * remediation exits 1 on a fresh/partial checkout (the originating work item), and a fail-closed-on-1 hook
 * would then abort every ref transaction repo-wide, exactly the documented failure/the originating work item fail-open
...
const referenceTransaction = Command.make(
	...
	Effect.fn(function* ({state}) {
		...
		// refs/heads/main divergence guard (the originating work item), and the batch-level HEAD-detach guard on
		// the primary checkout (the originating work item — a detach moves HEAD, not refs/heads/main, so it is a
		...
		if (verdict.kind === "refuse") {
			// the zero-scope fail-closed invariant "emit what you scanned": name the guarded ref + why before aborting.
