 ...
 *
 * Why at git's ref boundary (the originating work item root cause): the PULLER role force-moved the primary's
 * `main` off the merge seam onto a diverged commit — a "one `git push -f` clobbers
 * `origin/main`" loaded gun. The the originating work item `worktree-guard` bash-pin can't reach this class (the
 ...
 *
 * Safety property (the originating work item): a `refs/heads/main` update that would make local `main` a
 ...
 *
 * A second, orthogonal concern lives here too (the originating work item, the mechanical half): `decideHeadDetach`
 ...
 * then the documented failure divergence refuse (fail-closed).
 */
...
/**
 * Refuse a bare HEAD-detaching checkout on the shared PRIMARY checkout — the documented failure hazard: a
 ...
 * them: the symref-in-transaction work of git 2.45; the related failure modes regrounding). A detaching checkout
