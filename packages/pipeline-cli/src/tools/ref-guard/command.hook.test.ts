...
// vitest's 5000ms default (the documented failure timeouts). A generous suite-level timeout keeps the real-git
...
// guard only fired on CLI-invoked moves it would be theater with the documented failure hole; this proves
...
describe("ref-guard — installed reference-transaction hook fires on a BARE git ref-move (no pipeline command)", {
	...
	// Regression for Defect 2 (the originating work item follow-up): a `git fetch` updates refs/remotes/origin/* +
	...
	// Regression for Defect 1 (the related failure modes fail-open): when the CLI can't RUN (missing bin /
	...
	// the originating work item mechanical half (criteria 1/2/4): the SAME installed reference-transaction hook that
	...
	it("does NOT abort an attached commit on a feature branch on the PRIMARY (HEAD paired with its branch move)", () => {
		// A feature branch, not main, so the pre-existing the originating work item main-divergence guard is out of the
	...
	// The load-bearing PULLER flow (the originating work item): git ≥ 2.45 (CI/prod 2.55) queues this reattach as a HEAD
