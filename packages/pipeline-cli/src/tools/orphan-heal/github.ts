...
/** Resolve the target repo (`owner/name`) per the repository-resolution contract §1: env override → `GITHUB_REPOSITORY` → `gh repo view`. */
 ...
 * issue — a hand-/conversation-authored ADR/ROADMAP PR — is laneless (the orphan shape, the originating work item).
 */
 ...
 * deferred to first use (the repository-resolution contract §1), so the layer build is side-effect-free.
 */
