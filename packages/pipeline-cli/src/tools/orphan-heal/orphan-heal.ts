 ...
 * plan of "heal red CI on PR #N" items to file (the documented failure boundary path, steps 1–2).
 *
 * Founder ruling on the originating work item: engines heal only lanes they OWN. An orphan red PR — open,
 * CI-red, in no engine lane (hand-/conversation-authored, e.g. an ADR/ROADMAP PR that
 * closes no triaged issue — the originating work item) — is never healed, because no engine adopts a PR with no
 ...
 * a PR that already has an open one (the originating work item AC: idempotent emitter).
 */
 ...
 * boundary-ruling context an engine needs to adopt the lane (the documented failure boundary path).
 */
