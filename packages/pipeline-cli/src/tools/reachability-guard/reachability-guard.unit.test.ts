/**
 * Pure-core tests for `reachability-guard` (the feature reachability requirement, the originating work item): the reachable pass (a
 ...
 * (the zero-scope fail-closed invariant), and the keys.ts / journey-tag source parses. No IO — the filesystem seam is
...
describe("judge — fail-closed on zero scope (the zero-scope fail-closed invariant)", () => {
	it("FAILS with zero-scope when no flag definitions are parsed", () => {
		const verdict = judge(facts("example-repo-reactions", [], [], []));
		expect(verdict.pass).toBe(false);
		expect(verdict.pass === false && verdict.reason).toBe("zero-scope");
	});
});

describe("judge — unknown/unclassified flag fails closed (the feature reachability requirement §1)", () => {
	it("FAILS with unknown-flag when the key is not declared in keys.ts", () => {
		const verdict = judge(
			facts("made-up-key", [def("PHOENIX_REACTIONS", "example-repo-reactions")], [], []),
		);
		expect(verdict.pass).toBe(false);
...
describe("judge — the reachable pass (consumer + journey)", () => {
	it("PASSES when a .tsx consumes the constant AND a journey e2e is registered", () => {
		const verdict = judge(
			facts(
				"example-repo-reactions",
				[def("PHOENIX_REACTIONS", "example-repo-reactions")],
				["PHOENIX_REACTIONS"],
				["example-repo-reactions"],
			),
		);
...
describe("judge — the two fail modes name what's missing", () => {
	it("FAILS unreachable with missingConsumer when no .tsx references the constant", () => {
		...
		const verdict = judge(
			facts(
				"example-repo-karma-gates",
				[def("PHOENIX_KARMA_GATES", "example-repo-karma-gates")],
				[],
				["example-repo-karma-gates"],
			),
		);
	...
	it("FAILS unreachable with BOTH missing when neither a consumer nor a journey exists", () => {
		const verdict = judge(
			facts("example-repo-reactions", [def("PHOENIX_REACTIONS", "example-repo-reactions")], [], []),
		);
		expect(
			verdict.pass === false &&
...
describe("judge — the exemption path (the feature reachability requirement §3)", () => {
...
describe("renderReport", () => {
	it("names the missing UI consumer on an unreachable fail", () => {
		const report = renderReport({
			pass: false,
			flagKey: "example-repo-karma-gates",
			reason: "unreachable",
	...
	it("names the missing journey e2e on an unreachable fail", () => {
		const report = renderReport({
			pass: false,
			flagKey: "example-repo-reactions",
			reason: "unreachable",
		...
		expect(report).toContain("@journey:example-repo-reactions");
	});
...
describe("parseFlagDefinitions — read constant/key/exemption rows from keys.ts source", () => {
	it('parses each export const NAME = "flag-key" row', () => {
		const source = `
			...
			/** Reactions flag. */
			export const PHOENIX_REACTIONS = "example-repo-reactions";
		`;
		expect(parseFlagDefinitions(source)).toEqual([
			{constantName: "PANO_DRAFT_SAVE", flagKey: "pano-draft-save", exemptReason: null},
			{constantName: "PHOENIX_REACTIONS", flagKey: "example-repo-reactions", exemptReason: null},
		]);
	});
...
describe("parseJourneyTags — read @journey:<key> registrations from a spec source", () => {
	it("extracts the flag key from a @journey tag in a describe title", () => {
		const source = `test.describe("Reaction bar @journey:example-repo-reactions", () => {});`;
		expect(parseJourneyTags(source)).toEqual(["example-repo-reactions"]);
	});
