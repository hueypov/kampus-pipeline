 ...
 * (the feature reachability requirement).
 *
 * The one shared reachability contract both `plan-epic` (the originating work item) and `/release` (the originating work item)
 ...
 * (fail-closed, the zero-scope fail-closed invariant). A UI-less infra flag opts out with `@reachability-exempt:
...
const flagKeyArg = Argument.string("flag-key").pipe(
	Argument.withDescription("the Flagship flag key to assert reachable (e.g. example-repo-reactions)"),
);
...
export const reachabilityGuardCommand = Command.make("reachability-guard").pipe(
	...
	Command.withDescription(
		"Fail-closed gate: a flag can't graduate while its user-facing slice is unbuilt (the feature reachability requirement, #2529)",
	),
);
