/**
 * `checkReachability` over a fake repo dir — the filesystem-seam test (the feature reachability requirement, the originating work item).
...
describe("checkReachability — the exit-code gate over a fake repo dir", () => {
	it("SUCCEEDS when a .tsx consumes the constant AND a journey spec is registered (the proof)", async () => {
		writeKeys([{constantName: "PHOENIX_REACTIONS", flagKey: "example-repo-reactions"}]);
		writeConsumer("ReactionBar.tsx", "PHOENIX_REACTIONS");
		writeJourneySpec("28-reactions.spec.ts", "example-repo-reactions");
		const exit = await run(checkReachability(root, "example-repo-reactions"));
