/**
 * `reachability-guard` pure core (the feature reachability requirement) — decide whether a Flagship flag key is
 ...
 *
 * Per the feature reachability requirement §1 a flag is reachable iff BOTH assertions hold, unless it is exempt:
 ...
 *       in-title `@journey:<flag-key>` tag (the feature reachability requirement §2; the checker asserts *registration*,
 *       never runs playwright).
 *
 * The exemption model (the feature reachability requirement §3): a legitimately UI-less infra/containment flag opts
 ...
 *
 * Fail-closed on zero scope (the zero-scope fail-closed invariant): zero parsed flag definitions is a broken scope
 * assumption (wrong root, a `keys.ts` reshape), NOT a vacuous pass; an unknown/unclassified
 * flag key is likewise a failure, never a silent pass (the feature reachability requirement §1).
 */
...
export interface FlagDefinition {
	...
	/** The flag-key string the constant binds — e.g. `example-repo-reactions` (the Flagship key). */
...
export type ReachabilityVerdict =
	...
	/** No flag definitions parsed at all — fail closed, never a vacuous pass (the zero-scope fail-closed invariant). */
	| {readonly pass: false; readonly flagKey: string; readonly reason: "zero-scope"}
	/** The requested key is not declared in `keys.ts` — unclassified, fail closed (the feature reachability requirement §1). */
 ...
 * assertions (an exempt flag needs neither a consumer nor a journey — the feature reachability requirement §3), then
...
/** Render the human-readable report (the zero-scope fail-closed invariant §1 "emit what you scanned"). */
export const renderReport = (verdict: ReachabilityVerdict): string => {
	...
	if (verdict.reason === "zero-scope") {
		return (
			...
			"fail-closed (the zero-scope fail-closed invariant). Is the repo root correct, or did the flag-key module layout change?"
		);
	}
	if (verdict.reason === "unknown-flag") {
		return (
			`reachability-guard: '${verdict.flagKey}' is not declared in apps/web/src/flags/keys.ts — ` +
			"an unknown/unclassified flag key fails closed (the feature reachability requirement §1). Add the flag-key constant, or " +
	...
	if (verdict.missingJourney) {
		lines.push(
			`  MISSING JOURNEY E2E — no spec under apps/web/tests/e2e/ carries a @journey:${verdict.flagKey} ` +
				"tag. Tag the vertical's journey spec's test/describe title with it (the feature reachability requirement §2).",
		);
	}
	return (
		`reachability-guard: ${verdict.flagKey} is UNREACHABLE — its vertical's user-facing slice is ` +
		`unbuilt, so it must not graduate to 100% (the feature reachability requirement):\n${lines.join("\n")}`
	);
};
 ...
 * consuming-UI signal (the feature reachability requirement §1a): a constant referenced by a component — an import,
...
/**
 * The flag keys a spec registers via `@journey:<flag-key>` tags in its source (the feature reachability requirement
