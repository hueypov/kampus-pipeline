/**
 * Pure-core tests for `readme-guard` (the related failure modes): the dead-shell-ignoring scope,
 * the fail-closed-on-zero verdict (the zero-scope fail-closed invariant), and the workspace-glob parse. No IO —
...
describe("judge — scope to real workspace members (package.json-bearing dirs)", () => {
	...
	it("FAILS zero-scope (fail-closed, the zero-scope fail-closed invariant) when no member has a package.json", () => {
		const verdict = judge([
