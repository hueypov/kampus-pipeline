/**
 * The `readme-guard` filesystem gate — the IO seam behind the related failure modes's "every
 ...
 * spawning the bin (the core-in-its-own-file idiom; the originating work item).
 *
 ...
 * members are in scope (fail-closed, the zero-scope fail-closed invariant). A directory/file IO failure is an
	...
	Effect.gen(function* () {
		...
		if (!globs.includes(PACKAGES_GLOB)) {
			...
			return yield* Effect.fail(
				new CheckFailed({
					reason: `readme-guard: pnpm-workspace.yaml does not declare the \`${PACKAGES_GLOB}\` member glob (found: ${globs.join(", ") || "<none>"}) — the guard's scope assumption is broken, fail-closed (the zero-scope fail-closed invariant).`,
				}),
			);
		}
	});
 ...
 * members in scope (the zero-scope fail-closed invariant).
 */
