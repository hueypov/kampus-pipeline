 ...
 * member carries a `README.md` (issues the related failure modes). IO-free and total: every
 ...
 * none is a dead shell (the documented failure pipeline-cli consolidation left ~10 such
 * stragglers — a stale `.turbo/` log and nothing else, tracked for removal by
 * the originating work item), NOT a package. So the guard FILTERS candidates to `package.json`-bearing
 ...
 *
 * Fail-closed on zero scope (the zero-scope fail-closed invariant): if the scan finds zero members it is a
...
export type ReadmeGuardVerdict =
	...
	/** No `package.json`-bearing member found — fail closed, never a vacuous pass (the zero-scope fail-closed invariant). */
 ...
 * then fails closed on zero members (the zero-scope fail-closed invariant), else reds on any member without a
 * `README.md`.
 */
...
export const renderReport = (verdict: ReadmeGuardVerdict): string => {
	...
	if (verdict.reason === "zero-scope") {
		return (
			...
			"fail-closed (the zero-scope fail-closed invariant). Is the repo root correct, or did the workspace shape change?"
		);
	}
