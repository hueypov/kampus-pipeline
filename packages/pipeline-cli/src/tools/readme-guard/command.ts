 ...
 * convention (issues the related failure modes). Records the convention in the root `CLAUDE.md` and
 * enforces it fail-closed so the gap can't silently re-drift (the same drift class
 * as the schema-mirror the originating work item / node:sqlite the originating work item incidents):
 *
 ...
 * a `package.json`. Bare directories (the documented failure consolidation's dead shells, tracked
 * for removal by the originating work item) are ignored, so the guard reds on a real README gap, not on
 * leftover litter. The scope is grounded in `pnpm-workspace.yaml`'s `packages/*`
 * glob. Fail-closed on zero members (the zero-scope fail-closed invariant). The scan/IO lives in `gate.ts`; this
