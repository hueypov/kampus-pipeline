...
describe("resumeCapDecision — single healthy-subagent resume cap (the verified-claims and capped-resume discipline (b) / #2053)", () => {
	...
	it("records the cap value and its rationale (the verified-claims and capped-resume discipline AC: K and its rationale recorded)", () => {
		expect(HEALTHY_RESUME_CAP).toBe(5);
		const capped = resumeCapDecision(ledger({cycles: HEALTHY_RESUME_CAP}));
		// the respawn rationale names the lifetime axis and cross-references the originating work item + the verified-claims and capped-resume discipline
		expect(capped.rationale).toMatch(/lifetime cap K=5/);
		expect(capped.rationale).toMatch(/the verified-claims and capped-resume discipline/);
...
// the originating work item's crash budget (the verified-claims and capped-resume discipline: the two axes stay distinct, a resume never double-counts).
describe("resume-cap is distinct from #1751's crash budget (the verified-claims and capped-resume discipline — no double-count)", () => {
	...
	it("a healthy chain under the crash cap (K=2) still resumes — the two caps do not sum", () => {
		// the originating work item's crash cap is K=2. A healthy subagent resumed 3 times (past the crash K, under the
