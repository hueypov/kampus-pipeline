...
describe("isCoderBackOff — sanctioned coder back-off detection (the originating work item)", () => {
...
describe("backOffResult — the distinct terminal result (the originating work item AC)", () => {
...
describe("post-build routing — a back-off short-circuits the review/repair loop (the originating work item)", () => {
...
// Strengthen the originating work item: a null agent-result at a stage boundary yields the structured abort
// (StageAborted → the workflow's `{ aborted }` return), never a raw TypeError.
describe("stageResult — a null agent() result aborts cleanly, not a TypeError (the originating work item)", () => {
