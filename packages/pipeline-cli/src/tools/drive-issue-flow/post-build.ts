 ...
 * loop or short-circuits on a sanctioned coder BACK-OFF (the originating work item).
 *
 ...
 * `freeze-after-2` — ~5 wasted agent dispatches misreported as a frozen lane (the originating work item).
 *
 ...
 * carries the unit test (the `trivial-diff/route.ts` sibling shape, the diff-complexity routing rule §3).
 *
...
/** The distinct terminal result a back-off short-circuits to (the originating work item AC). */
