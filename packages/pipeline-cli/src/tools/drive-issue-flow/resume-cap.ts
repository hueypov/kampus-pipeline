 ...
 * replaces it with a fresh spawn (the verified-claims and capped-resume discipline mitigation (b), the originating work item).
 *
 * ## Why this exists — and why it is NOT the originating work item's crash budget
 *
 * The the originating work item confabulation near-miss was a subagent that **did not crash**: a triager was
 ...
 *
 * It is deliberately **distinct from the originating work item / the capped transient-resume policy** (`resume-policy.ts`), which governs
 ...
 * replaced by a fresh spawn (the verified-claims and capped-resume discipline mitigation (b)).
 *
 * ## Rationale for K = 5
 *
 * The the originating work item near-miss degraded on a late cycle of a chain that ran ~20+ tool calls per cycle
 ...
 * lifetime axis — distinct from the originating work item's K = 2 crash budget, which is tighter because a
 ...
 * which counts *crash* resumes — the two are never summed (the verified-claims and capped-resume discipline: no double-count).
 */
 ...
 * (the verified-claims and capped-resume discipline / the originating work item). Otherwise RESUME, at `cycle = cycles + 1`.
 *
 ...
 * crash count — so it can neither be driven by nor perturb the originating work item's crash budget (the verified-claims and capped-resume discipline: the
	...
	if (cycles >= cap) {
		return {
			...
			rationale:
				...
				`A long resume chain correlates with confabulation (the verified-claims and capped-resume discipline mitigation (b) / #1876). ` +
