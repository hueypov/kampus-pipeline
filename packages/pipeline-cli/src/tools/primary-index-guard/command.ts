 ...
 *
 * The BLOCKING §CP promotion of the read-only local tripwire core (the originating pull request). Wired as
 * git's own `pre-commit` hook (`lefthook.yml`), it fires as a commit carrying the documented failure
 ...
 * residual vector the originating work item named: `ref-guard` allows a fast-forward, `main-sync` only catches its own
 ...
 * a not-yet-installed / stripped-PATH env (the originating work item) or a bin remediation exit (the originating work item) must NOT abort
...
import {appendRecord, defaultLogPath, parseNameStatus} from "./tripwire.ts";
 ...
 * git IO stays a raw read-only subprocess.
 */
