 ...
 * local tripwire core (the originating pull request). Decides whether a commit carrying the documented failure
 ...
 *
 * Why THIS choke point (the residual vector the originating work item named). `ref-guard`'s `reference-transaction`
 ...
 * tripwire still records it as the documented failure bleed class). Reuses the tripwire's `decideTripwire`
...
/**
 * Decide whether to REFUSE a commit carrying the documented failure signature on the PRIMARY checkout:
 *
 ...
 * the originating work item mass deletion" has exactly one definition.
 */
export const decidePrimaryIndexCommit = (input: PrimaryIndexCommitInput): PrimaryIndexDecision => {
	...
	return {
		kind: "refuse",
		reason:
			`refusing a commit — ${renderWarning(decision.record)}. ` +
