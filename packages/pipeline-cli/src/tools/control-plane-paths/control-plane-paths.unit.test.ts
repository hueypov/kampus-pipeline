...
describe("CONTROL_PLANE_RE classifies the executable-guard control-plane classification boundary broadenings (#2761)", () => {
	it("classifies the NEW §CP paths as control-plane", () => {
		// bare gate-critical `.sh` guard directly under skills/ (the originating work item)
		expect(
		...
		// the release + review-trivial skill dirs (the originating work item) — review-trivial emits SHA-bound,
		...
		// a `.sh` helper in a NON-gate skill's SUBDIR (the originating work item) — the escape set the any-depth
		// `([^/]+/)*[^/]+\.sh$` clause closes. `report/footer.sh` emits the filing-provenance
		// marker triage uses to determine auto-close eligibility, so it feeds a gate decision yet
	...
	it("classifies the lint/GritQL governance config as control-plane (the lint-governance control-plane rule)", () => {
		// An ungated path to weaken a lint rule is a guard-relaxing vector — same class as the enforcement-surface control-plane test's
	...
	it("does NOT classify known non-§CP paths", () => {
		...
		// (the originating work item, reverse of the originating work item). Its child PRs auto-merge on the normal review gates; only the
		...
		// Over-broadening guard (the originating work item): the any-depth clause matches `.sh` LEAVES only — a
		...
		// biome-governance §CP (the lint-governance control-plane rule) is tightly anchored: some OTHER root config/file stays
...
// gates read from origin/main (the originating work item) must stay byte-equal to it. This is the cheap in-test twin
// of the codeowners-cp + validate-gate-path-drift.sh guards that run unconditionally in CI (the originating work item).
