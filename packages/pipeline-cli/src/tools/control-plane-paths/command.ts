/**
 * `control-plane-paths` — emit the canonical §CP boundary deterministically (the originating work item).
 *
 ...
 * to force (the originating work item). The regex is printed with NO trailing formatting so `$(…)` capture
