...
describe("parseAdrFile", () => {
	...
	it("throws NumberMismatchError when the filename prefix disagrees with the id", () => {
		...
		// filename-NNNN collision hide behind distinct ids (the originating work item).
...
describe("renderCompact — the ambient map (the on-demand decision-discovery rule)", () => {
...
describe("findRootDir — cwd-independent repo-root resolution (#447)", () => {
	...
	it("walks up from a package dir to the ancestor carrying the marker", () => {
		// The the originating work item case: run from packages/decisions-index, marker lives at the root.
		const root = "/repo";
