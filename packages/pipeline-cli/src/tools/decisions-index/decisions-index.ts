 ...
 * committed `index.md` (the on-demand decision-discovery rule, superseding the hand-maintained-index approach); the legacy builders are
 ...
 *    at `check` time. This is the within-tree half of the originating work item; two stale concurrent PR branches
 *    each adding `0114-*.md` are out of scope until both land on `main` (see the generated decision-index rule / the originating work item).
 */
 ...
 * which goes stale or races two authors onto the same guess (the originating work item). A lettered id (`0034a`,
 ...
 * `validate` (`findDuplicateId`) on merge — the two together are collision-proof (the originating work item).
 */
 ...
 * ascending by id (the on-demand decision-discovery rule). No table, links, or committed file — surfaced on demand via
 * `pipeline-cli decisions-index compact` (the documentation-based decision-discovery contract dropped the SessionStart hook).
...
/**
 * Build the compact ambient map from the raw `.decisions/` files (the on-demand decision-discovery rule). Same
...
/**
 * The next free ADR number from the raw `.decisions/` files (the originating work item). Parses every
