 ...
 *
 * The author + CI surface, moved into the pipeline-cli registry (the originating initiative, Phase 2
 * / the originating work item):
 *   pipeline-cli decisions-index compact           # emit the compact ambient ADR map to stdout (the on-demand decision-discovery rule / the originating work item)
 *   pipeline-cli decisions-index next              # emit the next free ADR number (deterministic allocator — the originating work item)
 ...
 *
 * `compact` is the ambient-discovery surface (the on-demand decision-discovery rule): there is no committed
 * `.decisions/index.md` anymore — discovery goes ambient via a SessionStart hook
 * (the originating work item) that injects this one-line-per-ADR map, with `ls .decisions/` + frontmatter
 * as the fallback. It derives purely from ADR frontmatter, so it never drifts.
 * `next` is the deterministic ADR-number allocator (the originating work item): it prints `max(id) + 1`
 ...
 * collision-proof without a date-slug rename (supersedes the originating work item).
 * `validate` is the PR-side number-lock backstop (duplicate / mismatched id — the originating work item).
 * `generate`/`check` are legacy committed-index surfaces retained for local use only;
 * they are no longer wired into any workflow (the index is not committed — the on-demand decision-discovery rule).
 *
 ...
 * `.decisions` default resolves to `packages/<pkg>/.decisions` and ENOENTs (the originating work item).
 * Walk up from cwd for the first ancestor carrying a repo-root marker and read
 * `.decisions` there; this is foreign-repo-safe (no example-repo path hardcoded) and
 ...
 * `NodeServices.layer`), so this resolver is testable against an in-memory filesystem. The walk
 * continues to the path fixpoint, but `fs.exists` yields an Effect so it lives here; a probe fault
...
const compact = Command.make(
	...
	Command.withDescription(
		"Emit the compact ambient ADR map (one line per ADR: id · title · status) to stdout (the on-demand decision-discovery rule)",
	),
);
...
export const decisionsIndexCommand = Command.make("decisions-index").pipe(
	...
	Command.withDescription(
		"Ambient ADR discovery: emit the compact map, allocate the next ADR number, + validate ADR files (the on-demand decision-discovery rule / #2064)",
	),
);
