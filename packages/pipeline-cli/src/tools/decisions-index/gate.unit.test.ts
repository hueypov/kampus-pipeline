 ...
 * test (the originating work item). The pure derivation (`buildIndex` + parse/sort/dup helpers) is
 ...
 * (the generated decision-index rule) from observable outcomes — never by spawning the bin:
...
// The gate Effects require the `FileSystem | Path` seam (v4 platform migration, the originating work item);
...
describe("checkIndex — the CI exit-code gate over a fake .decisions dir (the generated decision-index rule)", () => {
...
describe("validateAdrs — the PR gate after the index stopped being committed per-PR (#1492)", () => {
	it("PASSES even when the committed index.md is stale (freshness is no longer required)", async () => {
		// The whole point of the originating work item: a PR that adds an ADR but does NOT regenerate the
