/**
 * The `check`/`generate` IO gate — the filesystem seam behind the generated decision-index rule's two
 ...
 * the originating work item). `bin.ts` wires these effects to the Effect CLI and maps `CheckFailed` to
 ...
 * directory instead of touching real disk; a filesystem fault folds `PlatformError` → the `IoError` this gate carries. This is the one why-note for
...
/** Build the compact ambient map (the on-demand decision-discovery rule), same duplicate-id fold as `build`. */
...
/**
 * The PR-time gate after the index stopped being committed per-PR (the generated decision-index rule, issue
 * the originating work item). It parses every ADR file via `build` — so a duplicate id, a
 * filename/front-matter number mismatch, or a malformed file still fails the build
 * (the documented failure number-collision guard is preserved) — but it does **not** compare
 ...
 * status`), the map the SessionStart hook injects (the on-demand decision-discovery rule / the originating work item). Pure read: it
 ...
 * runs instead of eyeballing `.decisions/` before `/adr` (the originating work item). Pure read over the
