#!/usr/bin/env node
/**
 * `pipeline-cli` — the subcommand-router bin (the originating initiative, Phase-1 scaffold the originating work item).
 *
 ...
 * (the originating work item) instead of a raw static-load throw. On catch, `loadWithSelfHeal` first
 * tries a bounded one-shot `pnpm install` + retry (the documented failure self-heal — see its
 * docblock in `module-load-guard.ts`); only if that still can't link the dep does
 * the legible remediation print and the bin exit(1) (the documented failure fallback, preserved).
...
const install = async (): Promise<void> => {
	...
	// destructive install would otherwise follow it into the shared primary and purge it (the originating work item).
	...
	// armed `--config.confirm-modules-purge=true` it aborts on a purge instead (the originating work item).
