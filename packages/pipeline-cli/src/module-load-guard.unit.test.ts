...
describe("loadWithSelfHeal", () => {
	...
	it("self-heals the unlinked-dep case: one install, then the retry succeeds", async () => {
		const load = vi
			.fn()
			.mockRejectedValueOnce(unlinkedPkgErr("unlinked-demo-dependency"))
		...
		expect(onHealAttempt).toHaveBeenCalledWith("unlinked-demo-dependency");
	});

	it("falls back to the #1798 remediation when the retry still can't link the dep", async () => {
		const stillUnlinked = unlinkedPkgErr("unlinked-demo-dependency");
