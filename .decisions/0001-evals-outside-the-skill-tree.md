...
Not addressed here: the *other* half of the same probe finding, that staging a skill's prose into a without-arm lets the baseline read `SKILL.md` and follow it, which collapses arm separation independently of where the eval set lives. That is a runner-design constraint rather than a repository layout decision, and belongs with the runner.
